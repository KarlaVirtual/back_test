...
/**
 * Este archivo contiene la implementación de un script para procesar transacciones de caja
 * en el sistema de Casino Astoria MX. Se encarga de validar tokens, procesar parámetros
 * de entrada, aplicar filtros y generar respuestas JSON con los movimientos financieros
 * realizados en un rango de fechas.
 *
 * @category   Red
 * @package    API
...
if ($TokenHeader === $token) {
    ...
    $rules = [];
    //array_push($rules, array("field" => "usuario_perfil.perfil_id", "data" => "CAJERO", "op" => "eq"));
    array_push($rules, array("field" => "d.mandante", "data" => "$Mandante->mandante", "op" => "eq"));


    if ($dateFrom != "") {
        // array_push($rules, array("field" => "x.fecha_crea", "data" => "$dateFrom", "op" => "ge"));
        //array_push($rules, array("field" => "usuario.fecha_actualizacion", "data" => "$dateFrom", "op" => "ge"));

    }
    if ($dateTo != "") {
        // array_push($rules, array("field" => "x.fecha_crea", "data" => "$dateTo", "op" => "le"));
        //array_push($rules, array("field" => "usuario.fecha_actualizacion", "data" => "$dateTo", "op" => "le"));
    ...
    if ($mandantes->count[0]->{".count"} != "0") {
        ...
        foreach ($mandantes->data as $key => $value) {
            ...
            /* if($value->{"x.tipomov_id"} == 'S') {
                 ...
                 $FechaCrea = $value->{"x.fecha_crea"};
                 //$TransactionType = "Online";
                 $MovementId ="P".$value->{"x.flujocaja_id"};
             }
             if($value->{"x.tipomov_id"} == 'E') {
