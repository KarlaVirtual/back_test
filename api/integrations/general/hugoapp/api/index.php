...
/**
 * Este archivo contiene un script para manejar solicitudes HTTP relacionadas con la integración de la aplicación Hugo.
 * Proporciona múltiples puntos de entrada para realizar operaciones como autenticación, vinculación de cuentas,
 * verificación de cuentas, afiliación, depósitos, retiros, y más.
 *
 * @category Red
 * @package  API
 * @author   Desconocido
 ...
 * @var mixed $Hugoapp           Variable que almacena información relacionada con la aplicación Hugo.
...
use Backend\integrations\general\Hugoapp;
...
// Genera un hash personalizado para validar la solicitud.
...
if (true) {
    // Manejo de diferentes operaciones basadas en el URI.
    if (strpos($URI, "GETUSERINFORMATION") !== false) {
        // Documentación para cada operación específica se encuentra en el código.
        $Username = $_REQUEST["userName"];
        $DocumentType = $_REQUEST["documentType"];
        $sign = $_REQUEST["sign"];

        $Hugoapp = new Hugoapp($token, $sign);
        $response = ($Hugoapp->AuthByDocument($Username, $DocumentType));
    ...
    if (strpos($URI, "LINKACCOUNT") !== false) {
        ...
        /* Procesamos */
        $Hugoapp = new Hugoapp($token, $sign, $UsuarioId);
        $response = ($Hugoapp->LINKACCOUNT());
    ...
    if (strpos($URI, "VERIFYACCOUNT") !== false) {
        ...
        /* Procesamos */
        $Hugoapp = new Hugoapp($token, $sign, $UsuarioId, $establishmentId);
        $response = ($Hugoapp->VERIFYACCOUNT($VerificationCode, $DeviceId, $CellPhone));
    ...
    if (strpos($URI, "PERFORMAFFILIATION") !== false) {
        ...
        /* Procesamos */
        $Hugoapp = new Hugoapp('', '');
        $response = ($Hugoapp->PERFORMAFFILIATION($data));
    ...
    if (strpos($URI, "DEPOSIT") !== false) {
        ...
        /* Procesamos */
        $Hugoapp = new Hugoapp('', '', $userName, $establishmentId);
        $response = ($Hugoapp->DEPOSIT($transactionId, $value, $data));
    ...
    if (false) {
        if (strpos($URI, "WIDTHDRAWALCONFIRMATION") !== false) {
            ...
            $response = ($Hugoapp->WIDTHDRAWALCONFIRMATION($confirmationCode));
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '', $userName, $establishmentId);
            $response = ($Hugoapp->WIDTHDRAWAL($transactionId, $value, $data));
        ...
        if (strpos($URI, "SLOTSGETGAMECATEGORIES") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '');
            $response = ($Hugoapp->getCategories());
        ...
        if (strpos($URI, "SLOTSGETGAMES") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '');
            $response = ($Hugoapp->getGames($Keyword, 'true', "", "", "", $CategoryId));
        ...
        if (strpos($URI, "SLOTSGAMELAUNCH") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '', $userName);
            $response = ($Hugoapp->launchGame($GameID, $Demo, $Ip));
        ...
        if (strpos($URI, "SLOTSBETTINGHISTORY") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '', $userName);
            $response = ($Hugoapp->getBettingHistory($startDate, $endDate, $state, $gameType));
        ...
        if (strpos($URI, "VIRTUALSGETGAMECATEGORIES") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '');
            $response = ($Hugoapp->getCategories('VIRTUAL'));
        ...
        if (strpos($URI, "VIRTUALSBETTINGHISTORY") !== false) {
            ...
            /* Procesamos */
            $Hugoapp = new Hugoapp('', '', $userName);
            $response = ($Hugoapp->getBettingHistory($startDate, $endDate, $state, $gameType, 'VIRTUAL'));
    ...
    // Respuesta de error general si no se cumple la validación.
    $return = array(
