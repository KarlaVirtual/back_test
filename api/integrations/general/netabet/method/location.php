...
/**
 * Este archivo contiene la lógica para procesar solicitudes relacionadas con la integración de Netabet.
 * Se encarga de validar tokens, procesar parámetros de entrada y generar respuestas JSON con información
 * sobre puntos de venta y usuarios asociados.
 *
...
/**
 * Variables globales utilizadas en el archivo:
 *
...
if ($TokenHeader === $token) {
    ...
    array_push($rules, array("field" => "usuario.mandante", "data" => "6", "op" => "eq"));
    ...
    $mandantes = $PuntoVenta->getPuntoVentasCustom3("usuario.usuario_id,usuario.estado_valida,usuario.fecha_crea,usuario.fecha_actualizacion,usuario.moneda,usuario.fecha_ult,punto_venta.*", "punto_venta.puntoventa_id", "asc", 0, 100000, $jsonbetshop, true);
    ...
    if ($mandantes->count[0]->{".count"} != "0") {
        ...
        foreach ($mandantes->data as $key => $value) {
            ...
            $array["LocationId"] = $value->{"usuario.usuario_id"};
