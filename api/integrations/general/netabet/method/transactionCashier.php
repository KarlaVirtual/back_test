...
if($TokenHeader  === $token){
    ...
    array_push($rules, array("field" => "d.mandante", "data" => "6", "op" => "eq"));
    ...
    array_push($rules, array("field" => "x.ticket_id", "data" => "", "op" => "eq"));
    ...
    if ($mandantes->count[0]->{".count"} != "0") {
        ...
        foreach ($mandantes->data as $key => $value) {
            ...
            if($value->{"x.recarga_id"} != 0 && $value->{"x.recarga_id"} != '') {
                ...
                $FechaCrea = $value->{"x.fecha_crea_time"};
               // $TransactionType = "Online";
                $MovementId ="D".$value->{"x.flujocaja_id"};
            }

            if($value->{".ticket_id"} != ''  && $value->{".ticket_id"} != 0) {
           ...
           /* if($value->{"x.tipomov_id"} == 'S') {
            ...
            if($value->{"x.cuenta_id"} != 0 && $value->{"x.cuenta_id"} != ''){
