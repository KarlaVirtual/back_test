...
/**
 * Este archivo contiene un script para procesar solicitudes relacionadas con tickets deportivos.
 * Se encarga de validar tokens, procesar parámetros de entrada, ejecutar consultas SQL y generar
 * respuestas en formato JSON con la información de los tickets.
 *
 * @category   Red
 * @package    API
...
GROUP BY ticket_id ";
    ...
    foreach ($Tickets as $key => $value) {
        ...
        switch ($statusInit) {
            ...
            case 'S':
                $statusEnd = "Won";
                if ($value["it_ticket_enc.fecha_pago"] == "" && $value["it_ticket_enc.fecha_maxpago"] > date("Y-m-d")) {
                    $statusEnd = "Expired";
                }
                break;
            case 'T':
        ...
        $array["LocationId"] = strval($value["it_ticket_enc.usuario_id"]);
        if ($value["usuario.puntoventa_id"] != '' && $value["usuario.puntoventa_id"] != '0') {
            $array["LocationId"] = strval($value["usuario.puntoventa_id"]);
        }
        ...
        $array["SoldValue"] = strval(intval(round($value["it_ticket_enc.vlr_apuesta"], 2)));


        if (in_array($value["it_ticket_enc.bet_status"], array('J', 'M', 'D', 'A'))) {
            $array["RefundValue"] = strval(intval(round($value["it_ticket_enc.vlr_premio"], 2)));
        } else {
            $array["RefundValue"] = '0';
        }

        $array["SettledValue"] = strval(floatval(round($value["it_ticket_enc.vlr_premio"], 2)) - floatval(round($value["it_ticket_enc.impuesto"], 2)));
        $array["BetTaxRate"] = '0';
        $array["BetTaxValue"] = '0';
        $array["WinTaxType"] = '';
        ...
        if (in_array($value["it_ticket_enc.bet_status"], array('J', 'M', 'D', 'A'))) {
            ...
            $array["WinValue"] = strval(floatval(round($value["it_ticket_enc.vlr_premio"], 2)));
            $array["WinTaxRate"] = $value["it_ticket_enc.impuesto_pagopremio"];
            $array["WinTaxValue"] = strval(floatval(round($value["it_ticket_enc.impuesto"], 2)));
        }
    ...
    if ( ! empty($final)) {
        ...
        $response["TotalCount"] = oldCount($final);
        $response["Data"] = $final;
    } else {
        $response["Error"] = false;
