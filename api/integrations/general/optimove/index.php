...
/**
 * Este archivo contiene un script para procesar solicitudes HTTP, registrar logs,
 * manejar excepciones y cargar dinámicamente métodos basados en la entrada recibida.
 *
 * @category   Red
 * @package    API
...
try {
    ...
    $log = $log . ($URI);
    $log = $log . ($body);
    syslog(LOG_WARNING, "OPTIMOVEREQUEST :" . $URI . ' - ' . json_encode($_SERVER) . json_encode($_REQUEST) . ($body));
