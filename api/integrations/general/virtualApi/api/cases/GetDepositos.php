...
/**
 * Archivo: GetDepositos.php
 *
 * Este archivo contiene un script para procesar solicitudes relacionadas con depósitos de usuarios.
 * Incluye la validación de tokens, manejo de fechas, y ejecución de consultas SQL para obtener
 * información sobre transacciones de recarga de usuarios.
 *
 * @category   Red
 * @package    API
...
WHERE 1 = 1
  ...
  AND ur.estado = '$estado' LIMIT $NumberRows,$OffSet";
        ...
        $sql =
            "SELECT tp.transproducto_id,tp.externo_id, ur.puntoventa_id,ur.recarga_id, u.usuario_id,
        u.nombre,
       u.mandante,
       u.pais_id,
       ur.valor,
       ...
       CASE
           WHEN p.descripcion IS NULL AND (pv.puntoventa_id IS NOT NULL OR ur.puntoventa_id != 0) THEN pv.descripcion

           ELSE p.descripcion END AS Producto,
       CASE
           WHEN p2.descripcion IS NULL AND (pv.puntoventa_id IS NOT NULL OR ur.puntoventa_id <> 0) THEN  'Punto Venta'
           ELSE p2.descripcion END AS Medio_de_Deposito,
       ur.fecha_crea 
  ...
  AND ur.estado = '$estado'  LIMIT $NumberRows,$OffSet";
        ...
        if ($count[0]->{".count"} != "0") {
            ...
            foreach ($transacciones as $key => $value) {
                ...
                if ($value->{"tp.externo_id"} != '') {
                    $array["ExternoId"] = $value->{"tp.externo_id"};
                } else {
                    $array["ExternoId"] = 0;
                }
                $array["SolicitudId"] = $value->{"tp.transproducto_id"};
                $array["UsuarioId"] = $value->{"u.usuario_id"};
                $array["Nombre_Usuario"] = $value->{"u.nombre"};
                $array["Valor"] = $value->{"ur.valor"};
                $array["Medio"] = $value->{".Medio_de_Deposito"};
                $array["Nombre_Medio"] = $value->{".Producto"};
