...
/**
 * Este archivo maneja la confirmación de pagos a través de la pasarela Kashio.
 * Procesa los datos recibidos, actualiza el estado de las transacciones y realiza
 ...
 * @var mixed $Kashio                      Variable que almacena información sobre la pasarela de pago Kashio.
...
if (isset($data)) {
    //Kashio
    $TransactionId = $data->data->invoice->external_id;
    $externoId = $data->data->invoice->id;
    $status = $data->type;
    $amonut = $data->data->invoice->amount;
    $comment = $data->type;

    if ($status == 'invoice.created') {
        $status = 'PROGRESS';
    } elseif ($status == 'invoice.cancelled' || $status == 'invoice.rejected') {
        $status = 'CANCEL';
    } elseif ($status == 'invoice.paid') {
        $status = 'SUCCESS';
    }

    /* Procesamos */
    $Kashio = new Kashio($TransactionId, $status, $externoId, $amonut);
    $response = $Kashio->confirmation(json_encode($data));

    $response = json_decode($response);

    if ($response->result == 'ISPAYOUT') {
        $estado = 'P';
        switch ($status) {
            case "SUCCESS":
                $estado = 'A';
