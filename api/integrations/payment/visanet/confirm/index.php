...
/**
 * Este archivo maneja la confirmación de pagos a través de la integración con Visanet.
 * Procesa los datos recibidos, registra logs y realiza la confirmación de transacciones.
 *
 * @category   Red
 * @package    API
 * @subpackage Integrations\Payment\Visanet
 * @author     Desconocido
 * @version    1.0.0
 * @since      2025-05-09
 */
 ...
 * @var mixed $Visanet      Variable que hace referencia a la red de pagos Visanet.
 */
...
use Backend\integrations\payment\Visanet;
...
if (isset($data)) {
    ...
    $invoice = $confirm->customer_reply->merchantTransactionId;
    $result = "APROBADO";
    $documento_id = $confirm->customer_reply->authorization;
    $valor = $confirm->customer_reply->totalAmount;
    $usuario_id = "";
    $control = "";


    /* Procesamos */
    $Visanet = new Visanet($invoice, $usuario_id, $documento_id, $valor, $control, $result);
    $Visanet->confirmation(json_encode($data));
}