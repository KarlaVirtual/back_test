 ...
 * Proporciona endpoints para autenticar, consultar saldo, reservar, liberar y cancelar reservas
 * relacionadas con sesiones de juego y transacciones.
 *
 * @category   Casino
 * @package    API
 ...
 * @var SimpleXMLElement $data     Objeto XML que representa los datos enviados en el cuerpo de la solicitud.
 * @var string           $log      Variable utilizada para almacenar información de registro (logs).
...
use Backend\integrations\casino\Playngoagg;
...
if (strpos($URI, "authenticate") !== false || $data->getName() == "authenticate") {
    ...
    $externalId = strval($Authenticate->externalId);

    $Playngoagg = new Playngoagg($Token, $AccessToken, $externalId, "", $gameId, "2");

    $response = ($Playngoagg->Auth());
    $log = $log . "/" . time();
...
if (strpos($URI, "balance") !== false || $data->getName() == "balance") {
    ...
    $Playngoagg = new Playngoagg($Token, $accessToken, $externalId, "", "", "2");

    $response = ($Playngoagg->getBalance($gameId));
    $log = $log . "/" . time();
...
if (strpos($URI, "reserve") !== false || $data->getName() == "reserve") {
    ...
    $response = ($Playngoagg->Debit($gameId, $roundId, "", $real, $transactionId, json_encode($datos)));
    $log = $log . "/" . time();
...
if (strpos($URI, "release") !== false || $data->getName() == "release") {
    ...
    if ($type == 1) {
        $response = ($Playngoagg->Debit($gameId, $roundId, "", 0, "FS" . $transactionId, json_encode($datos), true));
    }

    if ($roundId == '0') {
        $roundId = 'R0' . $transactionId;
        $response = ($Playngoagg->Debit($gameId, $roundId, "", 0, "R0" . $transactionId, json_encode($datos), true));

        $reconstructedPKT = simplexml_load_string($response);

        if ((string)$reconstructedPKT->statusCode == '2' || $reconstructedPKT->statusCode == '5') {
            $log = $log . "/" . time();
    ...
    $response = ($Playngoagg->Credit($gameId, $roundId, "", $real, $transactionId, $isEnd, json_encode($datos)));
    $log = $log . "/" . time();
...
if (strpos($URI, "cancelReserve") !== false || $data->getName() == "cancelReserve") {
    ...
    $response = ($Playngoagg->Rollback($gameId, $roundId, "", $real, $transactionId, json_encode($datos)));
    $log = $log . "/" . time();
