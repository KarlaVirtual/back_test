...
/**
 * Este archivo maneja la confirmación de pagos a través de la integración Monnet.
 * Procesa los datos recibidos, actualiza el estado de las transacciones y registra
 * los logs correspondientes en la base de datos.
 *
 * @category   Red
 * @package    API
 * @subpackage Integrations\Payout\Monnet
 * @author     Desconocido
 * @version    1.0.0
 * @since      2025-05-09
 */
 ...
 * @var mixed $components                  Variable que almacena los componentes utilizados en un proceso o transacción.
 ...
 * @var mixed $amountUsd                   Variable que almacena el monto de la transacción en dólares estadounidenses (USD).
 * @var mixed $bank                        Variable que almacena información sobre un banco específico.
 * @var mixed $customerName                Variable que almacena el nombre del cliente.
 * @var mixed $accountNumber               Esta variable indica la cantidad total de elementos o registros, útil para iteraciones o validaciones.
 * @var mixed $customeremail               Variable que almacena la dirección de correo electrónico del cliente.
 * @var mixed $customerCellphone           Variable que almacena el número de teléfono móvil del cliente.
 * @var mixed $observationBank             Variable que almacena observaciones del banco respecto a una transacción.
...
if (isset($data)) {
    if ($data->output->status != null && $data->payout->orderId != null) {
        $transactionid = base64_decode($data->payout->orderId);
        $components = explode('_', $transactionid);
        $transactionid = $components[0];

        $amount = $data->payout->amount;
        $currency = $data->payout->currency;
        $status = $data->output->status;
        $responseBank = $data->payout->id;
    } else {
        ...
        $status = $data->status;
        $responseBank = $data->responseBank;
        $observationBank = $data->observationBank;
    }
