 ...
 * @var mixed $TicketId            Variable que almacena el identificador del ticket.
 * @var mixed $CurrencyCode        Variable que almacena el código de la moneda utilizada en una transacción.
 * @var mixed $CouponCode          Variable que almacena el código de cupón.
 * @var mixed $SessionId           Variable que almacena el identificador de sesión.
 * @var mixed $JackpotContribution Variable que almacena la contribución al bote.
 * @var mixed $StakeGross          Variable que almacena el monto bruto de la apuesta.
 ...
 * @var mixed $CouponStatus        Variable que almacena el estado del cupón.
 * @var mixed $CloseRound          Variable que indica si se debe cerrar la ronda.
 * @var mixed $Won                 Variable que almacena si una apuesta ha sido ganada.
 * @var mixed $ReasonCode          Variable que almacena el código de razón.
...
use Backend\integrations\virtual\VirtualGeneration;
...
if ( ! empty($params)) {
    ...
    if ($method == "/admin/dao/integrations/virtual/goldenrace/api//spend") {
        $params_json = json_decode($params);
        $requestid = key(get_object_vars($params_json));
    ...
    if (strpos($method, "api/insertBet") !== false) {
        ...
        $JackpotContribution = $values->JackpotContribution;
        $StakeGross = $values->StakeGross;

        $json = [];

        $VirtualGeneration = new VirtualGeneration($SessionId);
        $respuesta = $VirtualGeneration->Debit($TicketId, $StakeGross, $CouponCode, json_encode($json));
    ...
    if ($method == "/admin/dao/integrations/virtual/goldenrace/api/cancel") {
        $params_json = json_decode($params);
        $requestid = key(get_object_vars($params_json));
    ...
    if ($method == "/admin/dao/integrations/virtual/goldenrace/api/payout") {
        $params_json = json_decode($params);
        $requestid = key(get_object_vars($params_json));
    ...
    if (strpos($method, "api/betResult") !== false) {
        $values = json_decode($params);
        ...
        $SessionId = $values->SessionId;
        $CloseRound = $values->CloseRound;
        $Won = $values->Won;

        $json = [];

        $VirtualGeneration = new VirtualGeneration($SessionId);
        $respuesta = $VirtualGeneration->Credit($TicketId, $Won, $CouponCode, $CloseRound, $CouponStatus, json_encode($json));
    ...
    if (strpos($method, "api/cancelBet") !== false) {
        $values = json_decode($params);


        $TicketId = $values->TicketId;
        $SessionId = $values->SessionId;
        $ReasonCode = $values->StakeGross;
