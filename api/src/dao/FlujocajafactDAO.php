<?php namespace Backend\dao;
/**
 * Interfaz para el modelo o tabla 'Flujocajafact'.
 *
 * @author Daniel Tamayo <it@virtualsoft.tech>
 * @category    No
 * @package     No
 * @version     1.0
 * @since       No definida
 */
interface FlujocajafactDAO{

	/**
	 * Obtener el registro condicionado por la 
	 * llave primaria que se pasa como parámetro
	 *
	 * @param String $id llave primaria
	 */
	public function load($id);

	/**
	 * Obtener todos los registros de la base datos
	 */
	public function queryAll();
	
	/**
	 * Obtener todos los registros
	 * ordenadas por el nombre de la columna 
	 * que se pasa como parámetro
	 *
	 * @param String $orderColumn nombre de la columna
	 */
	public function queryAllOrderBy($orderColumn);
	
	/**
 	 * Eliminar todos los registros condicionados
 	 * por la llave primaria
 	 *
 	 * @param String $flujocaja_id llave primaria
 	 */
	public function delete($flujocaja_id);
	
	/**
 	 * Insertar un registro en la base de datos
 	 *
 	 * @param String flujoCaja flujoCaja
 	 */
	public function insert($flujocajafact);
	
	/**
 	 * Editar un registro en la base de datos
 	 *
 	 * @param Object flujoCaja flujoCaja
 	 */
	public function update($flujocajafact);	

	/**
	 * Eliminar todos los registros de la base de datos
	 */
	public function clean();







	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna FechaCrea sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value FechaCrea requerido
 	 */	
	public function queryByFechaCrea($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrEntradaEfectivo sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaEfectivo requerido
 	 */	
	public function queryByVlrEntradaEfectivo($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrEntradaBono sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaBono requerido
 	 */	
	public function queryByVlrEntradaBono($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrEntradaRecarga sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaRecarga requerido
 	 */	
	public function queryByVlrEntradaRecarga($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrEntradaTraslado sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaTraslado requerido
 	 */	
	public function queryByVlrEntradaTraslado($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrSalidaEfectivo sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaEfectivo requerido
 	 */	
	public function queryByVlrSalidaEfectivo($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrSalidaNotaret sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaNotaret requerido
 	 */	
	public function queryByVlrSalidaNotaret($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna VlrSalidaTraslado sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaTraslado requerido
 	 */	
	public function queryByVlrSalidaTraslado($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna CantTickets sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value CantTickets requerido
 	 */	
	public function queryByCantTickets($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna Mandante sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Mandante requerido
 	 */	
	public function queryByMandante($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna PuntoventaId sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value PuntoventaId requerido
 	 */	
	public function queryByPuntoventaId($value);







	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna FechaCrea sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value FechaCrea requerido
 	 */	
	public function deleteByFechaCrea($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrEntradaEfectivo sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaEfectivo requerido
 	 */	
	public function deleteByVlrEntradaEfectivo($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrEntradaBono sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaBono requerido
 	 */	
	public function deleteByVlrEntradaBono($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrEntradaRecarga sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaRecarga requerido
 	 */	
	public function deleteByVlrEntradaRecarga($value);
	
	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrEntradaTraslado sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrEntradaTraslado requerido
 	 */		
	public function deleteByVlrEntradaTraslado($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrSalidaEfectivo sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaEfectivo requerido
 	 */	
	public function deleteByVlrSalidaEfectivo($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrSalidaNotaret sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaNotaret requerido
 	 */	
	public function deleteByVlrSalidaNotaret($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna VlrSalidaTraslado sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value VlrSalidaTraslado requerido
 	 */	
	public function deleteByVlrSalidaTraslado($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna CantTickets sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value CantTickets requerido
 	 */	
	public function deleteByCantTickets($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna Mandante sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Mandante requerido
 	 */	
	public function deleteByMandante($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna PuntoventaId sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value PuntoventaId requerido
 	 */	
	public function deleteByPuntoventaId($value);


}
?>