<?php namespace Backend\dao;
/**
 * Interfaz para el modelo o tabla 'Itainment'.
 *
 * @author Daniel Tamayo <it@virtualsoft.tech>
 * @category    No
 * @package     No
 * @version     1.0
 * @since       No definida
 */
interface UsuarioOtrainfoDAO{

	/**
	 * Obtener el registro condicionado por la 
	 * llave primaria que se pasa como parámetro
	 *
	 * @param String $id llave primaria
	 */
	public function load($id);

	/**
	 * Obtener todos los registros de la base datos
	 */
	public function queryAll();
	
	/**
	 * Obtener todos los registros
	 * ordenadas por el nombre de la columna 
	 * que se pasa como parámetro
	 *
	 * @param String $orderColumn nombre de la columna
	 */
	public function queryAllOrderBy($orderColumn);
	
	/**
 	 * Eliminar todos los registros condicionados
 	 * por la llave primaria
 	 *
 	 * @param String $usuario_id llave primaria
 	 */	
	public function delete($usuario_id);
	
	/**
 	 * Insertar un registro en la base de datos
 	 *
 	 * @param String usuarioOtrainfo usuarioOtrainfo
 	 */
	public function insert($usuarioOtrainfo);
	
	/**
 	 * Editar un registro en la base de datos
 	 *
 	 * @param Object usuarioOtrainfo usuarioOtrainfo
 	 */
	public function update($usuarioOtrainfo);	

	/**
	 * Eliminar todos los registros de la base de datos
	 */
	public function clean();





	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna FechaNaci sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value FechaNaci requerido
 	 */	
	public function queryByFechaNacim($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna Direccion sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Direccion requerido
 	 */	
	public function queryByDireccion($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna BancoId sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value BancoId requerido
 	 */	
	public function queryByBancoId($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna TipoCuenta sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value TipoCuenta requerido
 	 */	
	public function queryByTipoCuenta($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna NumCuenta sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value NumCuenta requerido
 	 */	
	public function queryByNumCuenta($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna Mandante sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Mandante requerido
 	 */	
	public function queryByMandante($value);

	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * la columna AnexoDoc sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value AnexoDoc requerido
 	 */	
	public function queryByAnexoDoc($value);





	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna FechaNacim sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value FechaNacim requerido
 	 */	
	public function deleteByFechaNacim($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna Direccion sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Direccion requerido
 	 */	
	public function deleteByDireccion($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna BancoId sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value BancoId requerido
 	 */	
	public function deleteByBancoId($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna TipoCuenta sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value TipoCuenta requerido
 	 */	
	public function deleteByTipoCuenta($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna NumCuenta sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value NumCuenta requerido
 	 */	
	public function deleteByNumCuenta($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna Mandante sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value Mandante requerido
 	 */	
	public function deleteByMandante($value);

	/**
 	 * Eliminar todos los registros donde se encuentre que
 	 * la columna AnexoDoc sea igual al valor pasado como parámetro
 	 *
 	 * @param String $value AnexoDoc requerido
 	 */	
	public function deleteByAnexoDoc($value);


}
?>