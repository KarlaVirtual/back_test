...
class MemoizingInflectorTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testProtectsAgainstCacheOverflow()
    {
        ...
        $this->assertEquals(10, oldCount($cache['snake']));
        $this->assertEquals(10, oldCount($cache['camel']));
        ...
        $cache = $this->readAttribute($inflector, 'cache');
        $this->assertEquals(9, oldCount($cache['snake']));
        $this->assertEquals(9, oldCount($cache['camel']));
    }
}
