...
class OperationTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testContainsApiErrorInformation()
    {
        ...
        $this->assertEquals(1, oldCount($command->getErrorResponses()));
        $arr = $command->toArray();
        $this->assertEquals(1, oldCount($arr['errorResponses']));
        $command->addErrorResponse(400, 'Foo', 'Baz\\Bar');
        $this->assertEquals(2, oldCount($command->getErrorResponses()));
        $command->setErrorResponses(array());
        $this->assertEquals(0, oldCount($command->getErrorResponses()));
    }
