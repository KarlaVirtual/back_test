...
class HistoryPluginTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testAddsRequests()
    {
        ...
        $this->assertEquals(1, oldCount($h));
        $i = $h->getIterator();
        $this->assertEquals(1, oldCount($i));
    ...
    public function testMaintainsLimit()
    {
        ...
        $this->assertEquals(2, oldCount($h));
        $i = 0;
    ...
    public function testClearsHistory()
    {
        $h = new HistoryPlugin();
        $requests = $this->addRequests($h, 5);
        $this->assertEquals(5, oldCount($h));
        $h->clear();
        $this->assertEquals(0, oldCount($h));
    }
    ...
    public function testCanCastToString()
    {
        ...
        $request = $client->get();
        $request->send();
        $this->assertEquals(3, oldCount($h));
        $this->assertEquals(3, oldCount($mock->getReceivedRequests()));
