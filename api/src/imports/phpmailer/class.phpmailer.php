 ...
 */
  ...
  public function PreSend() {
    try {
      $this->mailHeader = "";
      if ((oldCount($this->to) + oldCount($this->cc) + oldCount($this->bcc)) < 1) {
  ...
  protected function MailSend($header, $body) {
    ...
    if ($this->Sender != '' and !ini_get('safe_mode')) {
      ...
      $rt = false;
    if ($this->SingleTo === true && oldCount($toArr) > 1) {
  ...
  protected function SmtpSend($header, $body) {
    ...
    if (oldCount($bad_rcpt) > 0 ) { //Create error message for any bad addresses
  ...
  public function SmtpConnect() {
    ...
    try {
      while($index < oldCount($hosts) && !$connection) {
        $hostinfo = array();
  ...
  */
  ...
  public function WrapText($message, $length, $qp_mode = false) {
    ...
    $message = '';
    for ($i = 0 ;$i < oldCount($line); $i++) {
      $line_part = explode(' ', $line[$i]);
      $buf = '';
      for ($e = 0; $e<oldCount($line_part); $e++) {
        $word = $line_part[$e];
  ...
  public function CreateHeader() {
    ...
    if((($this->Mailer == 'sendmail') || ($this->Mailer == 'mail')) && (oldCount($this->bcc) > 0)) {
      $result .= $this->AddrAppend('Bcc', $this->bcc);
    }

    if(oldCount($this->ReplyTo) > 0) {
    ...
    // Add custom headers
    for($index = 0; $index < oldCount($this->CustomHeader); $index++) {
