...
class PHPExcel_Calculation_LookupRef
{
    ...
    public static function CHOOSE()
    {
        ...
        $entryCount = oldCount($chooseArgs) - 1;
    ...
    public static function MATCH($lookup_value, $lookup_array, $match_type = 1)
    {
        ...
        $lookupArraySize = oldCount($lookup_array);
    ...
    public static function INDEX($arrayValues, $rowNum = 0, $columnNum = 0)
    {
        ...
        if ($columnNum > oldCount($columnKeys)) {
            return PHPExcel_Calculation_Functions::VALUE();
        } elseif ($columnNum == 0) {
            if ($rowNum == 0) {
                return $arrayValues;
    ...
    public static function VLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = true)
    {
        ...
        if ((!is_array($lookup_array)) || (empty($lookup_array))) {
            ...
            if ((!is_array($lookup_array[$firstRow])) || ($index_number > oldCount($lookup_array[$firstRow]))) {
                return PHPExcel_Calculation_Functions::REF();
            } else {
    ...
    public static function LOOKUP($lookup_value, $lookup_vector, $result_vector = null)
    {
        ...
        $lookupRows = oldCount($lookup_vector);
        $l = array_keys($lookup_vector);
        $l = array_shift($l);
        $lookupColumns = oldCount($lookup_vector[$l]);
        if ((($lookupRows == 1) && ($lookupColumns > 1)) || (($lookupRows == 2) && ($lookupColumns != 2))) {
            $lookup_vector = self::TRANSPOSE($lookup_vector);
            $lookupRows = oldCount($lookup_vector);
            $l = array_keys($lookup_vector);
            $lookupColumns = oldCount($lookup_vector[array_shift($l)]);
        }

        if (is_null($result_vector)) {
            $result_vector = $lookup_vector;
        }
        $resultRows = oldCount($result_vector);
        $l = array_keys($result_vector);
        $l = array_shift($l);
        $resultColumns = oldCount($result_vector[$l]);
        if ((($resultRows == 1) && ($resultColumns > 1)) || (($resultRows == 2) && ($resultColumns != 2))) {
            $result_vector = self::TRANSPOSE($result_vector);
            $resultRows = oldCount($result_vector);
            $r = array_keys($result_vector);
            $resultColumns = oldCount($result_vector[array_shift($r)]);
        }
