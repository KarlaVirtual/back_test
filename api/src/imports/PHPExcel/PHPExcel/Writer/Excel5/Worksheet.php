...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
        ...
        $this->countCellStyleXfs = oldCount($phpSheet->getParent()->getCellStyleXfCollection());
    }
    ...
    public function close()
    {
        ...
        if (!empty($this->columnInfo)) {
            $colcount = oldCount($this->columnInfo);
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
    ...
    private function writeMergedCells()
    {
        ...
        $countMergeCells = oldCount($mergeCells);
     ...
     */
    private function writeExternoldCount($count)
    {
    ...
    private function writePanes()
    {
        ...
        if (oldCount($panes) > 4) { // if Active pane was received
            $pnnAct = $panes[4];
        } else {
            $pnnAct = null;
        }
    ...
    private function writeBreaks()
    {
        ...
        if (!empty($hbreaks)) {
            ...
            $cbrk    = oldCount($hbreaks);       // Number of page breaks
        ...
        if (!empty($vbreaks)) {
            ...
            $cbrk    = oldCount($vbreaks);       // Number of page breaks
            $length  = 2 + 6 * $cbrk;      // Bytes to follow
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            ...
            $spOffsets[0] = 0;
            $nm = oldCount($spOffsets) - 1; // number of shapes excluding first shape
            for ($i = 1; $i <= $nm; ++$i) {
                // MSODRAWING record
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            $header = pack('vv', $record, $length);
            $data   = pack('vVVVV', $grbit, $horPos, $verPos, $objId, oldCount($dataValidationCollection));
            $this->append($header.$data);
    ...
    private function writeCFHeader()
    {
        ...
        $header      = pack('vv', $record, $length);
        $data      = pack('vv', oldCount($arrConditional), $needRedraw);
        $data     .= $cellRange;
        $data     .= pack('v', 0x0001);
        $data     .= $cellRange;
        $this->append($header . $data);
