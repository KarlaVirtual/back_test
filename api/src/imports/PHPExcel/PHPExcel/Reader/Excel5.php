                        ...
                        foreach ($ranges as $range) {
                            ...
                            if (oldCount($explodes) == 2) {
                                if ($docSheet = $this->phpExcel->getSheetByName($explodes[0])) {
                                    ...
                                    if (oldCount($coordinateStrings) == 2) {
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($isCellStyleXf) {
                ...
                $this->mapCellXfIndex[$this->xfIndex] = oldCount($this->phpExcel->getCellXfCollection()) - 1;
            }
    ...
    private function readLabelSst()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                $charPos = 0;
                $sstCount = oldCount($this->sst[$index]['fmtRuns']);
