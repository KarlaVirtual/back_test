...
class PHPExcel_Chart_Renderer_jpgraph
{
    ...
    private function renderPlotLine($groupID, $filled = false, $combination = false, $dimensions = '2d')
    {
        ...
        $labelCount = oldCount($this->chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotValuesByIndex(0)->getPointCount());
        if ($labelCount > 0) {
    ...
    private function renderPlotScatter($groupID, $bubble)
    {
        ...
        for ($i = 0; $i < $seriesCount; ++$i) {
            ...
            if ($scatterStyle == 'lineMarker') {
                ...
                list($splineDataY, $splineDataX) = $spline->Get(oldCount($dataValuesX) * self::$width / 20);
    ...
    private function renderPlotStock($groupID)
    {
        ...
        for ($j = 0; $j < oldCount($dataValues[0]); ++$j) {
            for ($i = 0; $i < $seriesCount; ++$i) {
    ...
    private function renderPieChart($groupCount, $dimensions = '2d', $doughnut = false, $multiplePlots = false)
    {
        ...
        for ($groupID = 0; $groupID < $iLimit; ++$groupID) {
            ...
            for ($j = 0; $j < $jLimit; ++$j) {
                ...
                $seriesPlot->SetColor(self::$colourSet[self::$plotColour++]);
                if (oldCount($datasetLabels) > 0) {
                    $seriesPlot->SetLabels(array_fill(0, oldCount($datasetLabels), ''));
                }
                if ($dimensions != '3d') {
    ...
    public function render($outputDestination)
    {
        ...
        if ($groupCount == 1) {
            ...
            if (oldCount($chartTypes) == 1) {
                $chartType = array_pop($chartTypes);
            } elseif (oldCount($chartTypes) == 0) {
