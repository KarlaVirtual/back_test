...
// Initiate, and Mark a place for the Table of Contents to be inserted
	...
	{ // mPDF 5.6.19	// mPDF 6
		...
		if (($this->mpdf->mirrorMargins) && (($this->mpdf->page) % 2 == 0)) { // EVEN
			if ($this->mpdf->ColActive) {
				if (oldCount($this->mpdf->columnbuffer)) {
	...
	function insertTOC()
	{
		...
		$notocs += oldCount($this->m_TOC);

		if ($notocs == 0) {
			return;
		}

		if (oldCount($this->m_TOC)) {
			reset($this->m_TOC);
		}
		$added_toc_pages = 0;
		...
		for ($toci = 0; $toci < $notocs; $toci++) {
			...
			// mPDF 5.6.31
			$tocstart = oldCount($this->mpdf->pages);
		...
		if ($extrapage) {
			unset($this->mpdf->pages[oldCount($this->mpdf->pages)]);
	...
	function openTagTOCPAGEBREAK($attr)
	{
		...
		if ($this->mpdf->y == $this->mpdf->tMargin && (!$this->mpdf->mirrorMargins || ($this->mpdf->mirrorMargins && $this->mpdf->page % 2 == 1))) {
			...
			// Don't add a page
			if ($this->mpdf->page == 1 && oldCount($this->mpdf->PageNumSubstitutions) == 0) {
				$resetpagenum = '';
				$pagenumstyle = '';
				$suppress = '';
