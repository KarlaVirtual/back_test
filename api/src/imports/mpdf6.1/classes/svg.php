...
class SVG
{
	...
	{
		$n = oldCount($this->mpdf_ref->gradients) + 1;
		...
		if (isset($gradient_info['transform'])) {
			...
			if (oldCount($m[0])) {
				for ($i = 0; $i < oldCount($m[0]); $i++) {
					...
					} else if ($c == 'translate' && oldCount($vv)) {
						$tm[4] = $vv[0];
						if (oldCount($vv) == 2) {
							$t_y = -$vv[1];
						} else {
		...
		for ($i = 0; $i < (oldCount($gradient_info['color'])); $i++) {
		...
		$ns = oldCount($gradient_info['color']);
		...
		if ($gradient_info['type'] == 'linear') {
			...
			// Gradient STOPs
			$stops = oldCount($gradient_info['color']);
			if ($stops < 2) {
				return '';
			}

			$range = $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'] - $gradient_info['color'][0]['offset'];
			...
			$grx2 = $x1 + ($x2 - $x1) * $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'];
			$gry2 = $y1 + ($y2 - $y1) * $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'];
			...
			$grx2 = $x1 + ($x0 - $x1) * $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'];
			$gry2 = $y1 + ($y0 - $y1) * $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'];
			$grir = $r * $gradient_info['color'][0]['offset'];
			$grr = $r * $gradient_info['color'][oldCount($gradient_info['color']) - 1]['offset'];
...
// If rX = 0 or rY = 0 then this arc is treated as a straight line
	...
	function svgPolyline($arguments, $ispolyline = true)
	{
		...
		for ($i = 2; $i < oldCount($arguments); $i += 2) {
			if ($ispolyline) {
				$tmp_x = $arguments[$i];
				$tmp_y = - $arguments[($i + 1)];
			} else {
	...
	function svgText()
	{
		// $tmp = oldCount($this->txt_style)-1;
		$current_style = $this->txt_style[oldCount($this->txt_style) - 1]; // mPDF 5.7.4
		$style = '';
		$op = '';
		$render = -1;
			...
			if (isset($this->svg_font[$current_style['font-family']])) {
				...
				while ($i < $nb) {
					...
					foreach ($commands as $cmd) {
						if (oldCount($cmd) == 3 || (isset($cmd[2]) && $cmd[2] == '')) {
		...
		$prev_style = $this->txt_style[oldCount($this->txt_style) - 1];
		$style = '';
			...
			function xml_svg2pdf_start($parser, $name, $attribs)
			{
				...
				switch (strtolower($name)) {
					...
					case 'path':
						...
						foreach ($commands as $c) {
							if ((isset($c) && oldCount($c) == 3) || (isset($c[2]) && $c[2] == '')) {
					...
					case 'text':
						...
						$svg_class->textanchor = $svg_class->txt_style[oldCount($svg_class->txt_style) - 1]['text-anchor']; // mPDF 5.7.4
					...
					case 'tspan':
						...
						$tmp = oldCount($svg_class->svg_style) - 1;
						...
						if (isset($attribs['x']) || isset($attribs['y'])) {
							...
							$svg_class->textanchor = $svg_class->txt_style[oldCount($svg_class->txt_style) - 1]['text-anchor'];
			...
			if ($i % 2 == 0) {
				...
				for ($sch = 0; $sch <= $subchunk; $sch++) {
					if (isset($chardata[$sch])) {
						$s = '';
						for ($j = 0; $j < oldCount($chardata[$sch]); $j++) {
