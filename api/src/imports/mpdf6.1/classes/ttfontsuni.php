	...
	function extractInfo($debug = false, $BMPonly = false, $useOTL = 0)
	{
		...
		if ($kern_offset && $version == 0 && $format == 0) {
			...
			for ($i = 0; $i < $nPairs; $i++) {
				...
				$val = $this->read_short();
				if (oldCount($glyphToChar[$left]) == 1 && oldCount($glyphToChar[$right]) == 1) {
					if ($left != 32 && $right != 32) {
		...
		if (isset($this->tables["GDEF"])) {
			...
			if ($MarkAttachClassDef_offset) {
				...
				foreach ($MarkAttachmentTypes AS $class => $glyphs) {
					...
					if (is_array($Marks) && oldCount($Marks)) {
				...
				for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
					...
					// LookupType 2: Multiple Substitution Subtable 1 => n
						...
						for ($g = 0; $g < oldCount($glyphs); $g++) {
							...
							if (!isset($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID']) || oldCount($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID']) == 0) {
								continue;
						...
						foreach ($volt AS $v) {
							...
							if (strpos('isol fina init medi fin2 fin3 med2', $v['tag']) !== false) {
								...
								if ($v['tag'] == 'isol')
									...
									$kk = 6;
								$rtl[$key][$kk] = $sub;
								if (isset($v['prel']) && oldCount($v['prel']))
									$rtl[$key]['prel'][$kk] = $v['prel'];
								if (isset($v['postl']) && oldCount($v['postl']))
...
//if (oldCount($loclsubs)) { print_r($loclsubs); exit; }
						...
						foreach ($volt AS $v) {
							...
							if (strpos('rphf half pref blwf pstf', $v['tag']) !== false) {
								if (isset($v['context']) && $v['context'] && $v['nBacktrack'] == 0 && $v['nLookahead'] == 0) {
									foreach ($v['rules'] AS $vs) {
										if (oldCount($vs['match']) == 2 && oldCount($vs['replace']) == 1) {
											$sub = $vs['replace'][0];
...
//print_r($rtlpua); exit;
					...
					//=====================================================================================
					//=====================================================================================
					if (oldCount($rtl) || oldCount($rphf) || oldCount($half) || oldCount($pref) || oldCount($blwf) || oldCount($pstf) || $finals) {
	...
	function _getGSUBarray(&$Lookup, &$lul, $scripttag)
	{
		...
		foreach ($lul AS $i => $tag) {
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				// LookupType 1: Single Substitution Subtable
				if ($Lookup[$i]['Type'] == 1) {
					for ($s = 0; $s < oldCount($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
				...
				// LookupType 4: Ligature Substitution Subtable
					...
					for ($s = 0; $s < oldCount($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
						...
						$subs = $this->_makeGSUBinputReplacement(oldCount($inputGlyphs), $substitute, $ignore, 0, oldCount($inputGlyphs), 0);
				...
				// LookupType 5: Chaining Contextual Substitution Subtable
						...
						foreach ($Lookup[$i]['Subtable'][$c]['SubClassSet'] AS $inputClass => $cscs) {
							for ($cscrule = 0; $cscrule < $cscs['SubClassRuleCnt']; $cscrule++) {
								...
								for ($b = 0; $b < $rule['SubstCount']; $b++) {
									...
									// $Lookup[$lup] = secondary Lookup
									for ($lus = 0; $lus < $Lookup[$lup]['SubtableCount']; $lus++) {
										if (isset($Lookup[$lup]['Subtable'][$lus]['subs']) && oldCount($Lookup[$lup]['Subtable'][$lus]['subs'])) {
											foreach ($Lookup[$lup]['Subtable'][$lus]['subs'] AS $luss) {
					...
					// Format 3: Coverage-based Context Glyph Substitution  p259
						...
						$nBsubs = 2 * oldCount($backtrackGlyphs);
						$nIsubs = (2 * $nInput) - 1;
						$contextInputMatch = $this->_makeGSUBcontextInputMatch($inputGlyphs, $ignore, array(), 0);
						$subRule = array('context' => 1, 'tag' => $tag, 'matchback' => $backtrackMatch, 'match' => ($contextInputMatch . $lookaheadMatch), 'nBacktrack' => oldCount($backtrackGlyphs), 'nInput' => $nInput, 'nLookahead' => oldCount($lookaheadGlyphs), 'rules' => array(),);
...
//print_r($Lookup[$i]);
//print_r($volt[(oldCount($volt)-1)]); exit;
				}
...
//print_r($Lookup); exit;
	...
	function _makeGSUBcontextInputMatch($inputGlyphs, $ignore, $lookupGlyphs, $seqIndex)
	{
		...
		$mLen = oldCount($lookupGlyphs); // nGlyphs in the secondary Lookup match
		$nInput = oldCount($inputGlyphs); // nGlyphs in the Primary Input sequence
		$str = "";
		for ($i = 0; $i < $nInput; $i++) {
			if ($i > 0) {
				$str .= $ignore . " ";
	...
	function _makeGSUBinputMatch($inputGlyphs, $ignore)
	{
		...
		$str = "";
		for ($i = 1; $i <= oldCount($inputGlyphs); $i++) {
			if ($i > 1) {
				$str .= $ignore . " ";
			}
			$str .= "(" . $inputGlyphs[($i - 1)] . ")";
	...
	function _makeGSUBbacktrackMatch($backtrackGlyphs, $ignore)
	{
		...
		$str = "";
		for ($i = (oldCount($backtrackGlyphs) - 1); $i >= 0; $i--) {
	...
	function _makeGSUBlookaheadMatch($lookaheadGlyphs, $ignore)
	{
		...
		// each item being e.g. E0AD|E0AF|F1FD
		$str = "";
		for ($i = 0; $i < oldCount($lookaheadGlyphs); $i++) {
...
?>';
	...
	{ // mPDF 5.7.1
		...
		$numGlyphs = $numberOfHMetrics = oldCount($subsetglyphs);
		...
		$segCount = oldCount($range) + 1; // + 1 Last segment has missing character 0xFFFF
		$searchRange = 1;
		$entrySelector = 0;
		...
		foreach ($range AS $start => $subrange) {
			$endCode = $start + (oldCount($subrange) - 1);
	...
	{ // mPDF 5.7.1
		...
		foreach ($subset AS $code) {
			...
			if (!isset($glyphSet[$originalGlyphIdx])) {
				$glyphSet[$originalGlyphIdx] = oldCount($glyphMap);
			...
			if ($numberOfContours < 0) {
				...
				while ($flags & GF_MORE) {
					...
					if (!isset($glyphSet[$glyphIdx])) {
						$glyphSet[$glyphIdx] = oldCount($glyphMap);
						$glyphMap[] = $glyphIdx;
					}
					if ($flags & GF_WORDS)
						$this->skip(4);
		...
		if (isset($this->tables['OS/2'])) {
			...
			$os2 = $this->_set_ushort($os2, 66, oldCount($subset)); // LastCharIndex
	...
	function getGlyphData($originalGlyphIdx, &$maxdepth, &$depth, &$points, &$contours)
	{
		...
		if (oldCount($this->glyphdata[$originalGlyphIdx]['compGlyphs'])) {
	...
	function getGlyphs($originalGlyphIdx, &$start, &$glyphSet, &$subsetglyphs)
	{
		...
		if ($numberOfContours < 0) {
			...
			while ($flags & GF_MORE) {
				...
				$glyphIdx = $this->read_ushort();
				if (!isset($glyphSet[$glyphIdx])) {
					$glyphSet[$glyphIdx] = oldCount($subsetglyphs); // old glyphID to new glyphID
	...
	{
		$stm = '';
		$numTables = oldCount($this->otables);
		$searchRange = 1;
		$entrySelector = 0;
	...
	{ // mPDF 5.7.1
		...
		if ($useOTL) {
			...
			foreach ($range AS $start => $subrange) {
				$idDelta = -($start - $subrange[0]);
				//$n += oldCount($subrange);	// ?? Line not required
				$cmap[] = $idDelta; // idDelta(s)
			}
			$cmap[] = 1; // idDelta of last Segment
			// idRangeOffset(s)
