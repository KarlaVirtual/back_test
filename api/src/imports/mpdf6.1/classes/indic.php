...
class INDIC
{
	...
	public static function insert_dotted_circles(&$info, $dottedcircle)
	{
		...
		while ($idx < oldCount($info)) {
			...
			if ($last_syllable != $syllable && $syllable_type == self::BROKEN_CLUSTER) {
				...
				while ($idx < oldCount($info) && $last_syllable == $info[$idx]['syllable'] && $info[$idx]['indic_category'] == self::OT_Repha)
					$idx++;
		...
		// I am not sue how this code below got in here, since $idx should now be > oldCount($info) and thus invalid.
	...
	public static function initial_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
	{
		...
		if ($scriptblock != UCDN::SCRIPT_KHMER) {
			 ...
			 * 	and has more than one consonant, Ra is excluded from candidates for
			 * 	base consonants. */
			if (oldCount($GSUBdata['rphf']) /* ?? $indic_plan->mask_array[RPHF] */ && $start + 3 <= $end &&
				(
				...
				//if ($indic_plan->rphf->would_substitute ($glyphs, oldCount($glyphs), true, face)) {
		...
		if ($scriptblock != UCDN::SCRIPT_KHMER) {
			if (oldCount($GSUBdata['pref']) && $base + 2 < $end) {
	...
	public static function final_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
	{
		...
		if (oldCount($GSUBdata['pref']) && $base + 1 < $end) { /* Otherwise there can't be any pre-base reordering Ra. */
			for ($i = $base + 1; $i < $end; $i++) {
