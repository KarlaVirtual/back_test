	...
	function applyOTL($str, $useOTL)
	{
		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			...
			if (!$GSUBscriptTag && !$GSUBlangsys && !$GPOSscriptTag && !$GPOSlangsys) {
				// Remove ZWJ and ZWNJ
				for ($i = 0; $i < oldCount($this->OTLdata); $i++) {
...
////////////////////////////////////////////////////////////////
			if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
				...
				if ($this->shaper == 'A') {
					...
					//-----------------------------------------------------------------------------------
					for ($ptr = 1; $ptr < oldCount($this->OTLdata); $ptr++) {
				...
				//===================================
					...
					//-----------------------------------------------------------------------------------
					for ($ptr = 0; $ptr < oldCount($this->OTLdata); $ptr++) {
						$char = $this->OTLdata[$ptr]['uni'];
						$sub = INDIC::decompose_indic($char);
						if ($sub) {
							$newinfo = array();
							for ($i = 0; $i < oldCount($sub); $i++) {
								$newinfo[$i] = array();
				...
				//==============================
					...
					if ($scriptblock == UCDN::SCRIPT_TIBETAN) {
						...
						for ($ptr = 0; $ptr < oldCount($this->OTLdata) - 1; $ptr++) {
			...
			if (($this->shaper == "K" || $this->shaper == "T" || $this->shaper == "L") || $scriptblock == UCDN::SCRIPT_TIBETAN) {
				...
				for ($i = oldCount($this->OTLdata) - 1; $i > 0; $i--) {
...
////////////////////////////////////////////////////////////////
			...
			if (($useOTL & 0xFF) && $GPOSscriptTag && $GPOSlangsys && $GPOSFeatures) {
				...
				//==============================
				if (oldCount($this->Entry) || oldCount($this->Exit)) {
					// RTL
					$incurs = false;
					for ($i = (oldCount($this->OTLdata) - 1); $i >= 0; $i--) {
		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			for ($i = 0; $i < oldCount($this->schOTLdata[$sch]); $i++) {
	...
	function _applyGSUBrules($usetags, $scriptTag, $langsys)
	{
		...
		foreach ($LookupList AS $lu => $tag) {
			...
			while ($ptr < (oldCount($this->OTLdata))) { // whilst there is another glyph ..0064
				...
				for ($s = 0; $s < $SubClassSetCnt; $s++) { // $SubClassSet is ordered by input class-may be NULL
					...
					if ($SubClassSetOffset[$s] > 0 && isset($InputClasses[$s][$currGID])) {
						...
						for ($b = 0; $b < $SubClassRuleCnt; $b++) {  // EACH RULE
							...
							$class0excl = array();
							for ($gc = 1; $gc <= oldCount($InputClasses); $gc++) {
				...
				for ($s = 0; $s < $ChainSubClassSetCnt; $s++) { // $ChainSubClassSet is ordered by input class-may be NULL
					...
					if ($ChainSubClassSetOffset[$s] > 0 && isset($InputClasses[$s][$currGID])) {
						...
						for ($b = 0; $b < $ChainSubClassRuleCnt; $b++) {  // EACH RULE
							...
							// Class 0 contains all the glyphs NOT in the other classes
							$bclass0excl = array();
							for ($gc = 1; $gc <= oldCount($BacktrackClasses); $gc++) {
							...
							// Class 0 contains all the glyphs NOT in the other classes
							$lclass0excl = array();
							for ($gc = 1; $gc <= oldCount($LookaheadClasses); $gc++) {
	...
	function _updateLigatureMarks($pos, $n)
	{
		if ($n > 0) {
			...
			for ($p = oldCount($this->OTLdata) - 1; $p >= 0; $p--) {
			...
			for ($p = ($pos + 1); $p < oldCount($this->OTLdata); $p++) {
				if (isset($this->assocLigs[$p])) {
					$tmp = $this->assocLigs[$p];
					unset($this->assocLigs[$p]);
			...
			for ($p = 0; $p < oldCount($this->OTLdata); $p++) {
				if (isset($this->assocMarks[$p])) {
	...
	function GSUBsubstitute($pos, $substitute, $Type, $GlyphPos = NULL)
	{
		...
		// LookupType 2: Multiple Substitution Subtable : 1 to n
			...
			for ($i = 0; $i < oldCount($substitute); $i++) {
				$uni = $substitute[$i];
				$newOTLdata[$i] = array();
			...
			if ($this->shaper == 'K' || $this->shaper == 'T' || $this->shaper == 'L') {
				if ($this->OTLdata[$pos]['wordend']) {
					$newOTLdata[oldCount($substitute) - 1]['wordend'] = true;
				}
			}

			array_splice($this->OTLdata, $pos, 1, $newOTLdata); // Replace 1 with n
			// Update position of Ligatures and associated Marks
			// oldCount($substitute)-1  is the number of glyphs added
			$nadd = oldCount($substitute) - 1;
			$this->_updateLigatureMarks($pos, $nadd);
			return oldCount($substitute);
		}
		// LookupType 4: Ligature Substitution Subtable : n to 1
		else if ($Type == 4) {
			...
			if ($contains_marks && !$contains_nonmarks) {
				...
				$lastPos = $GlyphPos[(oldCount($GlyphPos) - 1)];
			} else {
				/*
				...
				for ($i = 0; $i < oldCount($GlyphPos); $i++) {
					...
					$ic = 1;
					while ((($i == oldCount($GlyphPos) - 1) || (isset($GlyphPos[$i + 1]) && ($GlyphPos[$i] + $ic) < $GlyphPos[$i + 1])) && isset($this->OTLdata[($GlyphPos[$i] + $ic)]) && strpos($this->GlyphClassMarks, $this->OTLdata[($GlyphPos[$i] + $ic)]['hex']) !== false) {
						$newComp = $currComp;
				...
				$lastPos = $GlyphPos[(oldCount($GlyphPos) - 1)] + $ic - 1;
			...
			$ka = 0;
			if (isset($this->OTLdata[$GlyphPos[(oldCount($GlyphPos) - 1)]]['GPOSinfo']['kashida'])) {
				$ka = $this->OTLdata[$GlyphPos[(oldCount($GlyphPos) - 1)]]['GPOSinfo']['kashida'];
			}
			...
			for ($i = oldCount($GlyphPos) - 1; $i > 0; $i--) {
				$gpos = $GlyphPos[$i];
			...
			// oldCount($GlyphPos)-1  is the number of glyphs removed from string
			for ($p = ($GlyphPos[0] + 1); $p < (oldCount($this->OTLdata) + oldCount($GlyphPos) - 1); $p++) {
...
// Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
	function SEAlineBreaking()
	{
		...
		$rollover = array();
		$ptr = 0;
		while ($ptr < oldCount($this->OTLdata) - 3) {
			if (oldCount($rollover)) {
				$matches = $rollover;
				$rollover = array();
			} else {
				$matches = $this->checkwordmatch($dict, $ptr);
			}
			if (oldCount($matches) == 1) {
				$matchpos = $matches[0];
				...
				$secondmatch = false;
				for ($m = oldCount($matches) - 1; $m >= 0; $m--) {
					//for ($m=0;$m<oldCount($matches);$m++) {
					$firstmatch = $matches[$m];
					$matches2 = $this->checkwordmatch($dict, $firstmatch + 1);
					if (oldCount($matches2)) {
						// Set end of word marker in OTLdata at matchpos
				...
				if (!$secondmatch) {
					...
					$this->OTLdata[$matches[oldCount($matches) - 1]]['wordend'] = true;
					$ptr = $matches[oldCount($matches) - 1] + 1;
					// Move past any ASCII characters
					while (isset($this->OTLdata[$ptr]['uni']) && ($this->OTLdata[$ptr]['uni'] >> 8) == 0) {
						$ptr++;
					}
...
//echo "DICT_NODE_TYPE_LINEAR: ".dechex($c).'<br />';
				...
				if ($c == $m) {
					$ptr++;
					if ($ptr > oldCount($this->OTLdata) - 1) {
...
// the last of any Marks which immediately follow the current glyph
	...
	function checkContextMatch($Input, $Backtrack, $Lookahead, $ignore, $ptr)
	{
		...
		// BACKTRACK
		$checkpos = $ptr;
		for ($i = 0; $i < oldCount($Backtrack); $i++) {
			$checkpos--;
			while (isset($this->OTLdata[$checkpos]) && strpos($ignore, $this->OTLdata[$checkpos]['hex']) !== false) {
				$checkpos--;
			}
		...
		// LOOKAHEAD
		for ($i = 0; $i < oldCount($Lookahead); $i++) {
			$checkpos++;
			while (isset($this->OTLdata[$checkpos]) && strpos($ignore, $this->OTLdata[$checkpos]['hex']) !== false) {
				$checkpos++;
			}
	...
	function checkContextMatchMultiple($Input, $Backtrack, $Lookahead, $ignore, $ptr, $class0excl = '', $bclass0excl = '', $lclass0excl = '')
	{
		...
		$matched = array(0 => $ptr);
		$checkpos = $ptr;
		for ($i = 1; $i < oldCount($Input); $i++) { // Start at 1 - already matched the first InputGlyph
			$checkpos++;
			while (isset($this->OTLdata[$checkpos]) && strpos($ignore, $this->OTLdata[$checkpos]['hex']) !== false) {
				$checkpos++;
			}
...
////////////////////////////////////////////////////////////////
	...
	function _bidiSort($ta, $str = '', $dir, &$chunkOTLdata, $useGPOS)
	{
		...
		$maxlevel = 0;
		$numchars = oldCount($chunkOTLdata['char_data']);
		...
		for ($i = 0; $i < $numchars; ++$i) {
			if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) { // RLE
				...
				if (oldCount($remember)) {
					$last = oldCount($remember) - 1;
...
// **********************************************************************************************
	...
	function _bidiPrepare(&$para, $dir)
	{
			...
			for ($i = 0; $i < $numchars; ++$i) {
				if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) { // RLE
					...
					if ($chunkOTLdata['char_data'][$i]['uni'] == 8296) { // FSI
						...
						$nc2 = $nc;
						$i2 = $i;
						while (!($nc2 == ($numchunks - 1) && $i2 == ((oldCount($para[$nc2][18]['char_data'])) - 1))) {  // while not at end of last chunk
							$i2++;
							if ($i2 >= oldCount($para[$nc2][18]['char_data'])) {
								$nc2++;
								$i2 = 0;
							}
							if ($lvl > 0) {
		...
		for ($nc = ($numchunks - 1); $nc >= 0; $nc--) {
			if (oldCount($para[$nc][18]['char_data']) == 0) {
...
// Reorder, once divided into lines
	...
	function _bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir)
	{
		...
		$numchunks = oldCount($content);
		$maxlevel = 0;
		for ($nc = 0; $nc < $numchunks; $nc++) {
			$numchars = oldCount($cOTLdata[$nc]['char_data']);
			for ($i = 0; $i < $numchars; ++$i) {
...
// Remove one or more occurrences of $char (single character) from $txt and adjust OTLdata
	...
	function trimOTLdata(&$cOTLdata, $Left = true, $Right = true)
	{
		...
		$len = oldCount($cOTLdata['char_data']);
		$nLeft = 0;
		$nRight = 0;
		for ($i = 0; $i < $len; $i++) {
