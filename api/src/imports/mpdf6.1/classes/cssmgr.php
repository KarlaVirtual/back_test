...
//print_r($this->cascadeCSS); exit;
	...
	function readInlineCSS($html)
	{
		...
		$classproperties = array();
		for ($i = 0; $i < oldCount($properties); $i++) {
			// Ignores -webkit-gradient so doesn't override -moz-
	...
	{
		if (!is_array($prop) || (oldCount($prop) == 0))
			return array();
		$newprop = array();
		foreach ($prop AS $k => $v) {
			...
			if ($k == 'FONT') {
				...
				if (oldCount($bits) > 1) {
					$k = 'FONT-FAMILY';
					$v = $bits[(oldCount($bits) - 1)];
					$fs = $bits[(oldCount($bits) - 2)];
		...
		if ($tag == 'TR' && isset($p) && $p) {
			foreach ($p AS $k => $val) {
				if (preg_match('/' . $tag . '>>SELECTORNTHCHILD>>(.*)/', $k, $m)) {
					...
					if ($tag == 'TR') {
						$row = $this->mpdf->row;
						$thnr = (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_thead']) ? oldCount($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_thead']) : 0);
						$tfnr = (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) ? oldCount($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) : 0);
...
// mPDF 5.7.4   nth-child
	function _nthchild($f, $c)
	{
		...
		if ($f[0] == 'ODD') {
			...
			$a = 2;
			$b = 0;
		} else if (oldCount($f) == 2) {
			$a = 0;
			$b = $f[1] + 0;
		}  // e.g. (+6)
		else if (oldCount($f) == 3) {  // e.g. (2N)
			if ($f[2] == '') {
				$a = 1;
			} else if ($f[2] == '-') {
				$a = -1;
