			...
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				// LookupType 3: Alternate Forms
					...
					for ($s = 0; $s < oldCount($Lookup[$i]['Subtable'][$c]['subs']); $s++) {
						...
						$html .= '&nbsp; <span class="unicode">' . $this->formatUni($substitute) . '</span> ';
						if (oldCount($Lookup[$i]['Subtable'][$c]['subs'][$s]['substitute']) > 1) {
							for ($alt = 1; $alt < oldCount($Lookup[$i]['Subtable'][$c]['subs'][$s]['substitute']); $alt++) {
					...
					if ($SubstFormat == 1) {
						...
						for ($s = 0; $s < $Lookup[$i]['Subtable'][$c]['SubRuleSetCount']; $s++) {
							...
							foreach ($Lookup[$i]['Subtable'][$c]['SubRuleSet'][$s]['SubRule'] AS $rctr => $rule) {
								...
								for ($ff = 0; $ff < oldCount($inputGlyphs); $ff++) {
								...
								for ($b = 0; $b < $rule['SubstCount']; $b++) {
									...
									if (oldCount($inputGlyphs) > ($seqIndex + 1)) {
										$exL .= '<span class="inputother">';
										for ($ip = $seqIndex + 1; $ip < oldCount($inputGlyphs); $ip++) {
					...
					if ($SubstFormat == 1) {
						...
						for ($s = 0; $s < $Lookup[$i]['Subtable'][$c]['ChainSubRuleSetCount']; $s++) {
							...
							foreach ($Lookup[$i]['Subtable'][$c]['ChainSubRuleSet'][$s]['ChainSubRule'] AS $rctr => $rule) {
								...
								$exampleL = array();
								$html .= '<div class="context">CONTEXT: ';
								for ($ff = oldCount($backtrackGlyphs) - 1; $ff >= 0; $ff--) {
								...
								for ($ff = 0; $ff < oldCount($lookaheadGlyphs); $ff++) {
...
// Takes too long e.g. Calibri font - just list kerning pairs with this:
					...
					//===========
						...
						for ($i = 0; $i < $Class1Count; $i++) {
							for ($j = 0; $j < $Class2Count; $j++) {
								...
								for ($c1 = 0; $c1 < oldCount($Class1[$i]); $c1++) {
									...
									for ($c2 = 0; $c2 < oldCount($Class2[$j]); $c2++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					for ($i = 0; $i < oldCount($MarkGlyphs); $i++) {
					...
					for ($j = 0; $j < oldCount($BaseGlyphs); $j++) {
					...
					for ($j = 0; $j < min(oldCount($BaseGlyphs), 20); $j++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					for ($j = 0; $j < oldCount($LigatureGlyphs); $j++) {
				...
				////////////////////////////////////////////////////////////////////////////////
					...
					$firstMark = '';
					$html .= '<div class="glyphs">Marks: <span class="unchanged">';
					for ($i = 0; $i < oldCount($Mark1Glyphs); $i++) {
					...
					if ($firstMark) {
						...
						for ($j = 0; $j < oldCount($Mark2Glyphs); $j++) {
						...
						for ($j = 0; $j < min(oldCount($Mark2Glyphs), 20); $j++) {
