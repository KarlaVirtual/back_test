...
class grad
{
	...
	function CoonsPatchMesh($x, $y, $w, $h, $patch_array = array(), $x_min = 0, $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
	{
		...
		$n = oldCount($this->mpdf->gradients) + 1;
		...
		for ($i = 0; $i < oldCount($patch_array); $i++) {
			...
			for ($j = 0; $j < oldCount($patch_array[$i]['colors']); $j++) {
	...
	function Gradient($x, $y, $w, $h, $type, $stops = array(), $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
	{
		...
		if (!isset($stops[(oldCount($stops) - 1)]['offset'])) {
			$stops[(oldCount($stops) - 1)]['offset'] = 1;
		}
		...
		if (!$repeat && isset($stops[(oldCount($stops) - 1)]['offset']) && $stops[(oldCount($stops) - 1)]['offset'] < 1) {
			$endstop = $stops[(oldCount($stops) - 1)];
			$endstop['offset'] = 1;
			$stops[] = $endstop;
		}
		if ($stops[0]['offset'] > $stops[(oldCount($stops) - 1)]['offset']) {
			$stops[0]['offset'] = 0;
			$stops[(oldCount($stops) - 1)]['offset'] = 1;
		}
		...
		if ($repeat) {
			$ns = oldCount($this->mpdf->gradients[$n]['stops']);
			$offs = array();
			for ($i = 0; $i < $ns; $i++) {
		...
		if (!is_array($extend) || oldCount($extend) < 1) {
	...
	function parseMozGradient($bg)
	{
		...
		if (preg_match('/linear-gradient\((.*)\)/', $bg, $m)) {
			...
			if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i', $bgr[0]) && !preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0])) {
				$startStops = 1;
			} else if (trim($first[(oldCount($first) - 1)]) === "0") {
				$startStops = 1;
			} else {
			...
			$g['stops'] = array();
			for ($i = $startStops; $i < oldCount($bgr); $i++) {
				$stop = array();
				// parse stops
	...
	function parseBackgroundGradient($bg)
	{
		...
		$g = array();
		if (oldCount($bgr) > 6) {
			if (strtoupper(substr($bgr[0], 0, 1)) == 'L' && oldCount($bgr) == 7) {  // linear
				$g['type'] = 2;
