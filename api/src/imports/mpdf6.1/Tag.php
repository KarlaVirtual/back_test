	...
	{ // mPDF 6
		...
		switch ($tag) {
			...
			case 'BR':
				...
				$inlinepost = '';
				$iBDF = array();
				if (oldCount($this->mpdf->InlineBDF)) {
					...
					if (oldCount($iBDF)) {
						ksort($iBDF);
						for ($i = oldCount($iBDF) - 1; $i >= 0; $i--) {
							$inlinepre .= $this->mpdf->_setBidiCodes('end', $iBDF[$i]);
						}
						for ($i = 0; $i < oldCount($iBDF); $i++) {
				...
				if ($this->mpdf->tableLevel) {
					...
					/* -- END TABLES -- */
					if (oldCount($this->mpdf->textbuffer)) {
						$this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0] = preg_replace('/ $/', '', $this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0]);
						if (!empty($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][18])) {
							$this->mpdf->otl->trimOTLdata($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][18], false, true);
						} // *OTL*
					}
			...
			case 'MAIN':
				...
				if ($this->mpdf->blockjustfinished && !oldCount($this->mpdf->textbuffer) && $this->mpdf->y != $this->mpdf->tMargin && $this->mpdf->collapseBlockMargins) {
...
//		if ($currblk['inner_width'] < $mw) { throw new MpdfException("DIV is too narrow for text to fit!"); }
			...
			case 'JPGRAPH':
				...
				if (!is_array($this->mpdf->graphs[$gid]) || oldCount($this->mpdf->graphs[$gid]) == 0) {
					break;
				}
				$this->mpdf->ignorefollowingspaces = false;
			...
			case 'TABLE': // TABLE-BEGIN
				...
				if (oldCount($this->mpdf->textbuffer) == 0 && $this->mpdf->lastblocklevelchange == 1 && !$this->mpdf->tableLevel && !$this->mpdf->kwt) {
				...
				} else if (!$this->mpdf->tableLevel && oldCount($this->mpdf->textbuffer)) {
					$this->mpdf->printbuffer($this->mpdf->textbuffer, $blockstate);
				}
				...
				if ($this->mpdf->blockjustfinished && !oldCount($this->mpdf->textbuffer) && $this->mpdf->y != $this->mpdf->tMargin && $this->mpdf->collapseBlockMargins && $this->mpdf->tableLevel == 1) {
					$lastbottommargin = $this->mpdf->lastblockbottommargin;
				} else {
					$lastbottommargin = 0;
				}
	...
	{ // mPDF 6
		...
		if ($tag == 'FONT' || $tag == 'SPAN' || $tag == 'CODE' || $tag == 'KBD' || $tag == 'SAMP' || $tag == 'TT' || $tag == 'VAR' || $tag == 'INS' || $tag == 'STRONG' || $tag == 'CITE' || $tag == 'SUB' || $tag == 'SUP' || $tag == 'S' || $tag == 'STRIKE' || $tag == 'DEL' || $tag == 'Q' || $tag == 'EM' || $tag == 'B' || $tag == 'I' || $tag == 'U' | $tag == 'SMALL' || $tag == 'BIG' || $tag == 'ACRONYM' || $tag == 'MARK' || $tag == 'TIME' || $tag == 'PROGRESS' || $tag == 'METER' || $tag == 'BDO' || $tag == 'BDI'
			...
			if ($tag == 'PROGRESS' || $tag == 'METER') {
				...
				if (isset($this->mpdf->InlineProperties[$tag]) && oldCount($this->mpdf->InlineProperties[$tag])) {
					$tmpProps = array_pop($this->mpdf->InlineProperties[$tag]); // mPDF 5.7.4
					$this->mpdf->restoreInlineProperties($tmpProps);
				}
				if (isset($this->mpdf->InlineAnnots[$tag]) && oldCount($this->mpdf->InlineAnnots[$tag])) {  // *ANNOTATIONS*
					$annot = array_pop($this->mpdf->InlineAnnots[$tag]);  // *ANNOTATIONS*
				} // *ANNOTATIONS*
				if (isset($this->mpdf->InlineBDF[$tag]) && oldCount($this->mpdf->InlineBDF[$tag])) {  // mPDF 6
		...
		if ($tag == 'LEGEND') {
			if (oldCount($this->mpdf->textbuffer) && !$this->mpdf->tableLevel) {
				$leg = $this->mpdf->textbuffer[(oldCount($this->mpdf->textbuffer) - 1)];
				unset($this->mpdf->textbuffer[(oldCount($this->mpdf->textbuffer) - 1)]);
		...
		if ($tag == 'P' || $tag == 'DIV' || $tag == 'H1' || $tag == 'H2' || $tag == 'H3' || $tag == 'H4' || $tag == 'H5' || $tag == 'H6' || $tag == 'PRE' || $tag == 'FORM' || $tag == 'ADDRESS' || $tag == 'BLOCKQUOTE' || $tag == 'CENTER' || $tag == 'DT' || $tag == 'DD' || $tag == 'DL' || $tag == 'CAPTION' || $tag == 'FIELDSET' || $tag == 'UL' || $tag == 'OL' || $tag == 'LI' || $tag == 'ARTICLE' || $tag == 'ASIDE' || $tag == 'FIGURE' || $tag == 'FIGCAPTION' || $tag == 'FOOTER' || $tag == 'HEADER' || $tag == 'HGROUP' || $tag == 'MAIN' || $tag == 'NAV' || $tag == 'SECTION' || $tag == 'DETAILS' || $tag == 'SUMMARY'
			...
			if (preg_match('/^H\d/', $tag) && !$this->mpdf->tableLevel && !$this->mpdf->writingToC) {
				if (isset($this->mpdf->h2toc[$tag]) || isset($this->mpdf->h2bookmarks[$tag])) {
					$content = '';
					if (oldCount($this->mpdf->textbuffer) == 1) {
						$content = $this->mpdf->textbuffer[0][0];
					} else {
						for ($i = 0; $i < oldCount($this->mpdf->textbuffer); $i++) {
			...
			if (oldCount($this->mpdf->textbuffer) && $this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1]) {
				if (substr($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0], 0, 3) != "\xbb\xa4\xac") { // not special content
					// Right trim last content and adjust OTLdata
					if (preg_match('/[ ]+$/', $this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0], $m)) {
						$strip = strlen($m[0]);
						$this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0] = substr($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0], 0, (strlen($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][0]) - $strip));
						/* -- OTL -- */
						if (isset($this->mpdf->CurrentFont['useOTL']) && $this->mpdf->CurrentFont['useOTL']) {
							$this->mpdf->otl->trimOTLdata($this->mpdf->textbuffer[oldCount($this->mpdf->textbuffer) - 1][18], false, true); // mPDF 6  ZZZ99K
						}
						/* -- END OTL -- */
					}
				}
			...
			if (oldCount($this->mpdf->textbuffer) == 0 && $this->mpdf->lastblocklevelchange != 0) {
		...
		if (($tag == 'TH' or $tag == 'TD') && $this->mpdf->tableLevel) {
			...
			if (isset($this->mpdf->cell[$this->mpdf->row][$this->mpdf->col]['textbuffer'])) {
				$ntb = oldCount($this->mpdf->cell[$this->mpdf->row][$this->mpdf->col]['textbuffer']);
			} else {
				$ntb = 0;
			}
		...
		if ($tag == 'TABLE') { // TABLE-END (
			...
			if (isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot']) && oldCount($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'])) {
				$tfrows = array();
				...
				for ($i = oldCount($temp); $i < (oldCount($temp) + oldCount($temptf)); $i++) {
					$this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['is_tfoot'][$i] = true;
				}
				// Update nestedpos row references
				if (isset($this->mpdf->table[($this->mpdf->tableLevel + 1)]) && oldCount($this->mpdf->table[($this->mpdf->tableLevel + 1)])) {
					foreach ($this->mpdf->table[($this->mpdf->tableLevel + 1)] AS $nid => $nested) {
						$this->mpdf->table[($this->mpdf->tableLevel + 1)][$nid]['nestedpos'][0] -= oldCount($temptf);
					}
				}
