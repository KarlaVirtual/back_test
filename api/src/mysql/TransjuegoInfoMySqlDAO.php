...
* 
*/
class TransjuegoInfoMySqlDAO implements TransjuegoInfoDAO
{
    ...
    */
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE transjuegoinfo_id = ?';
	...
	{
		$sql = 'SELECT * FROM transjuego_info';
		$sqlQuery = new SqlQuery($sql);
	...
	{
		$sql = 'SELECT * FROM transjuego_info ORDER BY ' . $orderColumn;
		$sqlQuery = new SqlQuery($sql);
		return $this->getList($sqlQuery);
	}
	...
	/**
 	 ...
 	 *
 	 * @param String $transjuegoinfo_id transjuegoinfo_id primaria
 	 *
	 * @return boolean $ resultado de la consulta
     *
 	 */
	public function delete($transjuegoinfo_id)
	{
		$sql = 'DELETE FROM transjuego_info WHERE transjuegoinfo_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($transjuegoinfo_id);
	...
	* @return Array $json resultado de la consulta
    ...
    public function queryTransaccionesCustom($select,$sidx,$sord,$start,$limit,$filters,$searchOn,$grouping)
    {
        ...
        $sql = "SELECT count(*) count FROM transjuego_info   INNER JOIN transaccion_api ON (transjuego_info.transapi_id = transaccion_api.transapi_id)   LEFT OUTER JOIN transaccion_juego ON (transaccion_juego.transjuego_id = transjuego_info.transjuego_id) INNER JOIN usuario_mandante ON (usuario_mandante.usumandante_id = transaccion_api.usuario_id) " . $where;

        $sqlQuery = new SqlQuery($sql);

        $count = $this->execute2($sqlQuery);
        $sql = "SELECT ".$select." FROM transjuego_info INNER JOIN transaccion_api ON (transjuego_info.transapi_id = transaccion_api.transapi_id) LEFT OUTER JOIN transaccion_juego ON (transaccion_juego.transjuego_id = transjuego_info.transjuego_id) INNER JOIN usuario_mandante ON (usuario_mandante.usumandante_id = transaccion_api.usuario_id)   " . $where ." " . " order by " . $sidx . " " . $sord . " LIMIT " . $start . " , " . $limit;
	...
	/**
	 * Realiza una consulta personalizada en la base de datos con múltiples opciones de filtrado, ordenación, paginación y agrupación.
	 *
	 ...
	 * @param string $mainTable Tabla principal de la consulta.
	 * @param array $joins Arreglo de objetos que representan las uniones (joins) a otras tablas.
	 * @param bool $groupingCount Indica si se debe modificar el conteo para consultas con GROUP BY.
	 * @return string JSON con el conteo de registros y los datos resultantes de la consulta.
	 */
    public function querySuperCustom($select,$sidx,$sord,$start,$limit,$filters,$searchOn,$grouping, $mainTable, $joins = [], $groupingCount = false)
    {
        $where = " where 1=1 ";
        ...
        $sql = $groupByCountPrefix ."SELECT count(*) count FROM ". $mainTable . $strJoins . $where . $groupByCountSuffix;
        $sqlQuery = new SqlQuery($sql);
        $count = $this->execute2($sqlQuery);

        $sql = "SELECT ".$select." FROM ". $mainTable . $strJoins . $where ." " . " order by " . $sidx . " " . $sord . " LIMIT " . $start . " , " . $limit;
        $sqlQuery = new SqlQuery($sql);
        $result = $this->execute2($sqlQuery);
	...
	{
		$sql = 'INSERT INTO transjuego_info (transjuego_id, tipo, transaccion_id, descripcion,valor, usucrea_id, usumodif_id,producto_id,transapi_id,identificador,descripcion_txt) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
        ...
        $sqlQuery->set($transjuegoLog->descripcion);
        $sqlQuery->set($transjuegoLog->valor);
		$sqlQuery->setNumber($transjuegoLog->usucreaId);
        $sqlQuery->setNumber($transjuegoLog->usumodifId);
        $sqlQuery->setNumber($transjuegoLog->productoId);
        $sqlQuery->setNumber($transjuegoLog->transapiId);
        $sqlQuery->set($transjuegoLog->identificador);
        $sqlQuery->set($transjuegoLog->descripcionTxt);

		$id = $this->executeInsert($sqlQuery);
		$transjuegoLog->transjuegoinfoId = $id;
		return $id;
	}
	...
	{
		$sql = 'UPDATE transjuego_info SET transjuego_id = ?, tipo = ?, transaccion_id = ?, descripcion = ?, valor = ?,usucrea_id = ?, usumodif_id = ?, producto_id=?,transapi_id=?,identificador=?,descripcion_txt=? WHERE transjuegoinfo_id = ?';
		$sqlQuery = new SqlQuery($sql);
		...
		$sqlQuery->setNumber($transjuegoLog->transjuegoinfoId);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info';
		$sqlQuery = new SqlQuery($sql);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE transjuego_id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * las columnas transjuego_id y tipo sean iguales a los
    ...
    {
        $sql = 'SELECT * FROM transjuego_info WHERE transjuego_id = ? AND tipo=?';
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->setNumber($value);
        $sqlQuery->set($tipo);
        return $this->getList($sqlQuery);
	...
	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * las columnas transapi_id y tipo sean iguales a los
 	 * valores pasados como parámetros
 	 *
	 ...
	 *
 	 */
    public function queryByTransapiIdAndTipo($value,$tipo)
    {
        $sql = 'SELECT * FROM transjuego_info WHERE transapi_id = ? AND tipo=?';
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->setNumber($value);
        $sqlQuery->set($tipo);
        return $this->getList($sqlQuery);
    ...
    {
        $sql = 'SELECT * FROM transjuego_info WHERE identificador = ? ';
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->set($value);
        return $this->getList($sqlQuery);
    }
	...
	/**
 	 * Obtener todos los registros donde se encuentre que
 	 * las columnas transjuego_id y transaccion_id sean iguales a los
 	 * valores pasados como parámetros
 	 *
 	 * @param String $value transjuego_id requerido
    ...
    {
        $sql = 'SELECT * FROM transjuego_info WHERE transjuego_id = ? AND transaccion_id=?';
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->setNumber($value);
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE tipo = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE transaccion_id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE descripcion = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE fecha_crea = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE usucrea_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE fecha_modif = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'SELECT * FROM transjuego_info WHERE usumodif_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->getList($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE transjuego_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE tipo = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE transaccion_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE descripcion = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE fecha_crea = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE usucrea_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE fecha_modif = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	{
		$sql = 'DELETE FROM transjuego_info WHERE usumodif_id = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	/**
 	 * Crear y devolver un objeto del tipo TransjuegoInfo
 	 * con los valores de una consulta sql
 	 * 
 	 *
 	 * @param Arreglo $row arreglo asociativo
 	 *
 	 * @return Objeto $transjuegoLog TransjuegoInfo
 	 *
 	 * @access protected
 	 *
 	 */
	...
	{
		$transjuegoLog = new TransjuegoInfo();

		$transjuegoLog->transjuegoinfoId = $row['transjuegoinfo_id'];
		$transjuegoLog->transjuegoId = $row['transjuego_id'];
		$transjuegoLog->tipo = $row['tipo'];
		$transjuegoLog->transaccionId = $row['transaccion_id'];
        $transjuegoLog->descripcion = $row['descripcion'];
        ...
        $transjuegoLog->productoId = $row['producto_id'];
        $transjuegoLog->transapiId = $row['transapi_id'];
        $transjuegoLog->identificador = $row['identificador'];
        $transjuegoLog->descripcionTxt = $row['descripcion_txt'];
