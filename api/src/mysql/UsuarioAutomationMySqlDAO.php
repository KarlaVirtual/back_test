...
use Backend\dao\UsuarioAutomationDAO;
...
* 
*/
class UsuarioAutomationMySqlDAO implements UsuarioAutomationDAO
{
    ...
    *
    *
    * @param Objeto $transaction transautomationId
    *
    * @return no
    * @throws no
    *
    ...
    */
     ...
     * ordenadas por el nivel de la columna 
     * que se pasa como parámetro
     *
     * @param String $orderColumn nivel de la columna
     *
     * @return Array $ resultado de la consulta
     *
     */
    ...
    {
        $sql = 'INSERT INTO usuario_automation (usuario_id, tipo, valor, estado, usucrea_id, usumodif_id,automation_id,nivel,observacion,fecha_accion,usuaccion_id,externo_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        $sqlQuery = new SqlQuery($sql);
        ...
        $sqlQuery->set($usuario_automation->nivel);
        $sqlQuery->set($usuario_automation->observacion);
        $sqlQuery->set($usuario_automation->fechaAccion);
        $sqlQuery->set($usuario_automation->usuaccionId);

        if($usuario_automation->externoId == ""){
            $usuario_automation->externoId="0";
        }
        $sqlQuery->set($usuario_automation->externoId);
    ...
    {
        $sql = 'UPDATE usuario_automation SET usuario_id = ?, tipo = ?, valor = ?, estado = ?,  usucrea_id = ?,  usumodif_id = ?, automation_id = ?,nivel = ?,observacion = ?,fecha_accion = ?,usuaccion_id = ?,externo_id = ? WHERE usuautomation_id = ?';
        $sqlQuery = new SqlQuery($sql);
    ...
    {
        $usuario_automation = new UsuarioAutomation();
        ...
        $usuario_automation->nivel = $row['nivel'];
        $usuario_automation->observacion = $row['observacion'];
        $usuario_automation->fechaAccion = $row['fecha_accion'];
        $usuario_automation->usuaccionId = $row['usuaccion_id'];
        $usuario_automation->externoId = $row['externo_id'];
    ...
    */
    public function queryUsuarioAutomationsCustom($select, $sidx, $sord, $start, $limit, $filters, $searchOn)
    {
        ...
        $sql = 'SELECT count(*) count FROM usuario_automation INNER JOIN automation ON (usuario_automation.automation_id = automation.automation_id) INNER JOIN usuario_mandante ON (usuario_mandante.usumandante_id = usuario_automation.usuario_id)  ' . $where;
        ...
        $sql = 'SELECT ' .$select .'  FROM usuario_automation INNER JOIN automation ON (usuario_automation.automation_id = automation.automation_id)  INNER JOIN usuario_mandante ON (usuario_mandante.usumandante_id = usuario_automation.usuario_id) ' . $where . " order by " . $sidx . " " . $sord . " LIMIT " . $start . " , " . $limit;
