...
class UsuarioMandanteMySqlDAO implements UsuarioMandanteDAO
{
    ...
    */
     ...
     * las colomunas usuarioMandante y mandante sean iguales
     ...
     *
     */
    public function queryUsuarioMandanteAndMandante($usuarioMandante, $mandante)
    {

        $sql = 'SELECT * FROM usuario_mandante WHERE usuario_mandante = ? AND mandante = ?';
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->setNumber($usuarioMandante);
    ...
    {
        $sql = 'INSERT INTO usuario_mandante (mandante, usuario_mandante, nombres, apellidos, email, saldo, moneda, dir_ip, pais_id, estado, usucrea_id, usumodif_id,token_interno,token_externo, propio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        $sqlQuery = new SqlQuery($sql);
        ...
        $sqlQuery->set($Helpers->set_custom_secret_key('SECRET_PASSPHRASE_NAME', true)->encode_data($usuarioMandante->nombres));
        $sqlQuery->set($Helpers->set_custom_secret_key('SECRET_PASSPHRASE_LASTNAME', true)->encode_data($usuarioMandante->apellidos));
        $sqlQuery->set($Helpers->set_custom_secret_key('SECRET_PASSPHRASE_LOGIN', true)->encode_data($usuarioMandante->email));
        ...
        $sqlQuery->set($usuarioMandante->tokenInterno);
        $sqlQuery->set($usuarioMandante->tokenExterno);
        $sqlQuery->set($usuarioMandante->propio);
    ...
    {
        $sql = 'UPDATE usuario_mandante SET mandante = ?, usuario_mandante = ?, nombres = ?, apellidos = ?, email = ?, saldo = ?, moneda = ?, dir_ip = ?, pais_id = ?, estado = ?, usucrea_id = ?, usumodif_id = ?,token_interno=?,token_externo=?,propio=? WHERE usumandante_id = ?';
        $sqlQuery = new SqlQuery($sql);
    ...
    {
        $Helpers = new Helpers();
        $field = 'usuario_mandante.nombres';
        $field2 = $Helpers->set_custom_field($field);
        $sql = "SELECT * FROM usuario_mandante WHERE $field2 = ?";
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->set($value);
        return $this->getList($sqlQuery);
    }
    ...
    {
        $Helpers = new Helpers();
        $field = 'usuario_mandante.apellidos';
        $field2 = $Helpers->set_custom_field($field);
        $sql = "SELECT * FROM usuario_mandante WHERE $field2 = ?";
        $sqlQuery = new SqlQuery($sql);
        $sqlQuery->set($value);
    ...
    *
    */
    public function queryUsuariosMandantesCustom($select,$sidx,$sord,$start,$limit,$filters,$searchOn,$grouping)
    {
        ...
        $sql = "SELECT count(*) count FROM usuario_mandante LEFT OUTER JOIN usuario ON (usuario_mandante.usuario_mandante = usuario.usuario_id) LEFT OUTER JOIN registro ON (registro.usuario_id = usuario.usuario_id) LEFT OUTER JOIN pais ON (pais.pais_id = usuario.pais_id) LEFT OUTER JOIN mandante ON (usuario_mandante.mandante = mandante.mandante)  " . $where;

        $sqlQuery = new SqlQuery($sql);

        $count = $this->execute2($sqlQuery);
        $sql = "SELECT ".$select." FROM usuario_mandante LEFT OUTER JOIN usuario ON (usuario_mandante.usuario_mandante = usuario.usuario_id) LEFT OUTER JOIN registro ON (registro.usuario_id = usuario.usuario_id) LEFT OUTER JOIN pais ON (pais.pais_id = usuario.pais_id) LEFT OUTER JOIN mandante ON (usuario_mandante.mandante = mandante.mandante)  " . $where ." " . " order by " . $sidx . " " . $sord . " LIMIT " . $start . " , " . $limit;
    ...
    protected function readRow($row)
    {
        ...
        $usuarioMandante->nombres = $Helpers->set_custom_secret_key('SECRET_PASSPHRASE_NAME', true)->decode_data($row['nombres']);
        $usuarioMandante->apellidos = $Helpers->set_custom_secret_key('SECRET_PASSPHRASE_LASTNAME', true)->decode_data($row['apellidos']);
        $usuarioMandante->email = $Helpers->set_custom_secret_key('SECRET_PASSPHRASE_LOGIN', true)->decode_data($row['email']);
        ...
        $usuarioMandante->tokenInterno = $row['token_interno'];
        $usuarioMandante->tokenExterno = $row['token_externo'];
        $usuarioMandante->propio = $row['propio'];
