...
class UsuarioTokenMySqlDAO implements UsuarioTokenDAO
{
	...
	public function insert($usuarioToken){
		$sql = 'INSERT INTO usuario_token (usuario_id, proveedor_id, token, usucrea_id, fecha_crea, usumodif_id, fecha_modif) VALUES (?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
	...
	public function update($usuarioToken){
		$sql = 'UPDATE usuario_token SET usuario_id = ?, proveedor_id = ?, token = ?, usucrea_id = ?, fecha_crea = ?, usumodif_id = ?, fecha_modif = ? WHERE usutoken_id = ?';
		$sqlQuery = new SqlQuery($sql);
