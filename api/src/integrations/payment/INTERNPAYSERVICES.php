...
/**
 * Clase para gestionar servicios de integración de pagos con InternPay.
 *
 * @category   Red
 * @package    API
...
/**
 * Esta clase proporciona métodos para realizar solicitudes de pago, conexiones HTTP
 * y otras operaciones relacionadas con la integración de pagos. Incluye configuraciones
 * específicas para diferentes entornos y mandantes.
 */
class INTERNPAYSERVICES
{

    /**
     * Constructor de la clase `INTERNPAYSERVICES`.
     *
     ...
     *
     * Este método realiza varias operaciones, como calcular impuestos y comisiones,
     * registrar la transacción en la base de datos, y generar los datos necesarios
     * para la integración con Payphone. También maneja configuraciones específicas
     * según el entorno (desarrollo o producción) y el mandante/pais del usuario.
     *
    ...
    public function createRequestPayment(Usuario $Usuario, Producto $Producto, $valor, $urlSuccess, $urlFailed, $urlCancel)
    {
        ...
        $TOKEN_PAY = $credentials->TOKEN_PAY;
        ...
        if ($Result != "") {
            ...
            if ($ConfigurationEnvironment->isDevelopment()) {
                $baseUrl = "https://apidev.virtualsoft.tech/integrations/payment/internpay/api?id=";
            } else {
                $baseUrl = "https://integrations.virtualsoft.tech/payment/internpay/api?id=";
            }
    ...
    /**
     * Realiza una solicitud HTTP POST a un endpoint específico.
     *
     * Este método configura y ejecuta una solicitud HTTP POST utilizando cURL.
     * Además, selecciona el token de autenticación adecuado según el mandante y el país
    ...
    public function connectionPOST($data, $path, $mandante = '0', $pais = '173')
    {
        ...
        $curl->setOptionsArray(array(
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                'Authorization: Bearer ' . $TOKEN_PAY
            ),
        ));
