...
/**
 * Clase para gestionar los servicios de integración con MEGAPAYZ.
 *
 * Este archivo contiene la implementación de la clase `MEGAPAYZSERVICES`, que permite
 * realizar operaciones relacionadas con pagos a través de la plataforma MEGAPAYZ.
 *
 * @category Red
 * @package  API
...
/**
 * Clase principal para gestionar los servicios de integración con MEGAPAYZ.
 *
...
class MEGAPAYZSERVICES
{
    ...
    public function createRequestPayment(Usuario $Usuario, Producto $Producto, $valor, $urlSuccess, $urlFailed, $urlCancel)
    {
        ...
        $data['return_url'] = $this->URLDEPOSIT;

        $path = "/create-deposit";
        $Result = $this->connectionPOST($data, $Credentials->URL . $path);

        syslog(LOG_WARNING, "MEGAPAYZ DATA: " . $Usuario->usuarioId . json_encode($data) . " RESPONSE: " . $Result);
    ...
    /**
     * Realiza una conexión HTTP POST para crear una solicitud en la API de MEGAPAYZ.
     *
