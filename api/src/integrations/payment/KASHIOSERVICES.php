<?php
/**
 * Clase `KASHIOSERVICES` para gestionar servicios de pago y solicitudes de depósito.
 *
 * Este archivo contiene la implementación de la clase `KASHIOSERVICES`, que incluye
 * métodos para crear solicitudes de pago, realizar conexiones HTTP POST y obtener
 ...
 *
 * Esta clase gestiona servicios relacionados con pagos y solicitudes de depósito,
 * proporcionando métodos para interactuar con servicios externos y manejar transacciones.
 */
class KASHIOSERVICES
{
    /**
     * URL para gestionar depósitos.
     *
     * @var string
     */
    private $URLDEPOSIT = '';
     ...
     * específicos, calcula impuestos, registra la transacción y envía la
     * solicitud al servicio de pagos.
     *
     * @param Usuario  $Usuario    Objeto del usuario que realiza la solicitud.
     * @param Producto $Producto   Objeto del producto asociado a la solicitud.
     ...
     *
     * @return array Respuesta del servicio de pagos.
     */
    public function createRequestPayment(Usuario $Usuario, Producto $Producto, $valor, $urlSuccess, $urlFailed, $urlCancel)
    {
        ...
        $data['customer'] = [
            ...
            "external_id" => strval($Registro->usuarioId)
        ];
        $data['invoice_id'] = $transproductoId;
        ...
        $Result = $this->connectionPOST($data, $Credentials->URL . $path, $Credentials->KEY);

        syslog(LOG_WARNING, "KASHIO DATA: " . $Usuario->usuarioId . json_encode($data) . " RESPONSE: " . $Result);
     ...
     *
     * Este método utiliza cURL para enviar datos a un endpoint específico
     ...
     */
    public function connectionPOST($data, $url, $key)
    {
        $curl = curl_init();
     ...
     *
     * Este método detecta y devuelve la dirección IP del cliente que realiza
