...
/**
 * Clase ENGLOBASERVICES
 *
 * Esta clase proporciona servicios de integración de pagos con diferentes plataformas.
 * Incluye métodos para realizar solicitudes de pago, conexiones HTTP (POST y GET),
 * y utilidades como la obtención de la dirección IP del cliente.
 *
 * @category Red
 * @package  API
 * @author   Desconocido
...
class ENGLOBASERVICES
{
    ...
    /**
     * Crea una solicitud de pago para un usuario y un producto específico.
     *
     * Este método realiza los cálculos necesarios para impuestos y comisiones,
     * configura los datos de la transacción y genera una solicitud de pago.
     * Además, registra la transacción en la base de datos y devuelve los datos
     ...
     *
     * @return object Objeto con los datos de la solicitud de pago.
     */
    public function createRequestPayment(Usuario $Usuario, Producto $Producto, $valor, $urlSuccess, $urlFailed, $urlCancel)
    {
        ...
        if ($Result != "") {
            ...
            if ($ConfigurationEnvironment->isDevelopment()) {
                $baseUrl = "https://apidev.virtualsoft.tech/integrations/payment/englobamarketing/api?id=";
            } else {
                $baseUrl = "https://integrations.virtualsoft.tech/payment/englobamarketing/api?id=";
            }
    ...
    /**
     * Realiza una conexión HTTP POST al servicio de pagos.
     *
     * Este método envía datos en formato JSON a una URL específica utilizando el método POST.
     * Configura el token de autenticación según el mandante y el país proporcionados.
     *
     * @param array  $data     Datos a enviar en la solicitud POST.
     * @param string $path     Ruta del endpoint al que se realizará la solicitud.
     * @param string $mandante Identificador del mandante (por defecto '0').
     * @param string $pais     Código del país (por defecto '173').
     *
     * @return string Respuesta del servicio en formato JSON.
     */
    public function connectionPOST($data, $path, $mandante = '0', $pais = '173')
    {
        $Subproveedor = new Subproveedor("", "ENGLOBAMARKETING");
    ...
    /**
     * Realiza una conexión HTTP GET al servicio de pagos.
     *
     * Este método envía una solicitud GET a una URL específica para obtener información
     * relacionada con una transacción de producto.
     *
     * @param string $mandante         Identificador del mandante.
     * @param string $transproducto_id Identificador de la transacción del producto.
     * @param string $path             Ruta del endpoint al que se realizará la solicitud.
     *
     * @return string Respuesta del servicio en formato JSON.
     */
