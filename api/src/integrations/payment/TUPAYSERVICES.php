 ...
 * Clase TUPAYSERVICES
 *
 * Este archivo contiene la clase `TUPAYSERVICES` que gestiona la integración con el servicio de pagos TUPAY.
 *
 * @category Red
 * @package  API
 * @author   Desconocido
 ...
 * Clase TUPAYSERVICES
 *
 * Proporciona métodos para crear solicitudes de pago y gestionar la comunicación con el servicio de pagos TUPAY.
 */
class TUPAYSERVICES
{
    ...
    /**
     * Crea una solicitud de pago y la registra en el sistema.
     *
    ...
    public function createRequestPayment(Usuario $Usuario, Producto $Producto, $valor, $urlSuccess, $urlFailed, $urlCancel)
    {
        ...
        //Trae las credenciales de la base de datos
        ...
        if($Usuario->mandante == 0 && $Usuario->paisId == 94) {
            ...
            // Fecha actual en formato ISO 8601 con zona horaria -05:00
            $now = new DateTime("now", new DateTimeZone('America/Guatemala'));
            $timestamp = $now->format('Y-m-d\TH:i:sP');

            // Fecha de vencimiento (mañana) en mismo formato
            $now->modify('+1 day');
            $dueDate = $now->format('Y-m-d\TH:i:sP');
        }else {
            $timestamp = date("Y-m-d\TH:i:sP");
            $dueDate = date("Y-m-d\TH:i:sP", strtotime($timestamp . ' +1 day'));
        }

        $data = [
            'customId' => $transproductoId,
        ...
        //envía los datos a la api del proveedor
        $Result = $this->connection(json_encode($data), $Credentials->URL . $path, $Credentials->API_KEY);

        $Result = json_decode($Result);

        //Si la respuesta del proveedor es correcta, crea un nuevo registro en TransprodLog
        if ($Result->code == 200) {
            $TransprodLog = new TransprodLog();
            $TransprodLog->setTransproductoId($transproductoId);
            $TransprodLog->setEstado('E');
            $TransprodLog->setTipoGenera('A');
            $TransprodLog->setComentario('Envio Solicitud de deposito TUPAY');
            ...
            //Detecta dispositivos con condiciones especiales
            ...
            $data_ = array();
            $data_["success"] = true;
            $data_["url"] = $Result->data->checkoutUrl;
            $data_["target"] = '_blank';
        }
    ...
    /**
     * Realiza una conexión HTTP POST con el servicio de pagos.
     *
     * @param string $data    Datos en formato JSON que se enviarán en la solicitud.
     * @param string $Url     URL del servicio de pagos.
     * @param string $Api_Key Clave API para autenticar la solicitud.
     *
     * @return string Respuesta del servicio en formato JSON.
     */
    public function connection($data, $Url, $Api_Key)
    {
