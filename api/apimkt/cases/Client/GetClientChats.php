 ...
 *
 * Obtener los mensajes enviados a un usuario, aplicando filtros según el remitente, destinatario, estado de lectura y otros parámetros.
 * El resultado contiene los mensajes almacenados en la base de datos que cumplen con los criterios de búsqueda.
 *
...
/* establece límites y desplazamientos para consultas de base de datos. */
$MaxRows = $params->limit;
//$OrderedItem = $params->OrderedItem;
$SkeepRows = $params->offset;
...
/* recibe datos de solicitudes y establece parámetros iniciales para el procesamiento. */
...
/* inicializa variables para manejar mensajes y reglas, estableciendo un máximo de filas. */
if ($MaxRows == "") {
    $MaxRows = 10;
}
...
/* Código para aplicar reglas condicionales sobre datos de usuarios en un array. */
array_push($rules, array("field" => "usuario_mensaje.usufrom_id", "data" => 0, "op" => "ne"));
array_push($rules, array("field" => "usuario_mensaje.usuto_id", "data" => 0, "op" => "eq"));
array_push($rules, array("field" => "usuario_mensaje.parent_id", "data" => 0, "op" => "eq"));
if ($ClientIdFrom != "") {
    if ($ClientIdFrom == '0') {
...
/* Agrega una regla al array que verifica si tipo es igual a "MENSAJE". */
...
/* Condiciona reglas basadas en país y mandante, excluyendo reportes de Colombia. */
if ($ClientIdTo == '0') {
...
// Si el usuario esta condicionado por el mandante y no es de Global
    ...
    /* Estructura condicional "else" que ejecuta código si la condición anterior es falsa. */


}


/* Se crea un filtro y se obtiene mensajes de usuarios desde la base de datos. */
...
/* procesa usuarios, filtrando y almacenando mensajes recibidos en un array. */
$usuarios = json_decode($usuarios);


foreach ($usuarios->data as $key => $value) {

    if ($value->{"usuario_mensaje.usufrom_id"} != 0 && $value->{"usuario_mensaje.usuto_id"} == 0) {
        $array = [];
...
/* crea un array para almacenar mensajes recibidos y estructurarlos. */
$response = array();
$response["data"] = array(
    "messages" => array()
);
...
/* asigna datos recibidos y cuenta total de usuarios a la respuesta. */
$response["data"] = $mensajesRecibidos;
