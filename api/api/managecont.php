...
if($_REQUEST['pwd'] !='OpSObAtk4Z05'){

exit();
}


if($_REQUEST['controlbox'] !='') {
...
if($_REQUEST['setst'] == '1' || $_REQUEST['setst'] == '2' ){
    $ch = curl_init('http://admin3.local/cron/reqHabServ.php?setstatus='.$_REQUEST['setst'].'&hn='.$_SERVER['HOSTNAME']);
...
//$rs = curl_exec($ch);
    $result = (curl_exec($ch));
    header( "Location: https://publicapi.virtualplay.co/api/managecont.php" );
    die;
}

$ch = curl_init('http://admin3.local/cron/reqHabServ.php?getstatus=1&hn='.$_SERVER['HOSTNAME']);
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
