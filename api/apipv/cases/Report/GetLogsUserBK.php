...
// Procesa la consulta si los parámetros son válidos
if ($seguir) {

    // Convierte el flag de detalles a booleano
    if ($IsDetails == 1) {
        $IsDetails = false;
    } else {
        $IsDetails = true;
    ...
    // Aplica filtros adicionales según los parámetros y permisos del usuario
    if ($ProductId != "") {
    ...
    // Agrega filtros por región, jugador y usuario
    if ($Region != "") {
    ...
    // Aplica filtros por tipo de transacción
    if ($Type != "") {
        if ($Type == "N") {
    ...
    // Aplica filtros de seguridad basados en país y mandante
    ...
    foreach ($data->data as $key => $value) {
        ...
        // Mapea los tipos de transacción a descripciones legibles
        ...
        // Agrega información adicional de la transacción
