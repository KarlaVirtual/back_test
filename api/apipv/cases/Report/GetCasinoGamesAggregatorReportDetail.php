 ...
 * 
 * Obtiene el historial detallado de apuestas realizadas en juegos de casino,
 * incluyendo información de transacciones y valores
 *
 * @param int $id ID de la transacción a consultar
 *
 * @return array {
 *   "HasError": boolean,
 *   "AlertType": string,
...
// Obtiene y valida el ID de la transacción desde la solicitud
$Id = intval($_REQUEST["id"]);

if ($Id != "" && $Id != "0") {

    // Configura las reglas de filtrado para obtener los logs de transacciones
    $rules = [];
    array_push($rules, array("field" => "transjuego_log.transjuego_id", "data" => $Id, "op" => "eq"));

    // Prepara el JSON con los filtros para la consulta
    ...
    // Realiza la consulta a la base de datos para obtener los logs
    ...
    // Inicializa arreglos y contadores para procesar los resultados
    $final = [];
    $papuestas = 0;
    $ppremios = 0;
    $pcont = 0;

    // Procesa cada registro obtenido y construye el array de respuesta
    foreach ($data->data as $key => $value) {
        $CurrencyId = $value->{"usuario_mandante.moneda"};
        $array = [];

        // Mapea los campos necesarios para la respuesta
    ...
    // Si no hay ID válido, prepara una respuesta vacía
    $response["HasError"] = false;
    $response["AlertType"] = "success";
    $response["AlertMessage"] = "";
    $response["ModelErrors"] = [];
