 ...
 * 
 * Obtiene la lista de notas de retiro con filtros
 *
 * @param object $params {
 *   "ToDateLocal": string,         // Fecha final (Y-m-d)
 *   "FromDateLocal": string,       // Fecha inicial (Y-m-d)
 *   "BetShopId": int,             // ID del punto de venta
 *   "ClientId": int,              // ID del cliente
 *   "PaymentTypeId": int,         // ID del tipo de pago
 *   "State": string,              // Estado del retiro
 *   "WithDrawTypeId": int,        // ID del tipo de retiro
 *   "ByAllowDate": boolean,       // Filtrar por fecha de autorización
 ...
 *   "Data": array {
 *     "Objects": array[],         // Lista de retiros
 *     "Count": int               // Total de registros
 *   }
 * }
 */
// Inicializa el objeto CuentaCobro para manejar las operaciones de retiro
$CuentaCobro = new CuentaCobro();

// Procesa y formatea las fechas de inicio y fin del periodo
...
// Obtiene los parámetros de filtrado desde la solicitud
...
// Convierte el parámetro ByAllowDate a booleano y establece valores por defecto para la paginación
...
// Aplica filtros de fecha según si es por fecha de creación o fecha de autorización
...
// Agrega filtros adicionales según los parámetros recibidos
if ($BetShopId != "") {
...
// Aplica filtros específicos según el tipo de retiro
if ($WithDrawTypeId == "1") {
...
// Prepara y ejecuta la consulta para obtener los retiros
...
foreach ($cuentas->data as $key => $value) {
    ...
    // Asigna los datos básicos del retiro
    ...
    // Determina el método de pago y estado del retiro
    ...
    // Configura la información del banco si existe
    ...
    // Asigna los datos adicionales del retiro
