...
/**
 * Report/GetPointsUsers
 * 
 * Obtiene la lista de usuarios con sus puntos acumulados según los filtros especificados
 *
 * @param array $params {
 ...
 *   "PlayerId": int,       // ID del jugador a filtrar
 *   "CountrySelect": int   // ID del país a filtrar
 * }
 *
 * @return array {
 ...
 */
     ...
     * ),
     */
// Obtiene los parámetros de fecha, cliente y detalles del request
...
// Obtiene los parámetros de paginación y filtros
...
if ($seguir) {
    ...
    // Agrega filtros por usuario si se especifica
    if ($UserId != "") {
    ...
    // Agrega filtro por país si se especifica
    if ($CountrySelect != "") {
        array_push($rules, array("field" => "usuario.pais_id", "data" => "$CountrySelect ", "op" => "eq"));
    }

    // Aplica restricciones de acceso según configuración del usuario
    ...
    // Define los campos a seleccionar según el tipo de reporte
    $select = " usuario.usuario_id,usuario.moneda,usuario.nombre,usuario.fecha_crea,usuario_puntoslealtad.puntos_lealtad, usuario_puntoslealtad.puntos_aexpirar,usuario_puntoslealtad.puntos_expirados,usuario_puntoslealtad.puntos_redimidos";
    $grouping = "usuario.usuario_id";
    if ($Type == 1) {
        $select = " usuario.moneda,SUM(usuario.puntos_lealtad) puntos_lealtad,SUM(usuario.puntos_aexpirar) puntos_aexpirar, SUM(usuario.puntos_expirados) puntos_expirados,SUM(usuario.puntos_redimidos) puntos_redimidos";
    ...
    $Usuario = new Usuario();
    $data = $Usuario->getUsuariosCustom($select, "usuario.usuario_id", "asc", $SkeepRows, $MaxRows, $json, true, $grouping);
    ...
    foreach ($data->data as $value) {
        ...
        $array["PointsToExpire"] = $value->{"usuario_puntoslealtad.puntos_aexpirar"};
        $array["Points"] = $value->{"usuario_puntoslealtad.puntos_lealtad"} + intval($array["PointsToExpire"]);
        $array["ExpiredPoints"] = $value->{"usuario_puntoslealtad.puntos_expirados"};
        $array["RedeemedPoints"] = $value->{"usuario_puntoslealtad.puntos_redimidos"};
        if ($Type == 1) {
            $array["Points"] = $value->{".puntos_lealtad"};
            $array["PointsToExpire"] = $value->{".puntos_aexpirar"};
            $array["ExpiredPoints"] = $value->{".puntos_expirados"};
            $array["RedeemedPoints"] = $value->{".puntos_redimidos"};
        }
    ...
    // Prepara respuesta vacía si los parámetros no son válidos
    $response["HasError"] = false;
    $response["AlertType"] = "success";
    $response["AlertMessage"] = "Operation has completed successfully";
    $response["ModelErrors"] = [];
