 ...
 *
 * Este recurso actualiza el tipo de producto asociado a un partner.
 *
 * @param object $params Objeto con los siguientes valores:
 * @param int $params->Id ID del tipo de producto (obligatorio).
 * @param string $params->IsActivate Estado de activación ("A" o "I") (opcional).
 * @param string $params->Url URL asociada al producto (opcional).
 * @param int $params->ChangeKey Indica si se debe cambiar la clave (1 o 0) (opcional).
 * 
 * 
...
if ($Id != "") {
    ...
    /* asigna valores a variables y verifica condiciones de activación. */
    ...
    /* actualiza estado y siteKey de un objeto según condiciones específicas. */
    if ($IsActivate != "") {
    ...
    /* Se asignan valores a propiedades y se crea un objeto para manejo de base de datos. */
    ...
    /* Actualiza un registro en la base de datos y confirma la transacción sin errores. */
    $ProdMandanteTipoMySqlDAO->update($ProdMandanteTipo);
    $ProdMandanteTipoMySqlDAO->gsetTransaction()->commit();
    ...
    /* maneja un error, estableciendo datos de respuesta de tipo error. */
