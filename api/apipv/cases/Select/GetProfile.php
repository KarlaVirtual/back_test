 ...
 *   "Data": array[{         // Lista de perfiles
 *     "id": int,           // ID del perfil
 *     "value": string      // Valor del perfil
 *   }]
 * }
 *
...
// Define la consulta SQL para obtener todos los perfiles
$sql=
    "SELECT *
FROM perfil
WHERE 1 = 1" ;

// Inicializa los objetos necesarios para acceder a la base de datos
$UsuarioPerfil = new \Backend\dto\UsuarioPerfil();
$UsuarioPerfilMySqlDAO = new \Backend\mysql\UsuarioPerfilMySqlDAO();

// Obtiene y comienza una nueva transacción
$transaccion = $UsuarioPerfilMySqlDAO->getTransaction();
$transaccion->getConnection()->beginTransaction();

// Ejecuta la consulta y procesa los resultados
$final = $UsuarioPerfil->execQuery($transaccion, $sql);
$arrayF = array();
foreach ($final as $key => $value) {
    $array = array();
    $array["id"] = $value["perfil.perfil_id"];
    $array["value"] = $value["perfil.perfil_id"]; 
    array_push($arrayF, $array);
}

// Prepara la respuesta con los perfiles encontrados
