...
/**
 * ProviderPaymentWithdrawalSelect
 *
 * Este script permite obtener una lista de productos y proveedores filtrados según las condiciones especificadas.
 *
...
/* Se inicializa un objeto y se configuran parámetros para filtrar productos. */
...
/* Se inicializa un arreglo vacío llamado "rules" para almacenar reglas o directrices. */
$rules = [];

if ($keyword != "" & $keyword != null) {

    /* Se agregan reglas de filtrado a un arreglo según condiciones específicas del usuario. */
    array_push($rules, array("field" => "producto.descripcion", "data" => $keyword, "op" => "cn"));

    array_push($rules, array("field" => "proveedor.tipo", "data" => "PAYOUT", "op" => "eq"));
...
// Si el usuario esta condicionado por el mandante y no es de Global
    if ($_SESSION['Global'] == "N") {
        ...
        /* verifica una sesión y agrega reglas a un array si se cumplen condiciones. */
    ...
    /* Se filtran productos según reglas definidas y se codifican en formato JSON. */
    $filtro = array("rules" => $rules, "groupOp" => "AND");
    $json = json_encode($filtro);


    $productos = $ProductoMandante->getProductosMandanteCustom("producto.producto_id, producto.descripcion,proveedor.descripcion", "producto.producto_id", "asc", $SkeepRows, $MaxRows, $json, true);

    $productos = json_decode($productos);

    /* Construye un arreglo final con id y descripción de productos y proveedores. */
    $final = [];
...
/* define una respuesta con éxito y datos finales, sin errores. */
$response["HasError"] = false;
$response["AlertType"] = "success";
$response["AlertMessage"] = "";
$response["ModelErrors"] = [];

/* asigna valores a una respuesta JSON basada en datos obtenidos. */
$response["Data"] = $final;
