...
/**
 * Select/GamesCasinoSelect
 * 
 * Obtiene la lista de juegos de casino para mostrar en un select
 *
 * @param object $params {
 *   "Filter": string      // Texto para filtrar juegos por nombre
 * }
 *
 * @return array {
 ...
 *     "Objects": array[{    // Lista de juegos
 *       "id": int,         // ID del juego
 *       "value": string    // Nombre del juego
 *     }],
...
// Inicialización del array de reglas para el filtrado
$rules = [];
...
if ($keyword != "" & $keyword != null) {
    ...
    array_push($rules, array("field" => "proveedor.tipo", "data" => "'CASINO','LIVECASINO'", "op" => "in"));
    array_push($rules, array("field" => "proveedor.estado", "data" => "A", "op" => "eq"));
    array_push($rules, array("field" => "producto.estado", "data" => "A", "op" => "eq"));

    // Validación y configuración de reglas según el mandante del usuario
    ...
    // Validación y configuración de reglas según el país del usuario
    ...
    // Preparación del filtro JSON para la consulta
    $filtro = array("rules" => $rules, "groupOp" => "AND");
    $json = json_encode($filtro);

    // Obtención de productos según los filtros configurados
    $productos= $ProductoMandante->getProductosMandanteCustom("producto_mandante.prodmandante_id, producto.descripcion,proveedor.descripcion", "producto_mandante.prodmandante_id", "asc", $SkeepRows, $MaxRows, $json, true);

    $productos = json_decode($productos);
    $final = [];

    // Formateo de los datos obtenidos al formato requerido
