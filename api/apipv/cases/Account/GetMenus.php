 ...
 * Account/GetMenus
 *
 * Verificación y manejo del inicio de sesión y generación de menús.
 *
 * Este recurso verifica si el usuario ha iniciado sesión correctamente. En caso contrario, genera un mensaje de error. Si el usuario está logueado, se obtienen los menús y permisos según su perfil. Además, gestiona la autenticación y la asignación de permisos para las vistas específicas.
 *
 * @param object $params : Parámetros de entrada que contienen las configuraciones de filtrado y orden de los menús.
 * @param int $params->MaxRows : Número máximo de filas que se deben mostrar.
 * @param int $params->OrderedItem : Ítem de orden para la consulta.
 * @param int $params->SkeepRows : Número de filas a saltar para la paginación.
 *
 ...
 *  - *Data* (array): Contiene la información del estado de autenticación y los permisos de usuario.
 *
 ...
 *
 * @throws Exception Si el proceso de autenticación o la consulta de menús falla.
 *
 * @access public
 * @see no
 * @since no
...
/* Verifica si el usuario no está logueado y genera un mensaje de error. */
if (!$_SESSION['logueado']) {
    ...
    try {
        ...
        /* Se crean objetos UsuarioMandante y Usuario usando datos de sesión del usuario. */
     ...
     */
        ...
        /* Verifica si el usuario tiene acceso a ciertos menús en una sesión. */
        if ($_SESSION['usuario2'] == "5") {
            ...
            /* Se inicializa un objeto y se obtienen parámetros de sesión y entrada. */
            ...
            /* Establece un valor predeterminado y construye una consulta JSON con reglas específicas. */
            if ($MaxRows == "") {
                $MaxRows = 100000;
            }
            ...
            /* Obtiene y decodifica menús y submenús personalizados, almacenándolos en arreglos. */
            ...
            /* Se inicializan dos arrays: $submenus y $menus_string para almacenar elementos de menú. */
            $submenus = [];
            ...
            foreach ($menus->data as $key => $value) {
                ...
                /* Se crea un array asociativo con datos de menú y submenú. */
                $m = [];
                ...
                /* asigna valores a un arreglo y concatena identificadores de menú. */
                ...
                /* añade páginas a un menú si cumplen ciertas condiciones de ID. */
                ...
                /* Añade elementos a arrays y extrae datos de un objeto en PHP. */
            ...
            /* añade elementos a arrays y gestiona permisos de submenús. */
            array_push($menus_string, $arrayf["Pagina"]);
        ...
        array_push($menus_string, "view" . str_replace("_", "", str_replace(".php", "", $value["b.pagina"])));
            ...
            /* obtiene un menú y lo asigna a una variable de respuesta. */
            $menus_string = obtenerMenu();
        ...
        /* Manejo de excepciones en el inicio de sesión con mensajes de error personalizados. */
