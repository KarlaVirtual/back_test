...
/**
 * Accounting/GetDetailsIncomesPDF
 *
 * Generación de Soporte de Ingreso y PDF
 *
 * Este recurso genera un soporte de ingreso en formato HTML, que se convierte en un archivo PDF.
 * El soporte incluye detalles como el consecutivo de ingreso, fecha, concepto, descripción y valor,
 * y se genera con el logo y la información correspondiente.
 *
 * @param object $params : Objeto que contiene los parámetros necesarios para la generación del soporte de ingreso.
 * @param string $params->Id : El ID del ingreso para obtener los detalles asociados.
 * @param object $Ingreso : Instancia de la clase Ingreso que se utiliza para obtener la información del ingreso.
 * @param string $Ingreso->getUsuarioId() : ID del usuario asociado al ingreso.
 * @param string $Ingreso->getConsecutivo() : El consecutivo del ingreso que será parte del soporte.
 * @param string $Ingreso->getDescripcion() : Descripción del ingreso.
 * @param string $Ingreso->getConceptoId() : ID del concepto asociado al ingreso.
 * @param string $Ingreso->getProductotercId() : ID del producto tercero asociado al ingreso.
 * @param string $Ingreso->getTipoId() : ID del tipo de ingreso.
 * @param object $Concepto : Instancia de la clase Concepto utilizada para obtener la descripción del concepto.
 * @param object $ProductoTercero : Instancia de la clase ProductoTercero utilizada para obtener la descripción del producto tercero.
 * @param object $Clasificador : Instancia de la clase Clasificador utilizada para obtener la descripción del tipo de ingreso.
 * @param string $Clasificador->getAbreviado() : Abreviatura del tipo de clasificador que se utiliza para determinar el tipo de dinero.
 * @param string $Clasificador->getTipo() : Tipo de clasificador, como "TARJCRED" para tarjetas de crédito.
 * @param object $mpdf : Instancia de la clase mPDF utilizada para generar el archivo PDF del soporte de ingreso.
 *
 * @return object $response Objeto con los atributos de respuesta requeridos.
 *
 * El objeto $response es un array con los siguientes atributos:
 *  - *Pdf* (string): Contiene el archivo PDF codificado en base64.
 *  - *Pdf2* (string): Contiene el código HTML del soporte de ingreso.
 *  - *Data* (array): Contiene los datos totales asociados a la generación del soporte.
 ...
 *  - *pos* (int): Número de posiciones a saltar.
 *  - *total_count* (int): Contador total de los datos generados.
 *
 ...
 *
 * @throws Exception Si ocurre un error durante la creación del PDF o al acceder a los datos.
 *
 * @access public
 * @see no
 * @since no
...
/* Código para inicializar un tipo de dinero y generar un consecutivo de ingreso. */
...
/* Obtiene la descripción de un ingreso y su concepto, si está disponible. */
...
/* asigna descripciones a productos según sus IDs y tipos en condiciones específicas. */
...
/* Genera un PDF con detalles de soporte de ingreso, incluyendo logo y datos relevantes. */
...
/* Crea un PDF con márgenes espejados y visualización en dos páginas. */
...
/* Genera un PDF utilizando mPDF y guarda el archivo en el directorio temporal. */
$mpdf->WriteHTML($pdf);
...
/* lee un archivo, lo codifica en base64 y lo prepara para respuesta. */
...
/* asigna valores a un arreglo de respuesta sobre un PDF y datos totales. */
$response["Pdf2"] = $pdf;
...
/* Código PHP que define una respuesta JSON con tipo de alerta y conteo de datos. */
...
/* asigna el valor de $final a la clave "data" de la variable $response. */
$response["data"] = $final;