	...
	class QRcode {
		 ...
		 protected function mask($width, $frame, $level) {
			...
			if (QR_FIND_FROM_RANDOM !== false) {
				...
				for ($i = 0; $i <  $howManuOut; ++$i) {
					$remPos = rand (0, oldCount($checked_masks)-1);
		 ...
		 */
		 protected function newInputItem($mode, $size, $data, $bstream=null) {
			...
			if (oldCount($setData) < $size) {
				$setData = array_merge($setData, array_fill(0, ($size - oldCount($setData)), 0));
			}
		 ...
		 */
		 protected function createBitStream($items) {
			...
			foreach ($items as $key => $item) {
				...
				$bits = oldCount($items[$key]['bstream']);
				$total += $bits;
			}
		 ...
		 */
		 protected function appendBitstream($bitstream, $append) {
			if ((!is_array($append)) OR (oldCount($append) == 0)) {
				return $bitstream;
			}
			if (oldCount($bitstream) == 0) {
				return $append;
			}
