...
'.$content.'
        ...
        {
            if (!oldCount($this->_subHEADER)) return false;
        ...
        {
            if (!oldCount($this->_subFOOTER)) return false;
        ...
        protected function _setNewPositionForNewLine($curr = null)
        {
            ...
            $res = null;
            for ($sub->_parsePos; $sub->_parsePos<oldCount($sub->parsingHtml->code); $sub->_parsePos++) {
        ...
        {
            $this->_defList[oldCount($this->_defList)-1]['nb']++;
        }
        ...
        {
            $im = $this->_defList[oldCount($this->_defList)-1]['img'];
            $st = $this->_defList[oldCount($this->_defList)-1]['style'];
            $nb = $this->_defList[oldCount($this->_defList)-1]['nb'];
        ...
        protected function _listeAddLevel($type = 'ul', $style = '', $img = null)
        {
            ...
            // add the new level
            $this->_defList[oldCount($this->_defList)] = array('style' => $style, 'nb' => 0, 'img' => $img);
        }
        ...
        {
            if (oldCount($this->_defList)) {
                unset($this->_defList[oldCount($this->_defList)-1]);
        ...
        protected function _makeHTMLcode()
        {
            ...
            for ($this->_parsePos=0; $this->_parsePos<oldCount($this->parsingHtml->code); $this->_parsePos++) {
        ...
        protected function _calculateTableCellSize(&$cases, &$corr)
        {
            ...
            $sw = array();
            for ($x=0; $x<oldCount($corr[0]); $x++) {
                $m=0;
                for ($y=0; $y<oldCount($corr); $y++) {
        ...
        protected function _tag_open_PAGE_HEADER($param)
        {
            ...
            $this->_subHEADER = array();
            for ($this->_parsePos; $this->_parsePos<oldCount($this->parsingHtml->code); $this->_parsePos++) {
                $action = $this->parsingHtml->code[$this->_parsePos];
        ...
        protected function _tag_open_FIELDSET($param)
        {
            ...
            for ($tempPos = $this->_parsePos + 1; $tempPos<oldCount($this->parsingHtml->code); $tempPos++) {
                ...
                if ($action['name'] == 'legend' && !$action['close']) {
                    ...
                    $res = null;
                    for ($sub->_parsePos = 0; $sub->_parsePos<oldCount($sub->parsingHtml->code); $sub->_parsePos++) {
                        $action = $sub->parsingHtml->code[$sub->_parsePos];
        ...
        protected function _tag_open_WRITE($param)
        {
            ...
            while ($x+$w>$right && $x<$right+$space && oldCount($words)) {
        ...
        protected function _tag_open_UL($param, $other = 'ul')
        {
            ...
            if (!in_array($this->_previousCall, array('_tag_close_P', '_tag_close_UL'))) {
                if ($this->_maxH) $this->_tag_open_BR(array());
                if (!oldCount($this->_defList)) $this->_tag_open_BR(array());
            }
        ...
        protected function _tag_open_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                for ($pos=$this->_tempPos; $pos<oldCount($this->parsingHtml->code); $pos++) {
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (isset(HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {
                    ...
                    if (oldCount($colParam['style']['width'])) {
        ...
        {
            if (!isset($param['name'])) {
                $param['name'] = 'champs_pdf_'.(oldCount($this->_lstField)+1);
            }
        ...
        protected function _tag_open_OPTION($param)
        {
            ...
            $level = $this->parsingHtml->getLevel($this->_parsePos);
            $this->_parsePos+= oldCount($level);
            $value = isset($param['value']) ? $param['value'] : 'aut_tag_open_opt_'.(oldCount($this->_lstSelect)+1);
        ...
        {
            if (!isset($param['name']))  $param['name']  = 'champs_pdf_'.(oldCount($this->_lstField)+1);
            if (!isset($param['value'])) $param['value'] = '';
