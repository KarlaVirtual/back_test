...
class HTML2PDF_parsingCss
{
    ...
    {
        return isset($this->table[oldCount($this->table)-1]) ? $this->table[oldCount($this->table)-1] : $this->value;
    }
    ...
    */
        ...
        foreach ($styles as $nom => $val) {
            switch($nom)
            {
                ...
                case 'border-radius':
                    ...
                    if (oldCount($valH)<1 || oldCount($valH)>4) {
                        break;
                    }
                    ...
                    if (isset($val[1])) {
                        ...
                        if (oldCount($valV)<1 || oldCount($valV)>4) {
                            break;
                        }
                ...
                case 'border-top-left-radius':
                    ...
                    if (oldCount($val)<1 || oldCount($val)>2) {
                        break;
                    }
    ...
    {
        for ($k=oldCount($this->table)-1; $k>=0; $k--) {
    ...
    protected function _getFromCSS()
    {
        ...
        for ($i=oldCount($this->table)-1; $i>=0; $i--) {
    ...
    protected function _duplicateBorder(&$val)
    {
        ...
        if (oldCount($val)==1) {
            ...
            $val[3] = $val[0];
        // 2 values => L => R & T => B
        } else if (oldCount($val)==2) {
            $val[2] = $val[0];
            $val[3] = $val[1];
        // 3 values => T => B
        } else if (oldCount($val)==3) {
            $val[3] = $val[1];
        }
    }
        ...
        foreach ($match[1] as $code) {
            ...
            if (isset($tmp['type']) && strtolower($tmp['type'])=='text/css' && isset($tmp['href'])) {
                ...
                if (strpos($url, 'http://')!==false) {
                    ...
                    $urlSelf = $url; unset($urlSelf[oldCount($urlSelf)-1]); $urlSelf = 'http://'.implode('/', $urlSelf).'/';
