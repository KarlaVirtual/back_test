...
$message = "*CRON: (Inicio) * " . " ResumenesComisiones - Fecha: " . date("Y-m-d H:i:s");

exec("php -f ".__DIR__."../src/imports/Slack/message.php '" . $message . "' '#virtualsoft-cron' > /dev/null & ");

//BETWEEN '".$fecha1."' AND '".$fecha2."'

$strEliminado = "DELETE FROM usuario_comision WHERE date_format(fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "';
DELETE FROM usucomision_resumen WHERE date_format(fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "';
...
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "'
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
       ...
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND it_ticket_enc.beneficiario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
UNION
         SELECT clasificador.clasificador_id,
                ...
                0                                                                                     apuestas,
                SUM(CASE WHEN it_transaccion.tipo IN ('NEWCREDIT','WIN','REFUND','CASHOUT') then it_transaccion.valor else -it_transaccion.valor END) premios,
         ...
         WHERE it_transaccion.fecha_crea ='" . $fechaSoloDia . "'
           ...
           AND (it_transaccion.fecha_crea > it_ticket_enc.fecha_cierre/* OR
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
UNION
         SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,0 apuestas,SUM(CASE WHEN it_transaccion.tipo IN ('NEWCREDIT','WIN','REFUND','CASHOUT') then it_transaccion.valor else -it_transaccion.valor END) premios,concesionario.porcenpadre1,it_transaccion.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM it_transaccion
                  INNER JOIN it_ticket_enc ON (it_ticket_enc.ticket_id = it_transaccion.ticket_id)
                  INNER JOIN usuario ON (usuario.usuario_id = it_transaccion.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTAFF')
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
UNION
         SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,0 apuestas,SUM(CASE WHEN it_transaccion.tipo IN ('NEWCREDIT','WIN','REFUND','CASHOUT') then it_transaccion.valor else -it_transaccion.valor END) premios,concesionario.porcenpadre2,it_transaccion.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM it_transaccion
                  INNER JOIN it_ticket_enc ON (it_ticket_enc.ticket_id = it_transaccion.ticket_id)
                  INNER JOIN usuario ON (usuario.usuario_id = it_transaccion.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTAFF')
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,''
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTAFF')
               LEFT OUTER JOIN clasificador clasificador2
...
UNION
         ...
         GROUP BY it_transaccion.fecha_crea, usuario.usuario_id
         UNION
         SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(bono_log.valor) premios,concesionario.porcenhijo,bono_log.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM bono_log
                  ...
                  INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND
                                               ...
                                               concesionario.estado = 'A')
         WHERE date_format(bono_log.fecha_crea, '%Y-%m-%d') ='" . $fechaSoloDia . "' and bono_log.tipo IN ('F','D','ND') and bono_log.estado='L'
           AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
UNION
         ...
         GROUP BY it_transaccion.fecha_crea, usuario.usuario_id
         UNION
        SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,0 apuestas,SUM(bono_log.valor) premios,concesionario.porcenpadre1,bono_log.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM bono_log
                  INNER JOIN usuario ON (usuario.usuario_id = bono_log.usuario_id)
                         INNER JOIN registro ON (usuario.usuario_id = registro.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTNGRAFF')
...
UNION
         ...
         GROUP BY it_transaccion.fecha_crea, usuario.usuario_id
         UNION
        SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,0 apuestas,SUM(bono_log.valor) premios,concesionario.porcenpadre2,bono_log.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM bono_log
                  INNER JOIN usuario ON (usuario.usuario_id = bono_log.usuario_id)
                         INNER JOIN registro ON (usuario.usuario_id = registro.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTNGRAFF')
...
UNION
        SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(CASE WHEN it_transaccion.tipo IN ('NEWCREDIT','WIN','REFUND','CASHOUT') then it_transaccion.valor else -it_transaccion.valor END) premios,concesionario.porcenpadre3,it_transaccion.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         ...
         UNION
        SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(bono_log.valor) premios,concesionario.porcenpadre3,bono_log.fecha_crea fecha,0 usucrea_id,0 usumodif_id,''
         FROM bono_log
                  INNER JOIN usuario ON (usuario.usuario_id = bono_log.usuario_id)
                         INNER JOIN registro ON (usuario.usuario_id = registro.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTNGRAFF')
...
SELECT clasificador_id,
       ...
       usuhijo_id,
       usuario,
       (SUM(apuestas) - SUM(premios))                      valor,
       (SUM(apuestas) - SUM(premios)) * (porcenhijo / 100) comision,
       fecha,
       0                                                   usucrea_id,
       0                                                   usumodif_id,
       ''                                                  externo_id
FROM (
         ...
         WHERE it_ticket_enc.beneficiario_id != 0 AND mandante_detalle.valor = 2
           AND it_ticket_enc.tipo_beneficiario = 2
           AND concesionario.usuhijo_id != 0
           and it_transaccion.fecha_crea = '" . $fechaSoloDia . "'
...
SELECT clasificador_id,
       ...
       usuario,
       (SUM(apuestas) - SUM(premios))                        valor,
       (SUM(apuestas) - SUM(premios)) * (porcenpadre1 / 100) comision,
       fecha,
       0                                                     usucrea_id,
       0                                                     usumodif_id,
       ''                                                    externo_id
...
SELECT clasificador_id,
       ...
       usuario,
       (SUM(apuestas) - SUM(premios))                        valor,
       (SUM(apuestas) - SUM(premios)) * (porcenpadre2 / 100) comision,
       fecha,
       0                                                     usucrea_id,
       0                                                     usumodif_id,
       ''                                                    externo_id
...
SELECT clasificador_id,
       ...
       usuario,
       (SUM(apuestas) - SUM(premios))                        valor,
       (SUM(apuestas) - SUM(premios)) * (porcenpadre3 / 100) comision,
       fecha,
       0                                                     usucrea_id,
       0                                                     usumodif_id,
       ''                                                    externo_id
...
FROM (
         ...
         WHERE  concesionario.usuhijo_id != 0
           and usuario_casino_resumen.fecha_crea LIKE '%" . $fechaSoloDia . "%'
           AND usuario_perfil.perfil_id = 'USUONLINE'
         ...
         FROM bono_log
                  ...
                  INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND
                                               registro.afiliador_id = concesionario.usuhijo_id AND
                                               concesionario.estado = 'A')
         WHERE date_format(bono_log.fecha_crea, '%Y-%m-%d') ='" . $fechaSoloDia . "' and bono_log.tipo IN ('DC', 'DL', 'DV', 'NC', 'NL', 'NV') and bono_log.estado='L'
           AND registro.afiliador_id != 0
           AND concesionario.usuhijo_id !=0
         GROUP BY date_format(bono_log.fecha_crea, '%Y-%m-%d'), usuario.usuario_id
...
FROM (
         ...
         WHERE  usuario_casino_resumen.fecha_crea LIKE '%" . $fechaSoloDia . "%'
           AND usuario_perfil.perfil_id = 'USUONLINE'
           AND concesionario.usupadre_id != 0
         GROUP BY usuario_casino_resumen.fecha_crea, usuario_mandante.usuario_mandante
...
FROM it_ticket_enc
       ...
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND it_ticket_enc.beneficiario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND mandante_detalle.valor = 2
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
UNION
        SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(CASE WHEN it_transaccion.tipo IN ('NEWCREDIT','WIN','REFUND','CASHOUT') then it_transaccion.valor else -it_transaccion.valor END) premios,concesionario.porcenhijo,it_transaccion.fecha_crea fecha,0 usucrea_id,0 usumodif_id,'' externo_id
         FROM it_transaccion
                  INNER JOIN it_ticket_enc ON (it_ticket_enc.ticket_id = it_transaccion.ticket_id)
                  INNER JOIN usuario ON (usuario.usuario_id = it_transaccion.usuario_id)
                  INNER JOIN clasificador ON (clasificador.abreviado = 'SPORTPVIP')
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY it_ticket_enc.fecha_cierre, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPVIP')
       LEFT OUTER JOIN clasificador clasificador2
...
FROM (
         ...
         WHERE it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND mandante_detalle.valor = 2
           AND it_ticket_enc.beneficiario_id != 0
           AND it_ticket_enc.tipo_beneficiario = 1
           AND concesionario.usuhijo_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
...
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND concesionario.usuhijo_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND concesionario.usupadre_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND concesionario.usupadre2_id !=0 AND it_ticket_enc.bet_status != 'A'
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND concesionario.usupadre3_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND concesionario.usupadre4_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
ORDER BY fecha, usuario
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N'  AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I')) ) OR (it_ticket_enc.fecha_pago = '" . $fechaSoloDia . "' AND (mandante_detalle.valor IS NULL OR mandante_detalle.valor = 0) )) AND concesionario.usuhijo_id !=0 AND it_ticket_enc.bet_status != 'A'
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenhijo,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
...
WHERE it_transaccion.fecha_crea = '" . $fechaSoloDia . "'
  /*AND it_transaccion.tipo IN ('NEWCREDIT', 'NEWDEBIT')*/
    AND (it_transaccion.fecha_crea > it_ticket_enc.fecha_cierre AND (it_ticket_enc.fecha_pago IS NULL OR (it_ticket_enc.fecha_pago > it_transaccion.fecha_crea OR (it_ticket_enc.fecha_pago = it_transaccion.fecha_crea AND it_ticket_enc.hora_pago > it_transaccion.hora_crea))))
  AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre1,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
...
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre2,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
...
SELECT clasificador_id,concesionario_id,usuhijo_id,usuario,(SUM(apuestas) - SUM(premios)) valor,(SUM(apuestas) - SUM(premios))*(porcenhijo/100) comision,fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenhijo,it_ticket_enc.fecha_crea fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(-vlr_apuesta) apuestas, 0 premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ( (it_ticket_enc.eliminado='S'  AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) )) AND concesionario.usuhijo_id !=0 AND it_ticket_enc.premiado='S'
GROUP BY fecha, usuario.usuario_id
...
SELECT clasificador_id,concesionario_id,usupadre_id,usuario,(SUM(apuestas) - SUM(premios)) valor,(SUM(apuestas) - SUM(premios))*(porcenpadre1/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre1,it_ticket_enc.fecha_crea fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(-vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ( (it_ticket_enc.eliminado='S'  AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) )) AND concesionario.usupadre_id !=0 AND it_ticket_enc.premiado='S'
GROUP BY fecha, usuario.usuario_id
...
SELECT clasificador_id,concesionario_id,usupadre2_id,usuario,(SUM(apuestas) - SUM(premios)) valor,(SUM(apuestas) - SUM(premios))*(porcenpadre2/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM( vlr_apuesta ) apuestas, 0 premios,concesionario.porcenpadre2,it_ticket_enc.fecha_crea fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre2_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM( -vlr_apuesta ) apuestas, 0 premios,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre  fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ( (it_ticket_enc.eliminado='S'  AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre2_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) )) AND concesionario.usupadre2_id !=0 AND it_ticket_enc.premiado='S'
GROUP BY fecha, usuario.usuario_id
...
SELECT clasificador_id,concesionario_id,usupadre3_id,usuario,(SUM(apuestas) - SUM(premios)) valor,(SUM(apuestas) - SUM(premios))*(porcenpadre3/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre3,it_ticket_enc.fecha_crea fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
       INNER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre3_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM( -vlr_apuesta ) apuestas, 0 premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.eliminado='S'  AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre3_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) )) AND concesionario.usupadre3_id !=0 AND it_ticket_enc.premiado='S'
GROUP BY fecha, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre2_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre3_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM (
         ...
         WHERE
           it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND mandante_detalle.valor = 2
           AND concesionario.usuhijo_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
     ) a
GROUP BY fecha, usuario
...
SELECT clasificador_id,concesionario_id,usuhijo_id,usuario,(SUM(apuestas)) valor,(SUM(apuestas))*(porcenhijo/100) comision,fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenhijo,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
...
SELECT clasificador_id,concesionario_id,usupadre_id,usuario,(SUM(apuestas)) valor,(SUM(apuestas))*(porcenpadre1/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre1,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
...
SELECT clasificador_id,concesionario_id,usupadre2_id,usuario,(SUM(apuestas)) valor,(SUM(apuestas))*(porcenpadre2/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre2,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
...
SELECT clasificador_id,concesionario_id,usupadre3_id,usuario,(SUM(apuestas)) valor,(SUM(apuestas))*(porcenpadre3/100) comision,fecha,0 usucrea_id,0 usumodif_id,''
FROM (
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM(vlr_apuesta) apuestas, 0 premios,concesionario.porcenpadre3,CASE WHEN mandante_detalle.valor = 1 THEN it_ticket_enc.fecha_cierre WHEN it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I' THEN it_ticket_enc.fecha_cierre ELSE fecha_pago END fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       INNER JOIN usuario ON (usuario.usuario_id = it_ticket_enc.usuario_id)
...
FROM usuario_recarga
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'DEPOSITOPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.usuario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  date_format(usuario_recarga.fecha_elimina, '%Y-%m-%d') = '" . $fechaSoloDia . "'
...
WHERE  date_format(usuario_comision.fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "'  AND tipo='68'
...
foreach ($data as $datanum) {
    ...
    VALUES ('" . $datanum->{'usuario_comision.tipo'} . "','" . $datanum->{'.fecha'} . "','" . $datanum->{'usuario_comision.usuario_id'} . "','" . $datanum->{'usuario_comision.usuarioref_id'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.comision'} . "')";
    $BonoInterno->execQuery($transaccion, $sql);
}
...
$message = "*CRON: (Fin) * " . " ResumenesComisiones - Fecha: " . date("Y-m-d H:i:s");
