...
/** Función encargada de definir si un usuario cumple con la condición de primer depósito y actualziar la información del último usuario iterado
 * @param int $usuarioId Id del usuario por verificar
 * @param array $lastReviewedUser Instancia del array que acumula el usuario verificado en la itereación anterior y el estado de dicha verificación para futuras coincidencias
 * @return boolean
 */
function validateUserFirstDeposit(int $usuarioId, array &$lastReviewedUser): bool
{
    $validBet = true;

    //Verificando que usuario cumpla con condición de primer depósito
    if (!empty($lastReviewedUser) && $lastReviewedUser["userId"] == $usuarioId && !$lastReviewedUser["state"]) $validBet = false;
    elseif ($lastReviewedUser["userId"] != $usuarioId && !successOnFirstDepositCondition($usuarioId)) $validBet = false;

    //Almacenando ID último usuario bajo iteración de validaciones
    if ($lastReviewedUser["userId"] != $usuarioId) {
        $lastReviewedUser["userId"] = $usuarioId;
        $lastReviewedUser["state"] = $validBet;
    }
...
/** Función verifica que el usuario haya cumplido con la condición de primer depósito
 * @param int $usuarioId Id del usuario por verificar
 * @return boolean
 */
function successOnFirstDepositCondition(int $usuarioId): bool
{
    $rules = [];
    $LogroReferido = new LogroReferido();

    $select = "logro_referido.logroreferido_id";
    $sidx = "logro_referido.logroreferido_id";
    $rules[] = ['field' => 'clasificador.abreviado', 'data' => 'CONDMINFIRSTDEPOSITREFERRED', 'op' => 'eq'];
    $rules[] = ['field' => 'logro_referido.estado', 'data' => '"C", "R"', 'op' => 'in'];
    $rules[] = ['field' => 'logro_referido.estado_grupal', 'data' => '"C", "R", "PE"', 'op' => 'in'];
    $rules[] = ['field' => 'logro_referido.usuid_referido', 'data' => "{$usuarioId}", 'op' => 'eq'];
    $filters = ['rules' => $rules, 'groupOp' => 'AND'];

    $conditionFirstDeposit = $LogroReferido->getLogroReferidoCustom($select, $sidx, 'DESC', 0, 1, json_encode($filters), true, true);
    $conditionFirstDeposit = json_decode($conditionFirstDeposit);

    return $conditionFirstDeposit->count[0]->{'.count'} > 0;
}
...
$filename=__DIR__.'/cronLogrosReferidos';
...
//Conociendo información de la última ejecución del CRON
    ...
    try {
        ...
        foreach ($logroReferidoNewStatus as $logroUpdateInfo) {
            ...
            $LogroReferido->setFechaUso($fechaUso);
            $LogroReferido->setEstado($logroUpdateInfo['estado']);
            $newSuccessedAward = $LogroReferido->actualizarLogrosAgrupados($firstDepositTransaction, $LogroReferido);

            /** Lógica cancelación de premios por no cumplir primer depósito Parte 1*/
            //Almacenando el usuario_id de los usuarios a los cuales se les cancelarán los logros por no cumplir primer depósito
            if ($LogroReferido->estado == 'F' && !in_array($LogroReferido->usuidReferido, $cancelledReferreds)) {
                $cancelledReferreds[] = $LogroReferido->getUsuidReferido();
            }

            //En caso de haber varios premios por primer depósito a cargo de un mismo referido, se previene que uno de los premios cancele el otro
            $saveLastTuple = false;
            if (!$newSuccessedAward && $LogroReferido->getEstado() != 'F') $saveLastTuple = true;

            //Actualizando última dupla actualizada
            $lastUpdatedTuple = $LogroReferido->getUsuidReferido() . '_' . $LogroReferido->getTipoPremio();
            if ($saveLastTuple) $exemptedTuples[] = $lastUpdatedTuple;
            /** Fin lógica parte 1*/
        }

        $firstDepositTransaction->commit();

        /** Lógica cancelación de premios por no cumplir primer depósito Parte 2*/
        //Cancelando logros/Premios de usuarios que no cumplirieron con el mínimo de primer depósito
        $cancellingAchievementsTransaction = new Transaction();
        $LogroReferidoMySqlDAO = new LogroReferidoMySqlDAO($cancellingAchievementsTransaction);
        foreach ($cancelledReferreds as $referred) {
            $LogroReferido = new LogroReferido();
            $LogroReferido->cancelarLogrosPendientes($cancellingAchievementsTransaction, (int)$referred, $exemptedTuples);
        }

        $cancellingAchievementsTransaction->commit();
        /** Fin lógica parte 2 */
    ...
    try {
        ...
        $lastReviewedUser = [];
        $minSelPrice = null;
        $validBets = array_filter($sportbookBets, function ($bet) use ($BonoInterno, $sportBookMinBetTransaction, &$lastReviewedPartner_Country, &$minSelPrice, &$lastReviewedUser) {
            $validBet = true;

            //Verificando que usuario cumpla con condición de primer depósito
            if ($validBet) $validBet = validateUserFirstDeposit((int)$bet->{'usuario.usuario_id'}, $lastReviewedUser);

            //Verificando validez de las apuestas
            if ($validBet) {
                if ($bet->{'it_ticket_enc.bet_mode'} != 'PreLive') $validBet = false;
                elseif (in_array($bet->{'it_ticket_enc.bet_status'}, ['T', 'R', 'M'])) $validBet = false;
        ...
        for ($i = 0; $i <= count($validBets); $i += 1) {
            ...
            //Evitando suma de más apuestas a una condición de referidos que ya ha cumplido el valor objetivo
            if (!empty($logroReferidoId) && $betValue >= $goalValue) continue;
    ...
    try {
        ...
        for ($i = 0; $i <= $countCasinoBets; $i += 1) {
            ...
            //Verificando que usuario cumpla con condición de primer depósito
            if ($i < $countCasinoBets && !validateUserFirstDeposit((int)$casinoBets[$i]->{'logro_referido.usuid_referido'}, $lastReviewedUser)) continue;

            //Verificando que transacción no se haya contado en otra condición de referidos
...
//Finalizando proceso
    $sqlProcesoInterno2 = "UPDATE proceso_interno2 SET fecha_ultima='" . $fechaL1 . "' WHERE  tipo='" . $temporalType . "'";
