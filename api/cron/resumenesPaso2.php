...
";
  ...
  SELECT
    ...
    SUM(usuario_recarga.impuesto) impuesto
...
SELECT
  ...
  ORDER BY date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d');";
    ...
    $sqlRetiroUsuarioDiaExpiradasOTP = "
SELECT
    cuenta_cobro.usuario_id usuarioId,
    SUM(cuenta_cobro.valor) valor,
    date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d') fecha_crea,
  ...
  FROM cuenta_cobro
    ...
    WHERE date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND cuenta_cobro.estado ='Z' AND cuenta_cobro.mandante NOT IN (3,4,5,6,7,10,22,17,25)

  GROUP BY date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d'), cuenta_cobro.usuario_id, cuenta_cobro.version
  ORDER BY date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d');";

    $sqlRetiroUsuarioDiaExpiradasTiempo = "
SELECT
    cuenta_cobro.usuario_id usuarioId,
    SUM(cuenta_cobro.valor) valor,
    date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d') fecha_crea,
  ...
  FROM cuenta_cobro
    ...
    WHERE date_format(cuenta_cobro.fecha_accion, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND cuenta_cobro.estado ='W' AND cuenta_cobro.mandante NOT IN (3,4,5,6,7,10,22,17,25)
...
FROM it_transaccion
 ...
 INNER JOIN it_ticket_enc ON (it_ticket_enc.ticket_id = it_transaccion.ticket_id) 
    WHERE 
        ((((it_ticket_enc.eliminado = 'S' AND it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre) OR (it_ticket_enc.eliminado = 'N' )) AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "'))
        
        
       AND it_transaccion.tipo='BET'    AND usuario_perfil.mandante NOT IN (3,4,5,6,7,10,22,17,25)
GROUP BY it_ticket_enc.wallet,(it_transaccion.fecha_crea),it_transaccion.tipo, it_transaccion.usuario_id
ORDER BY it_ticket_enc.wallet,(it_transaccion.fecha_crea),it_transaccion.tipo,it_transaccion.usuario_id;";
...
ORDER BY it_ticket_enc.wallet,(it_ticket_enc.fecha_cierre),it_ticket_enc.usuario_id;";
    ...
    $sqlApuestasDeportivasUsuarioDiaIMPUESTOSapuesta = "
SELECT
  it_ticket_enc.usuario_id usuarioId, 
  SUM(it_ticket_enc.impuesto_apuesta) valor,
...
FROM it_ticket_enc
  INNER JOIN usuario_perfil ON (usuario_perfil.usuario_id = it_ticket_enc.usuario_id)
    WHERE (it_ticket_enc.fecha_crea) = '" . $fechaSoloDia . "'  AND impuesto_apuesta >0    AND usuario_perfil.mandante NOT IN (3,4,5,6,7,10,22,17,25)
GROUP BY it_ticket_enc.wallet,(it_ticket_enc.fecha_crea), it_ticket_enc.usuario_id
...
SELECT
  ...
  0 usumodif_id,
  'A' estado, 
  CASE WHEN transaccion_juego.tipo ='TAXBET' THEN '8' WHEN transaccion_juego.tipo ='TAXWIN' THEN '9'  ELSE NULL END tipo,
  COUNT(*) cantidad
      FROM transjuego_log      
      ...
      inner join time_dimension force index (time_dimension_timestampint_dbtimestamp_index)
        ...
        INNER JOIN transaccion_juego ON (transjuego_log.transjuego_id =transaccion_juego.transjuego_id)
             INNER JOIN usuario_mandante ON (transaccion_juego.usuario_id = usumandante_id)
    WHERE time_dimension.dbdate = '" . $fechaSoloDia . "' AND transjuego_log.tipo IN ('TAXBET','TAXWIN')  AND usuario_mandante.mandante NOT IN (3,4,5,6,7,10,22,17,25)
GROUP BY transaccion_juego.tipo,time_dimension.dbdate, transaccion_juego.usuario_id
ORDER BY time_dimension.dbdate,transaccion_juego.usuario_id;";
...
ORDER BY time_dimension.dbdate,transaccion_juego.producto_id,transaccion_juego.usuario_id,transaccion_juego.tipo;";
    ...
    $sqlDetalleImpuestoApuesCasinoDia = "
SELECT
  transaccion_juego.usuario_id usuarioId,
  transaccion_juego.producto_id producto_id,
  SUM(transjuego_log.valor) valor,
    ...
    if(true) {
        ...
        if(true) {
            ...
            foreach ($data as $datanum) {
                $sql = "INSERT INTO usuario_recarga_resumen (usuario_id, valor, fecha_crea, usucrea_id, usumodif_id, estado, cantidad,mediopago_id,puntoventa_id, impuesto)
              VALUES ('" . $datanum->{'usuario_recarga.usuarioId'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.fecha_crea'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'.usumodif_id'} . "','" . $datanum->{'.estado'} . "','" . $datanum->{'.cantidad'} . "','" . $datanum->{'.mediopago_id'} . "','" . $datanum->{'usuario_recarga.puntoventa_id'}. "','" . $datanum->{'.impuesto'} . "')";
            ...
            foreach ($data as $datanum) {
                ...
                $sql = "INSERT INTO usuario_recarga_resumen (usuario_id, valor, fecha_crea, usucrea_id, usumodif_id, estado, cantidad, impuesto)
              VALUES ('" . $datanum->{'.usuarioId'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.fecha_crea'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'.usumodif_id'} . "','" . $datanum->{'.estado'} . "','" . $datanum->{'.cantidad'}. "','" . $datanum->{'.impuesto'} . "')";
                $BonoInterno->execQuery($transaccion, $sql);
            }
            ...
            foreach ($data as $datanum) {
                $sql = "INSERT INTO usuario_recarga_resumen (usuario_id, valor, fecha_crea, usucrea_id, usumodif_id, estado, cantidad,mediopago_id, impuesto)
              VALUES ('" . $datanum->{'usuario_recarga.usuarioId'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.fecha_crea'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'.usumodif_id'} . "','" . $datanum->{'.estado'} . "','" . $datanum->{'.cantidad'} . "','" . $datanum->{'transaccion_producto.mediopago_id'}. "','" . $datanum->{'.impuesto'} . "')";
                $BonoInterno->execQuery($transaccion, $sql);
            }
            ...
            print_r($sqlRetiroUsuarioDiaExpiradasTiempo);

            $data = $BonoInterno->execQuery('', $sqlRetiroUsuarioDiaExpiradasTiempo);
            ...
            print_r($sqlRetiroUsuarioDiaExpiradasOTP);

            $data = $BonoInterno->execQuery('', $sqlRetiroUsuarioDiaExpiradasOTP);
        ...
        $data = $BonoInterno->execQuery('', $sqlApuestasDeportivasUsuarioDiaIMPUESTOSapuesta);
        ...
        foreach ($data as $datanum) {
            ...
            $sql = "INSERT INTO usuario_deporte_resumen (usuario_id, valor, fecha_crea, usucrea_id, usumodif_id, estado, tipo, cantidad)
              VALUES ('" . $datanum->{'it_transaccion.usuarioId'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.fecha_crea'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'it_ticket_enc.usumodif_id'} . "','" . $datanum->{'.estado'} . "','" . $datanum->{'.tipo'} . "','" . $datanum->{'.cantidad'} . "')";
            $BonoInterno->execQuery($transaccion, $sql);
        }
        ...
        $data = $BonoInterno->execQuery('', $sqlImpuestoApuestasCasinoDia);
        ...
        $log = "\r\n" . "-------------------------" . "\r\n";
        $log = $log . "ImpuestoApuestasCasinoDia: " . $fechaSoloDia . " - " . date('Y-m-d H:i:s');
        fwriteCustom('log_' . date("Y-m-d") . '.log',$log);
        print_r("PASO");
        ...
        $data = $BonoInterno->execQuery('', $sqlDetalleImpuestoApuesCasinoDia);

        foreach ($data as $datanum) {
            $sql = "INSERT INTO usucasino_detalle_resumen (usuario_id, producto_id, valor,valor_premios, fecha_crea, usucrea_id, usumodif_id, estado, tipo, cantidad)
              VALUES ('" . $datanum->{'transaccion_juego.usuarioId'} . "','" . $datanum->{'transaccion_juego.producto_id'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.valor_premios'} . "','" . $datanum->{'.fecha_crea'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'.usumodif_id'} . "','" . $datanum->{'.estado'} . "','" . $datanum->{'transjuego_log.tipo'} . "','" . $datanum->{'.cantidad'} . "')";
            $BonoInterno->execQuery($transaccion, $sql);
        }

        $log = "\r\n" . "-------------------------" . "\r\n";
        $log = $log . "DetalleImpuestoApuesCasinoDia: " . $fechaSoloDia . " - " . date('Y-m-d H:i:s');
        fwriteCustom('log_' . date("Y-m-d") . '.log',$log);
        print_r("PASO");
...
ON DUPLICATE KEY UPDATE usuario_saldo.saldo_recarga      = '" . $datanum->{'.saldo_recarga'} . "',
                         ...
                         usuario_saldo.saldo_impuestos_apuestas_casino      = '" . $datanum->{'.saldo_impuestos_apuestas_casino'} . "',
                         usuario_saldo.saldo_impuestos_depositos      = '" . $datanum->{'.saldo_impuestos_depositos'} . "'
