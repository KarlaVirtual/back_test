...
if ($pattern == 'F2BACK*' || $pattern == 'I2BACK*') {
...
// Definir el patrón
    if ($pattern == 'F10BACK*' || $pattern == 'F11BACK*' || $pattern == 'F12BACK*'
        ...
        $iterator = null;
        $count = 100; // Cantidad sugerida por iteración (no es exacta)
        $limit = 10000;
        $total = 0;
        ...
        while ($iterator !== 0 && $total < $limit) {
            ...
            $cantArrayKeysProcessed = array();
            foreach ($keys as $keyy => $key) {
                try {
                    syslog(LOG_WARNING,'cronRedisCRMKEY '.$key);
                    array_push($cantArrayKeysProcessed,$key);
                ...
                foreach ($activeProcesses as $key => $pid) {
                    ...
                    if ($res > 0) {
                        ...
                        if ((time() - $startTimes[$pid]) > $maxTime) {
                            echo "Matando proceso $pid por exceder 5 minutos\n";
            ...
            $redis->del($cantArrayKeysProcessed);
            syslog(LOG_WARNING,'cronRedisCRMKEYtotalFINALIZO '.json_encode($cantArrayKeysProcessed));
...
// Definir el patrón
    ...
    /** Nueva llave para envio de Inbox */
    if ($pattern == 'I2BACK*') {
