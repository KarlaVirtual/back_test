...
$message = "*CRON: (Inicio) * " . " ResumenesPaso4 - Fecha: " . date("Y-m-d H:i:s");
...
SELECT usuario.mandante,
       ...
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_final ELSE 0 END) +
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_premios_jackpot_casino ELSE 0 END) +  
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_premios_jackpot_deportivas ELSE 0 END) -
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_ventas_loteria ELSE 0 END) +
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_rollbacks_loteria ELSE 0 END) desfase,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_premios_jackpot_casino ELSE 0 END) saldo_premios_jackpot_casino,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_premios_jackpot_deportivas ELSE 0 END) saldo_premios_jackpot_deportivas,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_ventas_loteria ELSE 0 END) saldo_ventas_loteria,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_rollbacks_loteria ELSE 0 END) saldo_rollbacks_loteria,
       0                                                                                          cantidad_recargas,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_impuestos_apuestas_deportivas ELSE 0 END) saldo_impuestos_apuestas_deportivas,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_impuestos_apuestas_casino ELSE 0 END) saldo_impuestos_apuestas_casino,
       SUM(CASE WHEN usuario_perfil.perfil_id = 'USUONLINE' THEN saldo_impuestos_depositos ELSE 0 END) saldo_impuestos_depositos
...
where 1 = 1
  ...
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
  ...
  ";
    ...
    $sqlUsuarioSaldoResumen_PuntoVenta="SELECT usuario.mandante,
       ...
       SUM(saldo_apuestas_casino_vivo)                                                             saldo_apuestas_casino_vivo,
              ...
              SUM(saldo_inicial) - SUM(saldo_final) - SUM(saldo_recarga) + SUM(saldo_notaret_pagadas) - SUM(saldo_apuestas) + SUM(saldo_premios) - SUM(saldo_apuestas_casino) + SUM(saldo_premios_casino) - SUM(saldo_notaret_creadas) - SUM(saldo_notaret_eliminadas) + SUM(saldo_ajustes_entrada) - SUM(saldo_ajustes_salida) + SUM(saldo_notaret_pend) + SUM(saldo_premios_jackpot_casino) + SUM(saldo_premios_jackpot_deportivas) desfase,
...
where 1 = 1
  AND ((usuario_saldo.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ('PUNTOVENTA') and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo.fecha;";
    ...
    $sqlUsuarioSaldoResumen_Cajero="SELECT usuario.mandante,
       ...
       SUM(saldo_apuestas_casino_vivo)                                                             saldo_apuestas_casino_vivo,
              ...
              SUM(saldo_inicial) - SUM(saldo_final) - SUM(saldo_recarga) + SUM(saldo_notaret_pagadas) - SUM(saldo_apuestas) + SUM(saldo_premios) - SUM(saldo_apuestas_casino) + SUM(saldo_premios_casino) - SUM(saldo_notaret_creadas) - SUM(saldo_notaret_eliminadas) + SUM(saldo_ajustes_entrada) - SUM(saldo_ajustes_salida) + SUM(saldo_notaret_pend) + 
...
where 1 = 1
  AND ((usuario_saldo.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CAJERO') and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo.fecha;";
...
where 1 = 1
  AND ((usuario_saldo.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO') and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo.fecha;";
...
where 1 = 1
  AND ((usuario_saldo.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO2') and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo.fecha;";


    $sqlUsuarioSaldoResumen_Concesionario3="SELECT usuario.mandante,
       pais.pais_id,
       usuario_saldo.fecha,
       6,
       SUM(saldo_recarga)                                                                          saldo_recarga,
...
where 1 = 1
  AND ((usuario_saldo.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO3') and usuario.mandante not in (3,4,5,6,7,10,22,17,25)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo.fecha;";
        ...
        if(false) {
            ...
            foreach ($data as $datanum) {
                $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  ...
                                  saldo_bono_virtual_free_ganado, billetera_id, saldo_apuestas_casino_vivo, desfase, saldo_premios_jackpot_casino, 
                                  saldo_premios_jackpot_deportivas,saldo_ventas_loteria, saldo_rollbacks_loteria, cantidad_recargas, saldo_impuestos_apuestas_deportivas, saldo_impuestos_apuestas_casino,saldo_impuestos_depositos)
              VALUES (
...
print_r($sqlUsuarioSaldoResumen_PuntoVenta);
        $data = $BonoInterno->execQuery('', $sqlUsuarioSaldoResumen_PuntoVenta);
        foreach ($data as $datanum) {
            $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  saldo_notaret_pagadas, saldo_notaret_pend, saldo_ajustes_entrada,
                                  saldo_ajustes_salida, saldo_inicial, saldo_final, saldo_bono, saldo_creditos_inicial,
                                  ...
                                  saldo_notaret_eliminadas, saldo_bono_free_ganado, saldo_bono_casino_free_ganado,
                                  saldo_bono_casino_vivo, saldo_bono_casino_vivo_free_ganado, saldo_bono_virtual,
                                  saldo_bono_virtual_free_ganado, billetera_id, saldo_apuestas_casino_vivo, desfase, saldo_premios_jackpot_casino, saldo_premios_jackpot_deportivas, cantidad_recargas, saldo_impuestos_apuestas_deportivas, saldo_impuestos_apuestas_casino,saldo_impuestos_depositos)
              VALUES (
                                         '" . $datanum->{'usuario.mandante'} . "',
                                         '" . $datanum->{'pais.pais_id'} . "',
                           '" . $datanum->{'usuario_saldo.fecha'} . "',
        ...
        $data = $BonoInterno->execQuery('', $sqlUsuarioSaldoResumen_Cajero);
        foreach ($data as $datanum) {
            $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  saldo_notaret_pagadas, saldo_notaret_pend, saldo_ajustes_entrada,
                                  saldo_ajustes_salida, saldo_inicial, saldo_final, saldo_bono, saldo_creditos_inicial,
        ...
        $data = $BonoInterno->execQuery('', $sqlUsuarioSaldoResumen_Concesionario);
        foreach ($data as $datanum) {
            $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  saldo_notaret_pagadas, saldo_notaret_pend, saldo_ajustes_entrada,
                                  saldo_ajustes_salida, saldo_inicial, saldo_final, saldo_bono, saldo_creditos_inicial,
        ...
        $data = $BonoInterno->execQuery('', $sqlUsuarioSaldoResumen_Concesionario2);
        foreach ($data as $datanum) {
            $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  saldo_notaret_pagadas, saldo_notaret_pend, saldo_ajustes_entrada,
                                  saldo_ajustes_salida, saldo_inicial, saldo_final, saldo_bono, saldo_creditos_inicial,
        ...
        $data = $BonoInterno->execQuery('', $sqlUsuarioSaldoResumen_Concesionario3);
        foreach ($data as $datanum) {
            $sql = "INSERT INTO bodega_usuario_saldo (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
                                  saldo_notaret_pagadas, saldo_notaret_pend, saldo_ajustes_entrada,
                                  saldo_ajustes_salida, saldo_inicial, saldo_final, saldo_bono, saldo_creditos_inicial,
        ...
        //$procesoInterno=$BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesPaso4','".date("Y-m-d 00:00:00")."','0');");
    ...
    $message = "*CRON: (Fin) * " . " ResumenesPaso4 - Fecha: " . date("Y-m-d H:i:s");
    ...
    $message = "*CRON: (ERROR) * " . " ResumenesPaso4 - Fecha: " . date("Y-m-d H:i:s");
