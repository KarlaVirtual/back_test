...
/**
 * Clase 'CronJobActualizacionNivelesUsuarios'
 *
 *
 *
 *
 ...
 *
 */
class CronJobActualizacionNivelesUsuarios
{
    private $SlackVS;
    ...
    public function execute()
    {
        ...
        $hour = date('H');
        if ((intval($hour) >= 21 && intval($hour) <= 24) || (intval($hour) >= 0 && intval($hour) < 6)) {
            exit();
        }
        $filename = __DIR__ . '/cronActualizacionNivelesUsuarios';
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                    ...
                    if ($saldo_recarga > 0 && (($saldo_notaret_pagadas / $saldo_recarga) > 0.85)) {
                        $esSeguro = false;
                    }

                    //Usuario tiene que jugar un 1.6 el valor depositado
                    if ($saldo_recarga > 0 && (($saldo_apuestas + $saldo_apuestas_casino + $saldo_apuestas_casino_vivo) / $saldo_recarga) < 1.6) {
                        $esSeguro = false;
                    }


                    //GGR/Depositos al 16% (0.16)
                    if ($saldo_recarga > 0 && ((($saldo_apuestas + $saldo_apuestas_casino) - ($saldo_premios + $saldo_premios_casino)) / $saldo_recarga) < 0.16) {
                        $esSeguro = false;
                    }
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                    ...
                    //GGR/Depositos al 16% (0.16)
                    if ((($saldo_apuestas + $saldo_apuestas_casino) - ($saldo_premios + $saldo_premios_casino)) < ($saldo_recarga * 0.16)) {
                        $esSeguro = false;
                    }
...
from cuenta_cobro
         inner join usuario on usuario.usuario_id = cuenta_cobro.usuario_id
where usuario.mandante = 23 
  and cuenta_cobro.fecha_crea >= '" . $fechaL1 . "'
  and cuenta_cobro.fecha_crea < '" . $fechaL2 . "'
and cuenta_cobro.valor <10
and (cuenta_cobro.estado='A' OR cuenta_cobro.estado='M')
";
            $data2 = $BonoInterno->execQuery('', $sqlProcesoInterno2);
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                    ...
                    $esSeguro = true;
                    if ($CuentaCobro->getMediopagoId() != '' && $CuentaCobro->getValor() < 1000) {
                        $esSeguro = true;
...
from cuenta_cobro
         inner join usuario on usuario.usuario_id = cuenta_cobro.usuario_id
where usuario.mandante = 27 and usuario.pais_id=68
  and cuenta_cobro.fecha_crea >= '" . $fechaL1 . "'
  and cuenta_cobro.fecha_crea < '" . $fechaL2 . "'
and cuenta_cobro.valor >=10 and cuenta_cobro.valor <=100
and (cuenta_cobro.estado='A' OR cuenta_cobro.estado='M')
";
                $data2 = $BonoInterno->execQuery('', $sqlProcesoInterno2);
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                        ...
                        //Usuario tiene que jugar un 1.6 el valor depositado
                        if ($saldo_recarga > 0 && (($saldo_apuestas + $saldo_apuestas_casino + $saldo_apuestas_casino_vivo) / $saldo_recarga) < 1.8) {
                            $esSeguro = false;
                        }


                        //GGR/Depositos al 16% (0.16)
                        if ($saldo_recarga > 0 && ((($saldo_apuestas + $saldo_apuestas_casino) - ($saldo_premios + $saldo_premios_casino)) / $saldo_recarga) < 0.13) {
                            $esSeguro = false;
                        }

                        //Usuario no haya retirado menos del 90% de lo depositado
                        if ($saldo_recarga > 0 && (($saldo_notaret_pagadas) > 600)) {
                            $esSeguro = false;
                        }
...
where usuario.mandante = 27 and usuario.pais_id=68
  and cuenta_cobro.fecha_crea >= '" . $fechaL1 . "'
  and cuenta_cobro.fecha_crea < '" . $fechaL2 . "'
and cuenta_cobro.valor >=50 and cuenta_cobro.valor <=600
and (cuenta_cobro.estado='A' OR cuenta_cobro.estado='M')
";
                $data2 = $BonoInterno->execQuery('', $sqlProcesoInterno2);
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                        ...
                        //Usuario no haya retirado menos del 90% de lo depositado
                        if ($saldo_recarga > 0 && (($saldo_notaret_pagadas) > 100)) {
                            $esSeguro = false;
                        }
...
from cuenta_cobro
         inner join usuario on usuario.usuario_id = cuenta_cobro.usuario_id
where usuario.mandante = 21 and usuario.pais_id=243
  and cuenta_cobro.fecha_crea >= '" . $fechaL1 . "'
  and cuenta_cobro.fecha_crea < '" . $fechaL2 . "'
and cuenta_cobro.valor >=10 and cuenta_cobro.valor <=250
and (cuenta_cobro.estado='A' OR cuenta_cobro.estado='M')
";
                $data2 = $BonoInterno->execQuery('', $sqlProcesoInterno2);
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                        ...
                        //Usuario no haya retirado menos del 90% de lo depositado
                        if ($saldo_recarga > 0 && (($saldo_notaret_pagadas / $saldo_recarga) > 0.80)) {
                            $esSeguro = false;
                        }

                        //Usuario tiene que jugar un 1.6 el valor depositado
                        if ($saldo_recarga > 0 && (($saldo_apuestas + $saldo_apuestas_casino + $saldo_apuestas_casino_vivo) / $saldo_recarga) < 2) {
                            $esSeguro = false;
                        }


                        //GGR/Depositos al 16% (0.16)
                        if ($saldo_recarga > 0 && ((($saldo_apuestas + $saldo_apuestas_casino) - ($saldo_premios + $saldo_premios_casino)) / $saldo_recarga) < 0.1999) {
                            $esSeguro = false;
                        }
...
from cuenta_cobro
         inner join usuario on usuario.usuario_id = cuenta_cobro.usuario_id
where usuario.mandante = 21 and usuario.pais_id=232
  and cuenta_cobro.fecha_crea >= '" . $fechaL1 . "'
  and cuenta_cobro.fecha_crea < '" . $fechaL2 . "'
...
where fecha_pago  LIKE '" . date('Y-m-d') . "%' and usuario_id = '" . $Usuario->usuarioId . "'";
                        ...
                        //Usuario no haya retirado menos del 90% de lo depositado
                        if ($saldo_recarga > 0 && (($saldo_notaret_pagadas / $saldo_recarga) > 0.8)) {
                            $esSeguro = false;
                        }
