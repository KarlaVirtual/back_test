...
class CronJobSorteosStickers
{
    ...
    {
        $filenameGeneral = __DIR__ . '/lastrunCronJobSorteosStickers';
        $datefilename = date("Y-m-d H:i:s", filemtime($filenameGeneral));
        $message = "*CRON: (cronSorteos) * " . " - Fecha: " . date("Y-m-d H:i:s");

        if ($datefilename <= date("Y-m-d H:i:s", strtotime('-15 minute'))) {
            unlink($filenameGeneral);
        }

        if (file_exists($filenameGeneral)) {
            throw new Exception("There is a process currently running", "1");
            exit();
        }
        file_put_contents($filenameGeneral, 'RUN');
        ...
        $maxConcurrentProcesses = 100; // Máximo de 10 procesos a la vez
...
";
                    ...
                    $sqlProcesoInterno2 = "
UPDATE proceso_interno2 SET fecha_ultima='" . $fechaL1 . "' WHERE  tipo='SORTEO' AND fecha_ultima < '" . $fechaL1 . "';
";
                                ...
                                if ($ActivacionOtros) {
                                    ...
                                    VALUES ('{$datanum->{"usuario_mandante.usuario_mandante"}}','SORTEO','{$value->{"sorteo_interno.sorteo_id"}}','CASINO','{$datanum->{"transjuego_log.transjuegolog_id"}}','MINBETPRICE2CASINO')";
                                        ...
                                        VALUES ('{$datanum->{"usuario_mandante.usuario_mandante"}}','SORTEO','{$value->{"sorteo_interno.sorteo_id"}}','CASINO','{$datanum->{"transjuego_log.transjuegolog_id"}}','NEEDSUSCRIBE')";
                                                ...
                                                $UsuarioConfiguracion = $UsuarioConfiguracion->getUserBondABuser($datanum->{"usuario_mandante.usuario_mandante"});
                                                if (!empty($UsuarioConfiguracion->usuconfigId)) {
                                                    $cumpleCondicion = false;
                                                    $BonoInterno = new BonoInterno();
                                                    $sqlLog = "INSERT INTO log_rechazo_STB (usuario_id,tipo,tipo_id,transaccion,transaccion_id,descripcion)
                                        VALUES ('{$datanum->{"usuario_mandante.usuario_mandante"}}','SORTEO','{$value->{"sorteo_interno.sorteo_id"}}','CASINO','{$datanum->{"transjuego_log.transjuegolog_id"}}','Contingencia activa: Abusador de bonos')";
                                                    //$BonoInterno->execQuery($transaccion, $sqlLog);
                                                }
                                                ...
                                                if ($cumpleCondicion) {
                                                    ...
                                                    if ($pegatinas == 1) {
                                                        ...
                                                        $repiteSorteo = $BonoInterno->execQuery($transaccion, $sqlRepiteSorteo);

                                                        $sqlSorteo = "CALL insert_sorteos({$minBetPrice},{$datanum->{"transjuego_log.valor"}},{$value->{"sorteo_interno.sorteo_id"}},{$datanum->{"transaccion_juego.usuario_id"}}, '1',{$datanum->{"usuario_mandante.mandante"}})";
                                                        $BonoInterno->execQuery($transaccion, $sqlSorteo);

                                                        if (count($repiteSorteo) == 0) {
                                                            $result = $BonoInterno->execQuery($transaccion, "SELECT LAST_INSERT_ID() AS ususorteo_id;");
                                                            $idUsuSorteo = $result[0]->{'.ususorteo_id'};
                                                        } else {
                                                            $idUsuSorteo = $repiteSorteo[0]->{"a.preususorteo_id"};
                                                        }
                                        ...
                                        VALUES ('{$datanum->{"usuario_mandante.usuario_mandante"}}','SORTEO','{$value->{"sorteo_interno.sorteo_id"}}','CASINO','{$datanum->{"transjuego_log.transjuegolog_id"}}','Contingencia activa: Abusador de bonos')";
                                                ...
                                                if ($cumpleCondicion) {
                                                    ...
                                                    if ($pegatinas == 1) {
                                                        ...
                                                        $sqlSorteo = "CALL insert_sorteos({$minBetPrice},$valorTicket,{$value->{"sorteo_interno.sorteo_id"}},{$datanum->{"usuario_mandante.usumandante_id"}}, '3',{$datanum->{"usuario_mandante.mandante"}})";
                                                        $BonoInterno->execQuery($transaccion, $sqlSorteo);

                                                        if (count($repiteSorteo) == 0) {
                                                            $result = $BonoInterno->execQuery($transaccion, "SELECT LAST_INSERT_ID() AS ususorteo_id;");
                                                            $ususorteoId = $result[0]->{'.ususorteo_id'};
                                                        } else {
