...
// array_push($rules, array("field" => "it_ticket_enc.usuario_id", "data" => "$ClientId", "op" => "eq"));
            ...
            foreach ($tickets->data as $key => $value) {
                ...
                if (((floatval($value->{'it_ticket_enc.vlr_premio'}) / floatval($value->{'it_ticket_enc.vlr_apuesta'})) * 100) >= 85 && ((floatval($value->{'it_ticket_enc.vlr_premio'}) / floatval($value->{'it_ticket_enc.vlr_apuesta'})) * 100) < 100
