...
/**
 * Clase 'CronJobResumenesComisiones6AM'
 *
 *
 *
 *
 ...
 *
 */
class CronJobResumenesComisiones6AM
{
...
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0  
...
FROM it_ticket_enc
       ...
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND it_ticket_enc.beneficiario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
UNION
         ...
         WHERE it_transaccion.fecha_crea ='" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0  
...
UNION
         ...
         FROM bono_log
                  ...
                  INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND
                                               ...
                                               concesionario.estado = 'A')
         WHERE date_format(bono_log.fecha_crea, '%Y-%m-%d') ='" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)    and bono_log.tipo IN ('F','D','ND') and bono_log.estado='L'
           AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
FROM (
         ...
         WHERE it_ticket_enc.beneficiario_id != 0 AND mandante_detalle.valor = 2
           AND it_ticket_enc.tipo_beneficiario = 2
           AND concesionario.usuhijo_id != 0
           and it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND maxima_comision >= 0
         ...
         FROM bono_log
                  ...
                  INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND
                                               registro.afiliador_id = concesionario.usuhijo_id AND
                                               concesionario.estado = 'A')
         WHERE date_format(bono_log.fecha_crea, '%Y-%m-%d') ='" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   AND usuario.maxima_comision >=0   and bono_log.tipo IN ('F','D','ND') and bono_log.estado='L'
...
FROM (
         ...
         WHERE it_ticket_enc.beneficiario_id != 0 AND mandante_detalle.valor = 2
           AND it_ticket_enc.tipo_beneficiario = 2
           and it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  usuario.maxima_comision >=0 
           AND usuario_perfil.perfil_id = 'USUONLINE'
           AND concesionario.usupadre_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
...
FROM (
         ...
         WHERE it_ticket_enc.beneficiario_id != 0 AND mandante_detalle.valor = 2
           AND it_ticket_enc.tipo_beneficiario = 2
           and it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0  
           AND usuario_perfil.perfil_id = 'USUONLINE'
           AND concesionario.usupadre2_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
         UNION
...
FROM (
         ...
         WHERE  concesionario.usuhijo_id != 0
           and usuario_casino_resumen.fecha_crea LIKE '%" . $fechaSoloDia . "%' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
           AND usuario_perfil.perfil_id = 'USUONLINE'
         ...
         FROM bono_log
                  ...
                  INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND
                                               registro.afiliador_id = concesionario.usuhijo_id AND
                                               concesionario.estado = 'A')
         WHERE date_format(bono_log.fecha_crea, '%Y-%m-%d') ='" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)    and bono_log.tipo IN ('DC', 'DL', 'DV', 'NC', 'NL', 'NV') and bono_log.estado='L'
           AND registro.afiliador_id != 0
           AND concesionario.usuhijo_id !=0
         GROUP BY date_format(bono_log.fecha_crea, '%Y-%m-%d'), usuario.usuario_id
...
FROM (
         ...
         WHERE  usuario_casino_resumen.fecha_crea LIKE '%" . $fechaSoloDia . "%' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
           AND usuario_perfil.perfil_id = 'USUONLINE'
           AND concesionario.usupadre_id != 0
         GROUP BY usuario_casino_resumen.fecha_crea, usuario_mandante.usuario_mandante
...
WHERE  date_format(usuario_recarga.fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
    AND concesionario.usuhijo_id !=0
...
FROM it_ticket_enc
       ...
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND it_ticket_enc.beneficiario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  mandante_detalle.valor = 2
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND it_ticket_enc.beneficiario_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND maxima_comision >= 0 AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL))
...
FROM (
         ...
         WHERE it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  mandante_detalle.valor = 2
           AND it_ticket_enc.beneficiario_id != 0
           AND it_ticket_enc.tipo_beneficiario = 1
           AND concesionario.usuhijo_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
...
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  concesionario.usuhijo_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  concesionario.usupadre_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  concesionario.usupadre2_id !=0 AND it_ticket_enc.bet_status != 'A'
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  concesionario.usupadre3_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'SPORTPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N' AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  concesionario.usupadre4_id !=0
GROUP BY it_ticket_enc.ticket_id, usuario.usuario_id
ORDER BY fecha, usuario
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  it_ticket_enc.eliminado='N'  AND ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  (mandante_detalle.valor = 1 OR (it_ticket_enc.premiado = 'N' AND it_ticket_enc.estado = 'I')) ) OR (it_ticket_enc.fecha_pago = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  (mandante_detalle.valor IS NULL OR mandante_detalle.valor = 0) )) AND concesionario.usuhijo_id !=0 AND it_ticket_enc.bet_status != 'A'
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,SUM(-vlr_apuesta) apuestas, 0 premios, 0 bonos,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ( (it_ticket_enc.eliminado='S'  AND it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.fecha_crea != it_ticket_enc.fecha_cierre ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios, 0 bonos,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre_id,usuario.usuario_id usuario,SUM(-vlr_apuesta) apuestas, 0 premios, 0 bonos,concesionario.porcenpadre1,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre2_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre2_id,usuario.usuario_id usuario,SUM( -vlr_apuesta ) apuestas, 0 premios, 0 bonos,concesionario.porcenpadre2,it_ticket_enc.fecha_cierre  fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre3_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,SUM( -vlr_apuesta ) apuestas, 0 premios, 0 bonos,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usuhijo_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usuhijo_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios, 0 bonos,concesionario.porcenhijo,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre2_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
...
FROM it_ticket_enc
       ...
       LEFT OUTER JOIN clasificador clasificador2
                                  ...
                                  LEFT OUTER JOIN mandante_detalle ON (clasificador2.clasificador_id = mandante_detalle.tipo AND
                                                       mandante_detalle.pais_id = usuario.pais_id)
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE   ((it_ticket_enc.fecha_cierre = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  it_ticket_enc.eliminado='N' ) ) AND (mandante_detalle.valor = 1 OR (mandante_detalle.valor = 0 OR mandante_detalle.valor IS NULL)) AND concesionario.usupadre3_id !=0
GROUP BY fecha, usuario.usuario_id
UNION
SELECT clasificador.clasificador_id,concesionario.concesionario_id,concesionario.usupadre3_id,usuario.usuario_id usuario,0 apuestas,SUM(vlr_premio)  premios, 0 bonos,concesionario.porcenpadre3,it_ticket_enc.fecha_cierre fecha,0 usucrea_id,0 usumodif_id,'' externo_id
FROM it_ticket_enc
...
FROM (
         ...
         WHERE
           it_transaccion.fecha_crea = '" . $fechaSoloDia . "' AND it_ticket_enc.mandante  IN (3,4,5,6,7,10,22,25)     AND usuario.maxima_comision >=0    AND  mandante_detalle.valor = 2
           AND concesionario.usuhijo_id != 0
         GROUP BY it_transaccion.fecha_crea, it_transaccion.usuario_id
     ) a
GROUP BY fecha, usuario
...
FROM usuario_recarga
       ...
       LEFT OUTER JOIN clasificador ON (clasificador.abreviado =  'DEPOSITOPV')
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  date_format(usuario_recarga.fecha_elimina, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
...
FROM cuenta_cobro
       ...
       INNER JOIN concesionario ON (concesionario.prodinterno_id = clasificador.clasificador_id AND usuario.puntoventa_id = concesionario.usuhijo_id AND concesionario.estado='A')
WHERE  date_format(cuenta_cobro.fecha_pago, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
    AND concesionario.usuhijo_id !=0
...
WHERE  date_format(usuario_comision.fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)   
...
WHERE mandante_detalle.estado='A' AND date_format(usucomision_resumen.fecha_crea, '%Y-%m-%d') = '" . $fechaSoloDia . "' AND usuario.mandante  IN (3,4,5,6,7,10,22,25)    AND mandante_detalle.valor='1'";
...
//$BonoInterno->execQuery($transaccion, $strEliminado);
            ...
            fwriteCustom('log_' . date("Y-m-d") . '.log', $log);
            print_r(' sqlSPORTAFFDia1CON_IT_TICKETENC ');
            print_r($sqlSPORTAFFDia1CON_IT_TICKETENC);
            print_r(' sqlSPORTAFFDia1CON_IT_TICKETENC2');
            ...
            print_r(' sqlSPORTNGRAFFDia1 ');
            print_r($sqlSPORTNGRAFFDia1);
            print_r(' sqlSPORTNGRAFFDia1 2 ');
            ...
            print_r($sqlSPORTNGRAFFDia1CON_IT_TICKETENC);
            ...
            print_r("sqlSPORTPVIPDia1_CON_IT_TICKET_ENC1 ");
            print_r($sqlSPORTPVIPDia1_CON_IT_TICKET_ENC);
            print_r(" sqlSPORTPVIPDia1_CON_IT_TICKET_ENC2 ");
            ...
            print_r($sqlSPORTPVDia1_CON_IT_TICKET_ENC);
            ...
            print_r($sqlBETSPORTPVDia1_IT_TICKET_ENC);
    ...
    VALUES ('" . $datanum->{'.clasificador_id'} . "','" . $datanum->{'.concesionario_id'} . "','" . $datanum->{'.usuhijo_id'} . "','" . $datanum->{'.usuario'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.comision'} . "','" . $datanum->{'.fecha'} . "','" . $datanum->{'.usucrea_id'} . "','" . $datanum->{'.usumodif_id'} . "','" . $datanum->{'.cuenta_id'} . "')";
            ...
            fwriteCustom('log_' . date("Y-m-d") . '.log', $log);
            $procesoInterno = $BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesComisiones-6AM','" . date("Y-m-d 00:00:00") . "','0');");
...
//sleep(60);
    ...
    VALUES ('" . $datanum->{'usuario_comision.tipo'} . "','" . $datanum->{'.fecha'} . "','" . $datanum->{'usuario_comision.usuario_id'} . "','" . $datanum->{'usuario_comision.usuarioref_id'} . "','" . $datanum->{'.valor'} . "','" . $datanum->{'.comision'} . "','" . $datanum->{'.valor1'} . "','" . $datanum->{'.valor2'} . "','" . $datanum->{'.valor3'} . "','" . $datanum->{'.valor4'} . "','" . $datanum->{'.valor5'} . "','" . $datanum->{'.valor6'} . "')";
            ...
            $procesoInterno = $BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesComisiones2-6AM','" . date("Y-m-d 00:00:00") . "','0');");
...
";
        ...
        $procesoInterno = $BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesComisionesTotales-6AM','" . date("Y-m-d 00:00:00") . "','0');");
