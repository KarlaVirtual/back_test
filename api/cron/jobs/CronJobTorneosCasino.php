...
class CronJobTorneosCasino
{
    ...
    public function execute()
    {
        ...
        $sqlProcesoInterno2 = "SELECT * FROM proceso_interno2 WHERE tipo='TORNEO'";
        ...
        if ($_ENV['debug']) {
            ...
            $sqlProcesoInterno2 = "UPDATE proceso_interno2 SET fecha_ultima='" . $fechaL2 . "' WHERE  tipo='TORNEO';";
        ...
        if (true) {
            ...
            foreach ($Tipos as $tipo) {
                ...
                $torneos = $TorneoInterno->getTorneosCustom(" torneo_interno.* ", "torneo_interno.orden", "DESC", $SkeepRows, $MaxRows, $jsonfiltro, TRUE);
                    ...
                    try {
                        if ($ActivacionTorneos) {
                            ...
                            if (oldCount($condIdCategory) > 0 && !in_array('ALL', $condIdCategory)) {
                                ...
                                $sqlApuestasDeportivasUsuarioDiaCierre = "SELECT producto.producto_id FROM producto
                                    ...
                                    foreach ($dataUsuario as $key4 => $datanum) {
                                        ...
                                        try {
                                            ...
                                            if (!empty($UsuarioConfiguracion->usuconfigId)){
                                                ...
                                                $BonoInterno = new BonoInterno();
                                                $sqlLog = "INSERT INTO log_rechazo_STB (usuario_id,tipo,tipo_id,transaccion,transaccion_id,descripcion)
                                                VALUES ('{$datanum->{"usuario_mandante.usuario_mandante"}}','TORNEO','{$value->{"torneo_interno.torneo_id"}}','SPORTBOOK','{$datanum->{"transjuego_log.transjuegolog_id"}}','Contingencia activa: Abusador de bonos')";
                                                //$BonoInterno->execQuery($transaccion, $sqlLog);
                                            }
     ...
     *
     * @param mixed $transaccion Objeto o identificador de la transacción que se utilizará para ejecutar la consulta.
     * @param string $sql Consulta SQL que se desea ejecutar.
     * @return mixed Resultado de la consulta ejecutada, formateada como un objeto.
     */
