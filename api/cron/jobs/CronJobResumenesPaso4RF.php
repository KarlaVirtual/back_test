...
class CronJobResumenesPaso4RF
{
    ...
    public function execute()
    {
        ...
        $message = "*CRON: (Inicio) * " . " ResumenesPaso4RF - Fecha: " . date("Y-m-d H:i:s");
...
SELECT usuario.mandante,
       ...
       0                                                                                          cantidad_recargas
FROM " . $nameTable . " usuario_saldo_rf
         INNER JOIN usuario ON (usuario_saldo_rf.usuario_id = usuario.usuario_id)
         INNER JOIN pais ON (usuario.pais_id = pais.pais_id)
         inner join usuario_perfil ON (usuario.usuario_id = usuario_perfil.usuario_id)

where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "'
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;
...
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ('PUNTOVENTA') and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
...
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CAJERO') and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
...
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO') and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
...
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO2') and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
...
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
  AND ((usuario_perfil.perfil_id)) IN ( 'CONCESIONARIO3') and usuario.mandante not in (3,4,5,6,7,10,22)
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
            ...
            if (true) {
                ...
                if (false) {
                    ...
                    foreach ($data as $datanum) {
                        $sql = "INSERT INTO bodega_usuario_saldo_rf (mandante, pais_id, fecha, tipo, saldo_recarga, saldo_apuestas, saldo_premios,
              ...
              VALUES (
                           ...
                           '" . $datanum->{'usuario_saldo_rf.fecha'} . "',
                           '" . "1" . "',
                           ...
                           '" . $datanum->{'usuario_saldo_rf.billetera_id'} . "',
              ...
              VALUES (
                ...
                // $procesoInterno=$BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesPaso4RF','".date("Y-m-d 00:00:00")."','0');");
            ...
            $message = "*CRON: (Fin) * " . " ResumenesPaso4RF - Fecha: " . date("Y-m-d H:i:s");
            ...
            $message = "*CRON: (ERROR) * " . " ResumenesPaso4RF - Fecha: " . date("Y-m-d H:i:s");
