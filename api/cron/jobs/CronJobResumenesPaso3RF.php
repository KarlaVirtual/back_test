...
class CronJobResumenesPaso3RF
{
    ...
    public function execute()
    {
        ...
        $message = "*CRON: (Inicio) * " . " ResumenesPaso3RF - Fecha: " . date("Y-m-d H:i:s");
...
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha,usuario_saldo_rf.billetera_id;
...
FROM " . $nameTable . " usuario_saldo_rf
         ...
         LEFT OUTER JOIN (select count(*)      usuarios_recargas,
                                 ...
                                 usuario_recarga_resumen_rf.fecha_crea,
                                 usuario_id
                          from usuario_recarga_resumen_rf
                                   inner join day_dimension force index (day_dimension_dbtimestamp_timestrc_index)
                                              on (day_dimension.dbtimestamp = usuario_recarga_resumen_rf.fecha_crea) where puntoventa_id!=0
                          GROUP BY usuario_id, fecha_crea) usuario_recargas ON (usuario_recargas.fecha_crea = fecha and
                                                                                usuario_saldo_rf.usuario_id = usuario_recargas.usuario_id)
where 1 = 1
  AND ((usuario_saldo_rf.fecha)) >= '" . $fechaSoloDia . "' 
  AND ((usuario_saldo_rf.fecha)) <= '" . $fechaSoloDia . "' 
...
GROUP BY usuario.mandante, usuario.pais_id, usuario_saldo_rf.fecha;";
            ...
            if (true) {
              ...
              VALUES (
                ...
                // $procesoInterno=$BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesPaso3RF','".date("Y-m-d 00:00:00")."','0');");
            ...
            $message = "*CRON: (Fin) * " . " ResumenesPaso3RF - Fecha: " . date("Y-m-d H:i:s");
            ...
            $message = "*CRON: (ERROR) * " . " ResumenesPaso3RF - Fecha: " . date("Y-m-d H:i:s");
