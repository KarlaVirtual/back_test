...
/**
 * Clase 'CronJobSorteosStickersSportsbook'
 *
 *
 *
 *
 ...
 *
 */
class CronJobSorteosStickersSportsbook
{
    ...
    public function execute()
    {
        ...
        $filename = __DIR__ . '/lastrunCronJobSorteosStickersSportsbook';
...
";
        ...
        $fechaL1 = date('Y-m-d H:i:00', strtotime($line . '+0 minute'));
        $fechaL2 = date('Y-m-d H:i:00', strtotime($line . '+10 minute'));
...
//array_push($rules, array("field" => "it_ticket_enc.ticket_id", "data" => implode(',',$ArrayTransaction), "op" => "ni"));
            ...
            if ($fechaL2 != "") {
                ...
                if ($TypeBet == 2) {
                    array_push($rules, array("field" => "(it_ticket_enc.fecha_cierre_time)", "data" => "$fechaL2", "op" => "lt"));
                    $daydimensionFecha = 2;
                }
            }
                    ...
                    if ($dataUsuario !== NULL && $dataUsuario !== "" && $dataUsuario[0] !== NULL) {
                        ...
                        foreach ($dataUsuario as $key4 => $datanum) {
                            ...
                            if (!empty($UsuarioConfiguracion->usuconfigId)){
                                ...
                                $BonoInterno = new BonoInterno();
                                $sqlLog = "INSERT INTO log_rechazo_STB (usuario_id,tipo,tipo_id,transaccion,transaccion_id,descripcion)
                                VALUES ('{$datanum->{"usuario.usuario_id"}}','SORTEO','{$value->{"sorteo_interno.sorteo_id"}}','SPORTBOOK','{$datanum->{"it_ticket_enc.ticket_id"}}','Contingencia activa: Abusador de bonos')";
                                //$BonoInterno->execQuery($transaccion, $sqlLog);
                            }
