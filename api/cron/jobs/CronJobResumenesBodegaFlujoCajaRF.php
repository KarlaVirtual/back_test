...
/**
 * Clase 'CronJobResumenesBodegaFlujoCajaRF'
 *
 *
 *
 *
 ...
 *
 */
class CronJobResumenesBodegaFlujoCajaRF
{
    ...
    public function execute()
    {
        ...
        $message = "*CRON: (Inicio) * " . " resumenesBodegaFlujoCajaRF - Fecha: " . date("Y-m-d H:i:s");
...
//BETWEEN '".$fecha1."' AND '".$fecha2."'
            ...
            $strEliminado = "DELETE FROM bodega_flujo_caja_rf WHERE date_format(fecha, '%Y-%m-%d') = '" . $fechaSoloDia . "';";
...
from (select *
      ...
      from (select d.nombre                                                                         punto_venta,
                   ...
                   sum(case
                           when x.traslado = 'N' and ( x.ticket_id = '' or x.ticket_id is null) AND x.cupolog_id != '0' and x.recarga_id = '0' and x.tipomov_id = 'E'
                               then x.valor
            ...
            where 1 = 1
              and (((x.fecha_crea))) >=
                  ('" . $fechaSoloDia . "')
              and (((x.fecha_crea))) <=
                  ('" . $fechaSoloDia . "') and d.mandante  in (8)
      ...
      where z.fecha_crea <= ('" . $fechaSoloDia . "')
        ...
        and date(z.fecha_maxpago) >= date(now()) and d.mandante  in (8)
...
order by y.usuario_id, y.fecha_crea";
            ...
            foreach ($dataSaldoInicial as $datanum) {
                ...
                print_r($datanum);
                $sql = "INSERT INTO bodega_flujo_caja_rf (usuario_id, fecha, pais_id, mandante, concesionario_id, cant_tickets, valor_entrada_efectivo, valor_entrada_bono, valor_entrada_recarga,recargas_anuladas,valor_entrada_recarga_agentes, valor_entrada_traslado, valor_salida_traslado, valor_salida_notaret, valor_entrada, valor_salida, valor_salida_efectivo, premios_pend, impuestos, apuestas_void, premios_void) 
       ...
       ";
            ...
            $procesoInterno = $BonoInterno->execQuery($transaccion, "INSERT INTO procesos_internos ( tipo, fecha_crea, usucrea_id) VALUES ('ResumenesBodegaFlujoCajaRF','" . date("Y-m-d 00:00:00") . "','0');");
            ...
            $message = "*CRON: (Fin) * " . " BodegaFlujoCajaRF - Fecha: " . date("Y-m-d H:i:s");
            ...
            $message = "*CRON: (ERROR) * " . $e->getLine() . " BodegaFlujoCajaRF - Fecha: " . date("Y-m-d H:i:s");
