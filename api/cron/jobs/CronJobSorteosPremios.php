...
class CronJobSorteosPremios
{
    ...
    public function execute()
    {
        ...
        $filename = __DIR__ . '/lastrunCronJobSorteosPremios';
                ...
                try {
                    ...
                    foreach ($TotalPremios as $key3 => $values) {
                        ...
                        if ($SorteoDetalle->permiteGanador == '1') {
                            ...
                            array_push($rules, array("field" => "usuario_sorteo.estado", "data" => "'A'", "op" => "in"));

                        } else {
                            array_push($rules, array("field" => "usuario_sorteo.estado", "data" => "'A','R'", "op" => "in"));
                        }

                        if ($SorteoDetalle->jugadorExcluido == '1') {

                            $rules2 = [];
                            array_push($rules2, ['field' => 'usuario_sorteo.estado', 'data' => 'R', 'op' => 'eq']);
                            array_push($rules2, ['field' => 'usuario_sorteo.sorteo_id', 'data' => $SorteoInterno->sorteoId, 'op' => 'eq']);

                            $filter = json_encode(['rules' => $rules2, 'groupOp' => 'AND']);
                            $UsuarioSorteo2 = new UsuarioSorteo();
                            $allCouponsWIN = (string)$UsuarioSorteo2->getUsuarioSorteosCustom('usuario_sorteo.usuario_id', 'usuario_sorteo.ususorteo_id', 'asc', 0, 1000000, $filter, true,'');

                            $allCouponsWIN = json_decode($allCouponsWIN, true);
                            $usuariosGanadores =array();
                            foreach ($allCouponsWIN['data'] as $valuewin) {
                                array_push($usuariosGanadores, $valuewin["usuario_sorteo.usuario_id"]);
                            }
                            if (oldCount($usuariosGanadores) > 0) {
                        ...
                        array_push($rules, array("field" => "usuario_sorteo.sorteo_id", "data" => $SorteoInterno->sorteoId, "op" => "eq"));
                        ...
                        $data = $UsuarioSorteo->getUsuarioSorteosCustom("usuario_sorteo.ususorteo_id, usuario_mandante.usuario_mandante", $OrderedItem, 'asc', $SkeepRows, $MaxRows, $json, true, false,true);
                        $data = json_decode($data);
                        ...
                        if ($totalEngagedCoupons > 0 && false) {
                        ...
                        $UsuarioConfiguracion = $UsuarioConfiguracion->getUserBondABuser($data->data[0]->{"usuario_mandante.usuario_mandante"});

                        if (!empty($UsuarioConfiguracion->usuconfigId)) continue;
