...
/**
 * Clase 'CronJobSorteosStickersCompleted'
 *
 *
 *
 *
 ...
 *
 */
class CronJobSorteosStickersCompleted
{
    ...
    {
        $filename = __DIR__ . '/lastrunCronJobSorteosStickersCompleted';
        $datefilename = date("Y-m-d H:i:s", filemtime($filename));

        if ($datefilename <= date("Y-m-d H:i:s", strtotime('-15 minute'))) {
            unlink($filename);
        }
                ...
                if (true) {
                    ...
                    if ($pegatinas == 1) {
                        ...
                        if ($NUMBERCASINOSTICKERS != '') {
                            ...
                            $SorteoInterno = new SorteoInterno();
                            $sqlRepiteSorteo = "select count(*) count,usuario_id, um.usuario_mandante from preusuario_sorteo a inner join usuario_mandante um on um.usumandante_id = a.usuario_id where  a.ususorteo_id IS NULL and  a.sorteo_id='" . $value->{"sorteo_interno.sorteo_id"} . "' AND a.tipo='1' AND a.estado = 'A' GROUP BY usuario_id HAVING count >=" . $NUMBERCASINOSTICKERS . "";
                            ...
                            foreach ($repiteSorteo as $item) {
                                ...
                                if(!($this->validateBonusAbuser($item->{"um.usuario_mandante"}, $value->{"sorteo_interno.sorteo_id"}))) continue;
                                ...
                                foreach ($repiteSorteo2 as $item2) {
                                    ...
                                    if ($count == 0) {
                                        ...
                                        $sql = "INSERT INTO usuario_sorteo (sorteo_id, usuario_id, valor, posicion, valor_base, usucrea_id, usumodif_id, estado, error_id, id_externo, mandante, version, apostado, externo_id, valor_premio, premio)
                                        VALUES ( '" . $item2->{"a.sorteo_id"} . "', '" . $item2->{"a.usuario_id"} . "',1,0,'" . $item2->{"a.valor_base"} . "',0,0,'A',0,0,'" . $item2->{"a.mandante"} . "',1,0,0,0,'')";
                        ...
                        if ($NUMBERSPORTSBOOKSTICKERS != '') {
                            ...
                            $SorteoInterno = new SorteoInterno();
                            $sqlRepiteSorteo = "select count(*) count,usuario_id, um.usuario_mandante from preusuario_sorteo a inner join usuario_mandante um on um.usumandante_id = a.usuario_id where  a.ususorteo_id IS NULL and  a.sorteo_id='" . $value->{"sorteo_interno.sorteo_id"} . "' AND a.tipo='2' AND a.estado = 'A' GROUP BY usuario_id HAVING count >=" . $NUMBERSPORTSBOOKSTICKERS . "";
                            print_r($sqlRepiteSorteo);
                        ...
                        if ($NUMBERDEPOSITSTICKERS != '') {
                            ...
                            $SorteoInterno = new SorteoInterno();
                            $sqlRepiteSorteo = "select count(*) count,usuario_id, um.usuario_mandante from preusuario_sorteo a inner join usuario_mandante um on um.usumandante_id = a.usuario_id where  a.ususorteo_id IS NULL and  a.sorteo_id='" . $value->{"sorteo_interno.sorteo_id"} . "' AND a.tipo='3' AND a.estado = 'A' GROUP BY usuario_id HAVING count >=" . $NUMBERDEPOSITSTICKERS . "";
                            print_r($sqlRepiteSorteo);
    ...
    /**
     * Valida si el usuario tiene contingencia de abusador de bono
     */
    public function validateBonusAbuser($usuario_id, $sorteo_id)
    {
        $UsuarioConfiguracion = new UsuarioConfiguracion();
        $UsuarioConfiguracion = $UsuarioConfiguracion->getUserBondABuser($usuario_id);
        ...
        if (!empty($UsuarioConfiguracion->usuconfigId)){
            ...
            VALUES ('{$usuario_id}','SORTEO','{$sorteo_id}','CASINO','0','Contingencia activa: Abusador de bonos')";
