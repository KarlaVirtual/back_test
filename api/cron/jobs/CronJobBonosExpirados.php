...
";
        ...
        foreach ($datosBonosAExpirar as $datanum) {
            $BonoDetalleMySqlDAO = new BonoDetalleMySqlDAO();
            $transaccion = $BonoDetalleMySqlDAO->getTransaction();
            $BonoInterno->execQuery($transaccion, "UPDATE usuario_bono SET estado='E' WHERE usubono_id='" . $datanum->{'usuario_bono.usubono_id'} . "'  AND usuario_bono.estado = 'A'; ");
            $transaccion->commit();
...
";
        ...
        foreach ($datosBonosAExpirar as $datanum) {
            $BonoDetalleMySqlDAO = new BonoDetalleMySqlDAO();
            $transaccion = $BonoDetalleMySqlDAO->getTransaction();
            $BonoInterno->execQuery($transaccion, "UPDATE usuario_bono SET estado='E',fecha_expiracion='" . $datanum->{'.fecha_expiracion'} . "' WHERE usubono_id='" . $datanum->{'usuario_bono.usubono_id'} . "' AND usuario_bono.estado = 'A' ;");
            $transaccion->commit();
...
where
	ub.estado in ('P','L')
	and bono_detalle.tipo = 'EXPIRATIONDATESOFOUTSTANDINGBONDS'
...
// Si hay bonos a actualizar, realizar la actualización
        ...
        if (!empty($usubonoId)) {
            foreach ($usubonoId as $id) {
                ...
                $sqlUpdate = "UPDATE usuario_bono SET estado = 'E' WHERE usubono_id = $id  AND usuario_bono.estado in ('P','L') ";
                $BonoInterno = new BonoInterno();
...
// Recorrer el arreglo y procesar los datos
        foreach ($BonosDiasAExpirar as $item) {
            ...
            if (strtotime($fechaFinal) < time()) {
                ...
                $sqlUpdate = "UPDATE usuario_bono SET estado = 'E' WHERE usubono_id = $usubonoId  AND usuario_bono.estado in ('P')  ";
